...
int main(int argc, char *argv[])
{
  struct options opts[] = 
    {
      { CONNECTION, "connection", "Connect using the named connection", "C", 1},
      { INTERFACE, "interface", "Interface to use with the connection (for verification)", "I", 1},
      { 0, NULL, NULL, 0}
    };
  ...
  switch (result)
    ...
    case REQUEST_TIMEOUT:
      printf("\tThe request to change the connection timed out.  (Is the supplicant running?\n");
      return RET_TIMEOUT;
      break;

    case REQUEST_SUCCESS:
      printf("\tSuccess..  Your authentication show now be running.\n");
      break;
