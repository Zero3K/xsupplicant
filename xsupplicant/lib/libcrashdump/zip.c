...
/************************************************************/
extern zipFile ZEXPORT zipOpen2(pathname, append, globalcomment,
				pzlib_filefunc_def)
const char *pathname;
int append;
...
#    ifndef NO_ADDFILEINEXISTINGZIP
	...
	if (append == APPEND_STATUS_ADDINZIP) {
		...
		central_pos =
		    ziplocal_SearchCentralDir(&ziinit.z_filefunc,
					      ziinit.filestream);
		if (central_pos == 0)
			err = ZIP_ERRNO;
		...
		if (size_comment > 0) {
			...
			if (ziinit.globalcomment) {
				size_comment =
				    ZREAD(ziinit.z_filefunc, ziinit.filestream,
					  ziinit.globalcomment, size_comment);
...
extern int ZEXPORT zipOpenNewFileInZip3(file, filename, zipfi,
					...
					extrafield_global,
					size_extrafield_global, comment, method,
					level, raw, windowBits, memLevel,
					strategy, password, crcForCrypting)
zipFile file;
const char *filename;
...
#    endif
	...
	for (i = 0; i < size_extrafield_global; i++)
		*(zi->ci.central_header + SIZECENTRALHEADER + size_filename +
		  i) = *(((const char *)extrafield_global) + i);
	...
	if ((err == ZIP_OK) && (size_filename > 0))
		if (ZWRITE
		    (zi->z_filefunc, zi->filestream, filename,
...
#    endif
	...
	if (err == ZIP_OK)
		err =
		    add_data_in_datablock(&zi->central_dir,
