 ...
 *         libxml2.
 **/
xmlNodePtr xsupconfwrite_plugin_create_tree(struct config_plugins *plugs, uint8_t config_type, 
											 char write_all)
{
	...
	if (plugnode == NULL)
	{
		...
		printf("Couldn't allocate memory to store <Plugin> block!\n");
#endif
		return NULL;
	}
	...
	if ((write_all == TRUE) || (plugs->name != NULL))
	{
		...
		if (xmlNewChild(plugnode, NULL, (xmlChar *)"Name", (xmlChar *)temp) == NULL)
		{
#ifdef WRITE_PLUGINS_CONFIG
	...
	if ((write_all == TRUE) || (plugs->path != NULL))
	{
		...
		if (xmlNewChild(plugnode, NULL, (xmlChar *)"Path", (xmlChar *)temp) == NULL)
		{
#ifdef WRITE_PLUGINS_CONFIG
			printf("Couldn't allocate memory to store <Path> node!\n");
#endif
			xmlFreeNode(plugnode);
			free(temp);
			return NULL;
 ...
 *         libxml2.
 **/
xmlNodePtr xsupconfwrite_plugins_create_tree(struct config_plugins *plugs, uint8_t config_type,
													 char write_all, char write_to_disk)
{
	...
	{
#ifdef WRITE_PLUGINS_CONFIG
		printf("Couldn't allocate memory to store <Plugins> block!\n");
#endif
		return NULL;
	}
	...
	while (cur != NULL)
	{
		plugnode = xsupconfwrite_plugin_create_tree(cur, config_type, write_all);
		if (plugnode == NULL)
		{
#ifdef WRITE_PLUGINS_CONFIG
