...
void *xsupconfig_parse_eap_ttls_chap(void **attr, uint8_t config_type,
				     xmlNodePtr node)
{
	...
	if (ttls->phase2_data == NULL) {
		printf
		    ("Couldn't allocate memory to store CHAP configuration for "
		     "EAP-TTLS!  (Line %ld)\n",
		     xsupconfig_parse_get_line_num());
		exit(2);
	}
...
void *xsupconfig_parse_eap_ttls_chap_password(void **attr, uint8_t config_type,
					      xmlNodePtr node)
{
...
void *xsupconfig_parse_eap_ttls_enc_chap_password(void **attr,
						  uint8_t config_type,
						  xmlNodePtr node)
{
	struct config_pwd_only *chap = NULL;
...
parser eap_ttls_chap[] = {
	...
	{"Encrypted_Password", NULL, FALSE, OPTION_ANY_CONFIG,
	 &xsupconfig_parse_eap_ttls_enc_chap_password}
};
