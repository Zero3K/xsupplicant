...
ttls_phase2_methods ttls_p2_meths[] = {
	...
	{"chap", TTLS_PHASE2_CHAP, eap_ttls_chap, xsupconfig_parse_eap_ttls_chap},
	{"ms-chap", TTLS_PHASE2_MSCHAP, eap_ttls_mschap, xsupconfig_parse_eap_ttls_mschap},
	{"ms-chapv2", TTLS_PHASE2_MSCHAPV2, eap_ttls_mschapv2, xsupconfig_parse_eap_ttls_mschapv2},
...
ttls_phase2_methods *xsupconfig_parse_eap_ttls_phase2_get_method(int methnum)
{
	...
	{
		if (ttls_p2_meths[i].type_num == methnum) break;
		i++;
	}
...
void *xsupconfig_parse_eap_ttls_phase2(void **attr, xmlNodePtr node)
{
  ...
  if (meths->name == NULL)
  {
	  xsupconfig_common_log("Invalid EAP-TTLS phase 2 type '%d' requested!\n", cur->phase2_type);
	  return NULL;
  }
  ...
  {
	  xsupconfig_common_log("No method was available to init method!  (Fix the code!)\n");
  }
...
void *xsupconfig_parse_eap_ttls_phase2_eap(void **attr, xmlNodePtr node)
{
  ...
  if (t == NULL)
  {
	  xsupconfig_common_log("EAP-TTLS phase 2 configuration doesn't contain a valid EAP type to use!\n");
	  return NULL;
  }
  ...
  if (t == NULL)
  {
	  xsupconfig_common_log("EAP-TTLS phase 2 configuration data doesn't contain a valid EAP type to use!\n");
	  return NULL;
  }
  ...
  if (meth->name == NULL)
  {
	  xsupconfig_common_log("Invalid EAP-TTLS phase 2 method '%s' requested!\n", value);
	  return NULL;
  }
