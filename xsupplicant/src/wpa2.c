...
uint8_t wpa2_get_pairwise_crypt(context *ctx)
{
  ...
  if (keytype != 0)
  {
	if (keytype & CRYPT_FLAGS_CCMP) return CIPHER_CCMP;
	if (keytype & CRYPT_FLAGS_TKIP) return CIPHER_TKIP;
	if (keytype & CRYPT_FLAGS_WEP104) return CIPHER_WEP104;
	if (keytype & CRYPT_FLAGS_WEP40) return CIPHER_WEP40;
  }
