...
uint8_t eapol_get_eapol_ver(context *ctx)
{
  ...
  if (!xsup_assert((ctx != NULL), "ctx != NULL", FALSE)) return 2;

  if (ctx->conn == NULL)
    {
      debug_printf(DEBUG_INT, "No configuration available to determine "
...
int eapol_execute(context *workint)
{
  ...
  {
	  debug_printf(DEBUG_NORMAL, "Request to run the EAPoL check code without having "
  ...
  if (workint->recv_size > 0)
	...
	{
	  if (ntohs(temp->eapol_length) > (workint->recv_size - OFFSET_TO_EAP))
	  {
		  debug_printf(DEBUG_NORMAL, "Interface '%s' received a runt frame.  This frame has been discarded.  (Expected : %d,  Got : %d)\n",
			  workint->desc, ntohs(temp->eapol_length), (workint->recv_size - OFFSET_TO_EAP));
	  ...
	  if (temp->eapol_version > MAX_EAPOL_VER)
	    {
	      debug_printf(DEBUG_NORMAL, "EAPoL version too high! (Version "
			   "%d > %d)  Discarding!\n", temp->eapol_version,
			   MAX_EAPOL_VER);
	      snmp_inc_dot1xSuppInvalidEapolFramesRx();
	      workint->recv_size = 0;
		...
		case EAPOL_START:
		  ...
		  debug_printf(DEBUG_DOT1X_STATE, "Got EAPoL-Start! Ignoring!\n");
		...
		case EAPOL_LOGOFF:
		  ...
		  debug_printf(DEBUG_DOT1X_STATE, "Got EAPoL-Logoff! Ignoring!\n");
		  return XEIGNOREDFRAME;

		case EAPOL_KEY:
		  debug_printf(DEBUG_KEY_STATE, "Processing EAPoL-Key!\n");
	  ...
	  debug_printf(DEBUG_DOT1X_STATE, "Got a frame, but it isn't an EAPoL "
  ...
  if (sm_runs == 0xff)
    {
      debug_printf(DEBUG_NORMAL, "The state machine appears to have gotten "
