...
int statemachine_init(context * ctx)
{
	...
	ctx->statemachine =
	    (struct dot1x_state *)Malloc(sizeof(struct dot1x_state));
...
int statemachine_reinit(context * ctx)
{
	...
	if (ctx->desc != NULL) {
		...
		debug_printf((DEBUG_DOT1X_STATE | DEBUG_VERBOSE),
			     "Reinit state machine on interface '%s'.\n",
			     ctx->desc);
	}
...
int statemachine_change_to_s_force_auth(context * ctx)
{
	...
	debug_printf(DEBUG_DOT1X_STATE | DEBUG_VERBOSE,
		     "%s -- (global) -> S_FORCE_AUTH\n", ctx->desc);

	// Verify that we have valid parameters.
	if (!ctx) {
		debug_printf(DEBUG_NORMAL,
			     "Invalid interface context! (%s:%d)\n",
	...
	if (ipc_events_statemachine_transition
	    (ctx, IPC_STATEMACHINE_8021X, ctx->statemachine->curState,
	     S_FORCE_AUTH) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC 802.1X state change message!\n");
	}
#endif
	...
	if (cardif_windows_get_uptime(&ctx->statemachine->last_reauth) != 0) {
		debug_printf(DEBUG_NORMAL,
			     "Unable to determine the system uptime.  Your time connected counter will be wrong!\n");
	}
...
int statemachine_change_to_s_force_unauth(context * ctx)
{
	...
	// Display the state we are changing to.
	debug_printf((DEBUG_DOT1X_STATE | DEBUG_VERBOSE),
		     "%s -- (global) -> S_FORCE_UNAUTH\n", ctx->desc);

#if DOT1X_SM_EVENTS == 1
	// Let all connected listeners know that we transitioned state.
	if (ipc_events_statemachine_transition
	    (ctx, IPC_STATEMACHINE_8021X, ctx->statemachine->curState,
	     S_FORCE_UNAUTH) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC 802.1X state change message!\n");
	}
...
int statemachine_change_to_held(context * ctx)
{
	...
	if ((ctx->statemachine->curState != CONNECTING) &&
		...
		debug_printf(DEBUG_NORMAL,
			     "Invalid attempt to change to HELD state! "
...
int statemachine_change_to_connecting(context * ctx)
{
	...
	if ((ctx->statemachine->curState != CONNECTING) &&
	    ...
	    (ctx->statemachine->curState != AUTHENTICATING)) {
		debug_printf(DEBUG_NORMAL,
			     "Attempt to change to CONNECTING state "
	...
	if (ctx->conn != NULL) {
		...
		ctx->statemachine->startCount =
		    ctx->statemachine->startCount + 1;
	}
...
int statemachine_change_to_restart(context * ctx)
{
	...
	if ((ctx->statemachine->curState != AUTHENTICATED) &&
	    ...
	    (ctx->statemachine->curState != S_FORCE_UNAUTH)) {
		debug_printf(DEBUG_NORMAL,
			     "Attempt to change to RESTART state from "
...
int statemachine_change_to_authenticating(context * ctx)
{
	...
	if ((ctx->statemachine->curState != CONNECTING) &&
	    (ctx->statemachine->curState != RESTART)) {
		debug_printf(DEBUG_NORMAL,
			     "Attempt to change to AUTHENTICATING state "
...
void statemachine_do_authenticating(context * ctx)
{
	...
	if (ctx->statemachine->suppSuccess && ctx->statemachine->portValid) {
...
int statemachine_change_to_authenticated(context * ctx)
{
	...
	if (cardif_windows_get_uptime(&ctx->statemachine->last_reauth) != 0) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't determine the current uptime.  Your time authenticated counter will be wrong.\n");
	}
	...
	if (cardif_macosx_wireless_set_key_material(ctx) != XENONE) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't apply keys to the interface!\n");
	}
#endif
	...
	if ((!globals)
	    || (TEST_FLAG(globals->flags, CONFIG_GLOBALS_PASSIVE_SCAN))) {
		if (ctx->intType == ETH_802_11_INT) {
			// Start scan timer.
			if (timer_check_existing(ctx, PASSIVE_SCAN_TIMER) ==
			    FALSE) {
				// Set up a new timer, since this is the first time we have set a timer.
				debug_printf((DEBUG_DOT1X_STATE |
					      DEBUG_VERBOSE),
					     "Starting new passive scan timer.\n");
				timer_add_timer(ctx, PASSIVE_SCAN_TIMER,
						globals->passive_timeout, NULL,
...
int statemachine_is_success(context * ctx)
{
	...
	if (!xsup_assert
	    ((ctx->eap_state != NULL), "ctx->eap_state != NULL", FALSE))
		return FALSE;

	if (!xsup_assert((ctx->eap_state->eapReqData != NULL),
			 "ctx->eap_state->eapReqData != NULL", FALSE))
		return FALSE;
...
void statemachine_do_authenticated(context * ctx)
{
	...
	if ((ctx->statemachine->eapolEap == TRUE)
	    && (eapol_get_eap_type(ctx) == EAP_SUCCESS_PKT)) {
		ctx->statemachine->eapolEap = FALSE;
		debug_printf(DEBUG_DOT1X_STATE,
			     "Looks like another success...  Discarding.\n");
		FREE(ctx->recvframe);
		ctx->recv_size = 0;
	}
...
int statemachine_change_state(context * ctx, int newstate)
{
	...
	case ACQUIRED:
		debug_printf(DEBUG_NORMAL,
			     "Attempt to change to ACQUIRED state. "
			     "WTF are you doing!?\n");
		retval = statemachine_change_to_disconnected(ctx);
		break;
...
void statemachine_timer_tick(context * ctx)
{
	...
	debug_printf(DEBUG_DOT1X_STATE,
		     "Clock tick for '%s'! authWhile=%d heldWhile=%d "
		     "startWhen=%d curState=%s\n", ctx->desc,
		     ctx->statemachine->authWhile, ctx->statemachine->heldWhile,
...
{
	debug_printf((DEBUG_DOT1X_STATE | DEBUG_DEINIT | DEBUG_VERBOSE),
		     "Doing statemachine cleanup!\n");

	if (!ctx) {
		debug_printf(DEBUG_NORMAL,
			     "Invalid data passed in to statemachine_cleanup()!\n");
		return XEMALLOC;
	}
...
int txLogoff(context * ctx)
{
	...
	if ((ctx->prof == NULL) || (ctx->prof->method == NULL)) {
		debug_printf(DEBUG_DEINIT,
			     "No network information available for interface '%s'.  Assuming"
	...
	if (ctx->conn == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "No connection information available for interface '%s'.  Not sending LOGOFF!\n",
			     ctx->desc);
		return XENONE;
	}
	...
	if ((ctx->conn->association.auth_type == AUTH_NONE) &&
	    ((ctx->conn->association.association_type == ASSOC_AUTO) ||
	     ...
	     (ctx->conn->association.association_type == ASSOC_SHARED)))
		return XENONE;
...
int txStart(context * ctx)
{
	...
	if (ctx->conn != NULL) {
		if (((ctx->conn->association.association_type == ASSOC_WPA) ||
			...
			debug_printf(DEBUG_DOT1X_STATE,
				     "Doing WPA-PSK.. Not sending start!\n");
			return XENONE;
		}
	}
	...
	if (ctx->sendframe == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "There was no buffer allocated to send the "
			     "frame!\n");
		return XEMALLOC;
	}
