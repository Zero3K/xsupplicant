...
uint8_t *prf_plus(uint8_t *key, uint8_t *seed, uint8_t len)
{
	...
	if (!xsup_assert((len < 256), "len < 256", FALSE)) return NULL;			// This should be impossible, but put it here anyway in case we ever change the type of len in the future.

	iterations = (len / 20);

	if ((len % 20) != 0) iterations++;   // We need a fractional amount of data, so round up.

	Tn = Malloc(iterations * 20);
	if (Tn == NULL)
	{
		debug_printf(DEBUG_NORMAL, "Unable to allocate memory to hash cryptobinding data!\n");
		return NULL;
	}
	...
	debug_hex_dump(DEBUG_AUTHTYPES, seed, PEAP_CRYPTOBINDING_IPMK_SEED_LEN);
	...
	debug_hex_dump(DEBUG_AUTHTYPES, temp_data, (PEAP_CRYPTOBINDING_IPMK_SEED_LEN+3));
	// Malloc should have inited everything else to 0x00, so we don't need to set those.

	HMAC(EVP_sha1(), key, 40, temp_data, (PEAP_CRYPTOBINDING_IPMK_SEED_LEN+3), (unsigned char *)&mac, 
	   &mdlen);

	if (mdlen != 20)
	{
		debug_printf(DEBUG_NORMAL, "The SHA1 hash function didn't return valid data!\n");
		FREE(temp_data);
		FREE(Tn);
		return NULL;
	}
	...
	if (iterations >= 2)
	{
		...
		{
			temp_data = Malloc(20 + PEAP_CRYPTOBINDING_IPMK_SEED_LEN+3);
			if (temp_data == NULL)
			{
				debug_printf(DEBUG_NORMAL, "Unable to allocate memory to hash cryptobinding data!\n");
				return NULL;
			...
			debug_hex_dump(DEBUG_AUTHTYPES, temp_data, (20+PEAP_CRYPTOBINDING_IPMK_SEED_LEN+3));

			// Malloc should have inited everything else to 0x00, so we don't need to set those.

			HMAC(EVP_sha1(), key, 40, temp_data, (20 + PEAP_CRYPTOBINDING_IPMK_SEED_LEN+3), (unsigned char *)&mac, 
			   &mdlen);

			if (mdlen != 20)
			{
				debug_printf(DEBUG_NORMAL, "SHA1 hash didn't return valid data in %s()!\n", __FUNCTION__);
				FREE(temp_data);
				FREE(Tn);
				return NULL;
			}
 ...
 **/
void peap_extensions_process_cryptobinding_tlv(eap_type_data *eapdata, struct phase2_data *p2d, uint8_t *in, uint16_t in_size, 
		      uint8_t *out, uint16_t *out_size)
{
	...
	if (!xsup_assert((in != NULL), "in != NULL", FALSE)) return;
	if (!xsup_assert((in_size > 0), "in_size > 0", FALSE)) return;
	if (!xsup_assert((out != NULL), "out != NULL", FALSE)) return;
	if (!xsup_assert((out_size != NULL), "out_size != NULL", FALSE)) return;

	tlvdata = (peap_tlv_header *)in;

	if ((ntohs(tlvdata->tlv_type) & (~PEAP_TLV_TYPE_FLAGS)) != PEAP_TLV_CRYPTOBINDING)
	{
		debug_printf(DEBUG_NORMAL, "Error processing PEAP TLV extension!   %s() was called, but the TLV handed in wasn't a result TLV!\n", __FUNCTION__);
		return;
	}
	...
	{
		debug_printf(DEBUG_NORMAL, "Server sent us something other than a cryptobinding request!?\n");
		return;
	}
	...
	if (mytls_vars == NULL) 
	{
		debug_printf(DEBUG_NORMAL, "Unable to obtain TLS key data needed for PEAP cryptobinding!\n");
		return;
	}
	...
	if (ipmk == NULL)
	{
		debug_printf(DEBUG_NORMAL, "Unable to generate the IPMK for PEAP Cryptobinding.\n");
		return;
	}
	...
	HMAC(EVP_sha1(), cmk, 20, &tohash_data[0], 61, (unsigned char *)&mac, 
		   &mdlen);

	if (mdlen != 20)
	{
		debug_printf(DEBUG_NORMAL, "Unable to compute the Compound MAC.  SHA-1 didn't return 20 bytes of data!\n");
		return;
	}
 ...
 * @param[out] out_size   A pointer to an integer that will be incremented by the size of the TLV we write back.
 **/
void peap_extensions_process_result_tlv(struct phase2_data *p2d, uint8_t *in, uint16_t in_size, 
		      uint8_t *out, uint16_t *out_size)
{
	peap_tlv_header *tlvdata = NULL;
	uint16_t *result_val = NULL;
	...
	if ((ntohs(tlvdata->tlv_type) & (~PEAP_TLV_TYPE_FLAGS)) != PEAP_TLV_RESULT)
	{
		debug_printf(DEBUG_NORMAL, "Error processing PEAP TLV extension!   %s() was called, but the TLV handed in wasn't a result TLV!\n", __FUNCTION__);
		return;
	}
	...
	case PEAP_TLV_RESULT_RESERVED:
		debug_printf(DEBUG_NORMAL, "Server sent us a reserved, or unknown result code of %d.\n", result_val);
		// Fall through.
		      ...
		      uint8_t *out, uint16_t *out_size)
{
	...
	if (!xsup_assert((eapdata != NULL), "eapdata != NULL", FALSE)) return XEGENERROR;
	if (!xsup_assert((p2d != NULL), "p2d != NULL", FALSE)) return XEGENERROR;
	if (!xsup_assert((in != NULL), "in != NULL", FALSE)) return XEGENERROR;
	if (!xsup_assert((in_size > 0), "in_size > 0", FALSE)) return XEGENERROR;
	if (!xsup_assert((out != NULL), "out != NULL", FALSE)) return XEGENERROR;
	if (!xsup_assert((out_size != NULL), "out_size != NULL", FALSE)) return XEGENERROR;
	...
	while (dataofs < in_size)
	{
		...
		debug_printf(DEBUG_AUTHTYPES, "Masked TLV TYPE : %d\n", (ntohs(tlvdata->tlv_type) & (~PEAP_TLV_TYPE_FLAGS)));
		debug_printf(DEBUG_AUTHTYPES, "Length          : %d\n", ntohs(tlvdata->tlv_length));
		if ((ntohs(tlvdata->tlv_type) & PEAP_TLV_MANDATORY_FLAG) == PEAP_TLV_MANDATORY_FLAG)
		{
		...
		case PEAP_TLV_RESULT:
			debug_printf(DEBUG_AUTHTYPES, "TLV is a Result TLV.\n");
			peap_extensions_process_result_tlv(p2d, dataptr, (in_size - dataofs), &out[outdataofs], out_size);
		...
		case PEAP_TLV_CRYPTOBINDING:
			debug_printf(DEBUG_AUTHTYPES, "TLV is a Cryptobinding TLV.\n");
			peap_extensions_process_cryptobinding_tlv(eapdata, p2d, dataptr, (in_size - dataofs), &out[outdataofs], out_size);
		...
		default:
			debug_printf(DEBUG_AUTHTYPES, "TLV is of unknown type %d.\n", (ntohs(tlvdata->tlv_type) & (~PEAP_TLV_TYPE_FLAGS)));
			dataofs += sizeof(peap_tlv_header)+ntohs(tlvdata->tlv_length);
			break;
		}
	}
