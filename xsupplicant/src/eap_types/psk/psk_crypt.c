 ...
 *
 **/ 
void cmac_aes_128_subkey(unsigned char *key, unsigned char *k1,
			 unsigned char *k2) 
{
...
void AES_counter_encrypt(uint8_t * key, uint8_t * nonce, uint8_t * data,
			   uint16_t data_size,  uint8_t ** result,
			   uint16_t * res_size) 
{
		  ...
		  unsigned char **out, unsigned int *out_size) 
{
		...
		return 0;
			      ...
			      "PSK tags do not match.  Failing authentication.\n");
		return 0;
		}
	AES_counter_encrypt(key, nonce_mac, payload, payload_size, &temp_data,
			      &res_size);
