...
void eappsk_process_packet2(eap_type_data * eapdata) 
{
	...
	mac_s =	cmac_aes_128(pskdata->ak, mac_s_data,
	...
	if (eax_decrypt(pskdata->keydata, pskpacket->pchn_nonce, (unsigned char *)eaphdr,
...
uint8_t * eappsk_create_packet1(eap_type_data * eapdata) 
{
	...
	size = sizeof(struct eap_header) + 1 + 16 + 16 + 16 +
...
uint8_t * eappsk_create_packet3(eap_type_data * eapdata) 
{
	...
	if (eax_encrypt(pskdata->keydata, &pskdata->nonce[0], (unsigned char *)eaphdr,
