...
void eappsk_init(eap_type_data * eapdata) 
{
		...
		FREE(eapdata->eap_data);
			      ...
			      "Unable to allocate memory to store EAP-PSK state data.\n");
...
void eappsk_check(eap_type_data * eapdata) 
{
	      ...
	      ((eapdata->eapReqData != NULL), "eapdata->eapReqData != NULL",
	       FALSE))
			      ...
			      "There is no valid configuration for " 
			      "EAP-PSK!\n");
				      ...
				      "No password available for EAP-PSK!\n");
    ...
    uint8_t * eappsk_derive_keyblock(uint8_t * kdk, uint8_t * rand_p) 
{
	...
	if (!xsup_assert((kdk != NULL), "kdk != NULL", FALSE))
		return NULL;
	if (!xsup_assert((rand_p != NULL), "rand_p != NULL", FALSE))
		return NULL;
	    ...
	    keyblock = Malloc(16 * 9);
			      ...
			      "Unable to allocate the memory to generate keyblock!\n");
    ...
    uint8_t * eappsk_get_psk(char *password, char *id_s, char *id_p)
{
	...
	int needed_length = 0;
		    ...
		    needed_length =
		    strlen(password) + (16 - (strlen(password) % 16));
		datablock = Malloc(needed_length);
		if (datablock == NULL)
			 {
			debug_printf(DEBUG_NORMAL,
				      "Unable to allocate memory needed to store password data block!\n");
			...
			AES_ecb_encrypt(&datablock[(i * 16)], &temp_data,
					  &key, AES_ENCRYPT);
			      ...
			      "Unable to allocate memory needed to store PSK result.\n");
		return NULL;
		}
	if (PKCS5_PBKDF2_HMAC_SHA1(p16, 16, salt, 12, 5000, 16, datablock) !=
	      TRUE)
	...
	context *ctx = NULL;
	    ...
	    if (strlen(password) == 16)
		    ...
		    psk = Malloc(16);
				      ...
				      "Unable to allocate memory to store PSK information!\n");
				      ...
				      "Unable to derive the PSK from the provided password!\n");
...
void eappsk_process_packet0(eap_type_data * eapdata) 
{
	    ...
	    (eappsk_packet0 *) & eapdata->eapReqData[sizeof(struct eap_header) +
						     1];
	    ...
	    ntohs(eaphdr->eap_length) - sizeof(struct eap_header) - 1 - 16;
	pskdata->id_s = Malloc(nai_size + 2);
	if (pskdata->id_s == NULL)
		 {
		debug_printf(DEBUG_NORMAL,
			      "Unable to allocate memory needed to store ID_S!\n");
...
void eappsk_process_packet2(eap_type_data * eapdata) 
{
	    ...
	    (eappsk_packet2 *) & eapdata->eapReqData[sizeof(struct eap_header)];
			      ...
			      "The server random sent in packet 0 doesn't match the one sent in packet 2!  Authentication failed.\n");
	    ...
	    mac_s_data = Malloc(strlen(pskdata->id_s) + 16);
			      ...
			      "Unable to allocate memory to store MAC_S value!\n");
	    ...
	    cmac_aes_128(pskdata->ak, mac_s_data,
			 ((strlen(pskdata->id_s) + 16) * 8));
	    ...
	    pskdata->keydata =
	    eappsk_derive_keyblock(pskdata->kdk, pskdata->rand_p);
	eaphdr = (struct eap_header *)eapdata->eapReqData;
	pchn_size =
	    ntohs(eaphdr->eap_length) - 16 - 16 - 1 - sizeof(struct eap_header);
	      ...
	      (pskdata->keydata, pskpacket->pchn_nonce, (unsigned char *)eaphdr,
	       &pskpacket->pchn_tag[0], &pskpacket->pchn_flags, 1, &result,
	       &result_size) == 0)
		 {
		debug_printf(DEBUG_NORMAL,
			      "Failed to decrypt the protected channel.\n");
			      ...
			      "PSK extended authentication is not currently supported!\n");
			      ...
			      "Server indicates the PSK handshake was a success.\n");
		break;
	case EAP_PSK_FLAGS_FAILURE:
		debug_printf(DEBUG_NORMAL,
			      "Server indicates the PSK handshake failed.\n");
    ...
    uint8_t * eappsk_create_packet1(eap_type_data * eapdata) 
{
	    ...
	    size =
	    sizeof(struct eap_header) + 1 + 16 + 16 + 16 +
	    strlen(eapdata->ident);
	respData = Malloc(size);
	if (respData == NULL)
		 {
		debug_printf(DEBUG_NORMAL,
			      "Unable to allocate memory to store EAP-PSK response.\n");
	...
	memcpy(&pskpacket1->nai_p[0], &eapdata->ident[0],
		strlen(eapdata->ident));
	    ...
	    mac_p_data =
	    Malloc(strlen(eapdata->ident) + strlen(pskdata->id_s) + 16 + 16);
	if (mac_p_data == NULL)
		 {
		debug_printf(DEBUG_NORMAL,
			      "Unable to allocate memory needed to store MAC_P creation data!\n");
		return NULL;
		}
	memcpy(mac_p_data, eapdata->ident, strlen(eapdata->ident));
	memcpy(&mac_p_data[strlen(eapdata->ident)], pskdata->id_s,
		strlen(pskdata->id_s));
    ...
    uint8_t * eappsk_create_packet3(eap_type_data * eapdata) 
{
	    ...
	    eap_type_common_get_eap_reqid(eapdata->eapReqData);
	      ...
	      (pskdata->keydata, &pskdata->nonce[0], (unsigned char *)eaphdr,
	       &toencdata[0], 1, &encdata, &enc_size,
	       (unsigned char *)&tag[0]) != 1)
		 {
		debug_printf(DEBUG_NORMAL,
			      "Unable to encrypt protected channel data.\n");
...
void eappsk_process(eap_type_data * eapdata) 
{
	...
	flags = flags >> 6;
			      ...
			      "Receipt of packet ID 1 not allowed on supplicants!\n");
			      ...
			      "Receipt of packet ID 3 not allowed on supplicants!\n");
...
uint8_t * eappsk_buildResp(eap_type_data * eapdata) 
{
		...
		return NULL;
		    ...
		    // Not allowed.
		    debug_printf(DEBUG_NORMAL,
				 "Got a request to send a type 0 PSK packet!?  This isn't allowed!\n");
		    ...
		    // Not allowed.
		    debug_printf(DEBUG_NORMAL,
				 "Got a request to send a type 2 PSK packet!?  This isn't allowed!\n");
		    ...
		    // Mass confusion!
		    debug_printf(DEBUG_NORMAL,
				 "Got a request for type %d, which isn't defined by the standard!\n",
				 pskdata->packetnum);
