	...
	if ((phase2data == NULL) || (phase2data->password == NULL)) {
		...
		if (ctx == NULL) {
			debug_printf(DEBUG_NORMAL,
				     "No password available for TTLS-MSCHAP!\n");
			eap_type_common_fail(eapdata);
			return;
		}

		if ((TEST_FLAG(outerdata->flags, TTLS_FLAGS_USE_LOGON_CREDS))
		    && (logon_creds_password_available() == TRUE)) {
			if (ctx->prof != NULL) {
				FREE(ctx->prof->temp_password);
				ctx->prof->temp_password =
 ...
 *
 ************************************************************************/
void mschap_buildResp(eap_type_data * eapdata, uint8_t * out,
		      uint16_t * outsize)
{
	struct config_eap_ttls *outerdata = NULL;
	struct config_pwd_only *phase2data = NULL;
	...
	*outsize = 0;
	...
	if (challenge == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Invalid implicit challenge in %s()!\n",
			     __FUNCTION__);
		return;
	}
	...
	build_avp(MS_CHAP_CHALLENGE, MS_VENDOR_ATTR,
		  (MANDITORY_FLAG | VENDOR_FLAG),
		  (uint8_t *) & mschap_challenge, 8,
	...
	build_avp(MS_CHAP_RESPONSE, MS_VENDOR_ATTR,
		  (MANDITORY_FLAG | VENDOR_FLAG), mschap_answer, 50,
		  &out[avp_offset], &avp_out_size);
