...
char *get_cert_common_name(SSL * ssl_ctx)
{
	...
	if (commonName == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory to hold the common name!\n");
	...
	if (X509_NAME_get_text_by_NID(X509_get_subject_name(server_cert),
		...
		debug_printf(DEBUG_NORMAL,
			     "Couldn't extract common name from server certificate!\n");
		return NULL;
	}
...
static int ssl_verify_callback(int preverify_ok, X509_STORE_CTX * ctx)
{
	...
	TRACE if (!xsup_assert((ctx != NULL), "ctx != NULL", FALSE))
		return XEMALLOC;
	...
	if (!preverify_ok) {
		debug_printf(DEBUG_TLS_CORE,
			     "     --- SSL_verify error : num=%d:%s:depth=%d:%s\n",
...
int tls_funcs_load_engine(struct tls_vars *mytls_vars, struct smartcard *sc)
{
	if (!engine_load_dynamic_opensc(sc)) {
		debug_printf(DEBUG_NORMAL,
			     "OpenSC Engine will be unavailable!\n");
		return XEGENERROR;
	}
...
int tls_funcs_init(struct tls_vars *mytls_vars, uint8_t eaptype)
{
	...
	if (mytls_vars->resumable != TRUE) {
		...
		if (mytls_vars->ctx == NULL) {
			debug_printf(DEBUG_NORMAL,
				     "Couldn't initialize OpenSSL TLS library!\n");
...
int tls_funcs_build_new_session(struct tls_vars *mytls_vars)
{
	...
	if (mytls_vars->ctx == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "The SSL context is NULL.  (This shouldn't happen.)  Working around it.\n");
		if (tls_funcs_init(mytls_vars, mytls_vars->method_in_use) !=
		    XENONE)
...
int ossl_funcs_do_start(struct tls_vars *mytls_vars)
{
	...
	if ((mytls_vars->ssl == NULL)
		...
		if (mytls_vars->resume != FALSE) {
			...
			if (!sess) {
				debug_printf(DEBUG_TLS_CORE,
					     "Couldn't get session information!"
	...
	if (sess != NULL) {
		...
		if (SSL_set_session(mytls_vars->ssl, sess) <= 0) {
			debug_printf(DEBUG_NORMAL,
				     "There was an error attempting to resume "
				     "the session!\n");
			tls_funcs_process_error();
		}
	}
	...
	if (tempdata == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory to store return "
			     "TLS data!\n");
		ipc_events_malloc_failed(NULL);
		return XEMALLOC;
	}

	ressize = BIO_read(mytls_vars->ssl_out, tempdata, 1500);
	if (ressize <= 0) {
		FREE(tempdata);
		return XEGENERROR;
	}
	...
	if (NULL == mytls_vars->tlsoutqueue) {
		...
		if (queue_create(&mytls_vars->tlsoutqueue) < 0) {
			debug_printf(DEBUG_NORMAL,
				     "Couldn't create data queue for response data!\n");
			FREE(tempdata);
			return XEGENERROR;
		}
	}
	...
	if (queue_enqueue(&mytls_vars->tlsoutqueue, tempdata, ressize) < 0) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't enqueue response data!  Destroying queue.\n");
		if (queue_destroy(&mytls_vars->tlsoutqueue) < 0) {
			debug_printf(DEBUG_NORMAL,
				     "Couldn't destroy queue!  This is *REALLY* bad!\n");
			FREE(tempdata);
			return XEGENERROR;
		}
	}
 ...
 **/
int tls_funcs_get_embedded_appdata(struct tls_vars *mytls_vars,
				   uint8_t ** buffer, uint16_t * size)
{
	...
	if (!xsup_assert((size != NULL), "size != NULL", FALSE))
		return -1;

	if (!SSL_is_init_finished(mytls_vars->ssl)) {
		debug_printf(DEBUG_TLS_CORE,
			     "Attempted to get appdata from an SSL session that wasn't finished.\n");
		return -1;
	}
	...
	if ((*buffer) == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Unable to acquire a buffer to store embedded application data!\n");
		return -1;
	}
	...
	if (result <= 0) {
		debug_printf(DEBUG_AUTHTYPES,
			     "No embedded application data found! (result = %d) - OpenSSL error : %d\n",
...
int tls_funcs_wildcard_match(char *pattern, char *target)
{
	...
	if (!xsup_assert((pattern != NULL), "pattern != NULL", FALSE))
		return XEBADCN;

	if (!xsup_assert((target != NULL), "target != NULL", FALSE))
		return XEBADCN;
	...
	if (offset == NULL)
		return XEBADCN;	// The substring wasn't found.  The CN is invalid.
		...
		if (mytls_vars->cnexact == 1) {
			...
			if (cnname != NULL) {
				if (strcmp(mytls_vars->cncheck, cnname) != 0) {
					...
					debug_printf(DEBUG_TLS_CORE,
						     "Certificate CN matched!\n");
				}
			}
		} else {
			debug_printf(DEBUG_TLS_CORE,
				     "Looking for a relative match!\n");
			...
			if (cnname != NULL) {
				...
				do {
					...
					if (temp2 != NULL)
						temp2 = 0x00;	// Convert the character to a NULL so that temp just points to a single entry.
				 ...
				 uint8_t * eappacket)
{
	...
	if (mytls_vars->ssl == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "The server sent a mid-conversation message when we expected a start.  If this is a wireless connection, it is likely that the wireless card hopped to a new AP in the middle of a conversation.\n");
	...
	if (temp == 0x00) {
		...
		if (mytls_vars->expected_in == 0) {
			...
			debug_printf(DEBUG_TLS_CORE,
				     "Expecting a total of %d byte(s). (Single packet message)\n",
	...
	if ((temp & EAPTLS_MORE_FRAGS) == EAPTLS_MORE_FRAGS) {
		debug_printf(DEBUG_AUTHTYPES,
			     "More fragments are expected, will send an ACK.\n");
	...
	if (mytls_vars->send_ack == FALSE) {
		...
		debug_printf(DEBUG_TLS_CORE,
			     "Writing %d byte(s) of data to OpenSSL.\n",
			     mytls_vars->expected_in);
		...
		if (err != 1) {
			...
			if (err != 0) {
				debug_printf(DEBUG_INT,
					     "OpenSSL error : %d  (%d, %d, %d)\n",
					     err, ERR_GET_LIB(err),
					     ERR_GET_FUNC(err),
					     ERR_GET_REASON(err));

#ifdef OPENSSL_HELLO_EXTENSION_SUPPORTED
				if ((mytls_vars->method_in_use == EAP_TYPE_FAST)
				    && (ERR_GET_REASON(err) ==
					SSL_R_SSLV3_ALERT_BAD_CERTIFICATE)) {
					// The server indicated our PAC is invalid.
					debug_printf(DEBUG_NORMAL,
						     "Your EAP-FAST PAC has expired.  Flushing and requesting a new provision.\n");
					return PAC_EXPIRED;
				}
				...
				if ((ERR_GET_REASON(err) ==
					...
					if (ctx == NULL)	// Shouldn't be possible!
					{
						...
						if (error_str != NULL) {
							sprintf(error_str,
								"Authentication handshake failed.  Reason : %s",
							...
							ipc_events_error(NULL,
									 IPC_EVENT_ERROR_TLS_HANDSHAKE_FAILED,
									 error_str);
							FREE(error_str);
						}
						...
						debug_printf(DEBUG_NORMAL,
							     "Got an unexpected message on interface '%s'.  Trying to restart authentication.\n",
							     ctx->desc);
						if (ctx->intType ==
						    ETH_802_11_INT) {
		...
		debug_printf(DEBUG_AUTHTYPES,
			     "BIO indicates there are %d byte(s) available to read.\n",
			     size);
		if (size > 0) {
			tempdata = Malloc(size);
			if (tempdata == NULL) {
				debug_printf(DEBUG_NORMAL,
					     "Couldn't allocate memory to store OpenSSL's response data!\n");
				return DONE;
			}

			err = BIO_read(mytls_vars->ssl_out, tempdata, size);
			if (err > 0) {
				if (mytls_vars->tlsoutqueue == NULL) {
					if (queue_create
					    (&mytls_vars->tlsoutqueue) < 0) {
						debug_printf(DEBUG_NORMAL,
							     "Couldn't create queue data for OpenSSL's TLS response data!\n");
						FREE(tempdata);
						return DONE;
					}
				}
				...
				if (queue_enqueue
				    (&mytls_vars->tlsoutqueue, tempdata,
				     size) < 0) {
					debug_printf(DEBUG_NORMAL,
						     "Couldn't enqueue OpenSSL's TLS response data!\n");
					FREE(tempdata);
					return DONE;
				}
			} else {
				debug_printf(DEBUG_NORMAL,
					     "The BIO claimed there was data "
	...
	if (SSL_get_state(mytls_vars->ssl) == 0x0003) {
		if (tls_funcs_cn_check(mytls_vars) != XENONE) {
			debug_printf(DEBUG_NORMAL,
				     "Failed certificate common name "
				     "check!\n");
			queue_destroy(&mytls_vars->tlsinqueue);
			queue_destroy(&mytls_vars->tlsoutqueue);
			return DONE;
...
{
	TRACE if (eappacket[sizeof(struct eap_header)] == EAPTLS_START) {
			 ...
			 uint8_t ** result, uint16_t * res_size)
{
	...
	res_size32 = (*res_size);
	more =
	    queue_dequeue(&mytls_vars->tlsoutqueue, &dequeue_data, &res_size32);
	*res_size = (uint16_t) res_size32;	// The value of res_size32 will always fit in *res_size

	if (more < 0) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't get more fragments to send to authenticator.\n");
		return XEGENERROR;
	}
	...
	if (queue_queue_done(&mytls_vars->tlsoutqueue) != 0) {
		debug_printf(DEBUG_TLS_CORE,
			     "Finished with queue... Freeing.\n");
		if (queue_destroy(&mytls_vars->tlsoutqueue) != 0) {
			debug_printf(DEBUG_NORMAL,
				     "Couldn't destroy queue data!  (We will probably leak memory.)\n");
		}
	}
...
{
	TRACE if (!xsup_assert((ssl != NULL), "ssl != NULL", FALSE))
		return;
...
{
	TRACE if (!xsup_assert((buf != NULL), "buf != NULL", FALSE))
		return XEMALLOC;
			      ...
			      char *root_dir, char *crl_dir)
{
	...
	if ((!root_cert) && (!root_dir)) {
		debug_printf(DEBUG_NORMAL,
			     "Error loading cert!  Path to cert is NULL!\n");
		return XETLSCERTLOAD;
	}

	if (mytls_vars->ctx == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Invalid context in tls_funcs_load_root_certs()!\n");
		return XEMALLOC;
	}
	...
	SSL_CTX_set_info_callback(mytls_vars->ctx,
				  (void (*)(const SSL *, int, int))
				  ssl_info_callback);

	if (SSL_CTX_load_verify_locations(mytls_vars->ctx, root_cert, root_dir)
	    == 0) {
		debug_printf(DEBUG_NORMAL,
			     "Failed to initialize path to root certificate!\n");
		tls_funcs_process_error();
	...
	debug_printf(DEBUG_TLS_CORE,
		     "Loaded root certificate %s and directory %s\n", root_cert,
		     root_dir);

	if (crl_dir) {
		if (SSL_CTX_load_verify_locations
		    (mytls_vars->ctx, NULL, crl_dir) == 0) {
			debug_printf(DEBUG_NORMAL,
				     "Failed to initalize path to CRLs!\n");
			tls_funcs_process_error();
	...
	if (SSL_CTX_set_default_verify_paths(mytls_vars->ctx) == 0) {
		debug_printf(DEBUG_NORMAL,
			     "Failed to initalize default paths for root certificates!\n");
		tls_funcs_process_error();
...
int tls_funcs_load_random(struct tls_vars *mytls_vars, char *random_file)
{
	...
	if (RAND_load_file(file, 1024) < 0) {
		...
		debug_printf(DEBUG_NORMAL,
			     "Couldn't load random data from %s\n", file);
			     ...
			     char *client_cert, char *key_file, char *password)
{
	...
	if (SSL_CTX_use_PrivateKey_file(mytls_vars->ctx, key_file,
		...
		debug_printf(DEBUG_NORMAL,
			     "Couldn't load client private key!\n");
		return XETLSCERTLOAD;
	}
...
ossl_tls_funcs_PRF(uint8_t * secret, int secret_len, uint8_t * label,
		   int label_len, uint8_t * seed, int seed_len,
		   uint8_t * output, int outlen)
{
	...
	TRACE if (!xsup_assert((secret != NULL), "secret != NULL", FALSE))
		return XEMALLOC;
	...
	if (P_seed == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Error with malloc of P_seed in tls_funcs_PRF().\n");
		ipc_events_malloc_failed(NULL);
		return XEMALLOC;
	}
	...
	if (P_MD5_buf == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Error with malloc of P_MD5_buf in tls_funcs_PRF().\n");
		ipc_events_malloc_failed(NULL);
		FREE(P_seed);
		return XEMALLOC;
	}
	...
	if (P_SHA1_buf == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Error with malloc of P_SHA1_buf in tls_funcs_PRF().\n");
		ipc_events_malloc_failed(NULL);
		FREE(P_seed);
		FREE(P_MD5_buf);
		return XEMALLOC;
	...
	hash = EVP_sha1();
	/* Initialize A_SHA1 */
	HMAC(hash, S2, L_S2, P_seed, P_seed_len, A_SHA1,
	     (u_int *) & hashed_len);
	...
	for (i = 0; i < SHA1_iterations; i++) {
		...
		HMAC_Update(&ctx, P_seed, P_seed_len);
		HMAC_Final(&ctx, P_SHA1_buf + i * (SHA_DIGEST_LENGTH),
			   (u_int *) & hashed_len);
		HMAC_cleanup(&ctx);
				...
				uint8_t * sesskey, uint16_t sesskeylen)
{
	...
	TRACE debug_printf(DEBUG_TLS_CORE, "Generating key block!\n");
	...
	if (!SSL_get_session(mytls_vars->ssl)) {
		debug_printf(DEBUG_NORMAL,
			     "No valid SSL session data found!\n");
		return NULL;
	}
		     ...
		     uint16_t fragsize)
{
	...
	if ((newfrag[0] & EAPTLS_LENGTH_INCL) == EAPTLS_LENGTH_INCL) {
		...
		if (mytls_vars->tlsinqueue != NULL) {
			...
			if (queue_create(&mytls_vars->tlsinqueue) != 0) {
				debug_printf(DEBUG_NORMAL,
					     "Couldn't create queue to store incoming fragments!\n");
				return XEGENERROR;
			}
		}
		...
		if (queue_create(&mytls_vars->tlsinqueue) != 0) {
			debug_printf(DEBUG_NORMAL,
				     "Couldn't create queue to store incoming message.\n");
			return XEGENERROR;
		}
	}
	...
	if (queue_get_size(&mytls_vars->tlsinqueue, &value32) != 0) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't determine the queue size!  We won't be able to "
			     "continue!\n");
		return XEGENERROR;
	}

	debug_printf(DEBUG_TLS_CORE,
		     "Total expected data size should be %d.  (0 means we don't know what to expect!) We currently"
	...
	if (queue_enqueue(&mytls_vars->tlsinqueue, p, fragsize) != 0) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't enqueue the data fragment!\n");
		return XEGENERROR;
	}

	if (queue_get_size(&mytls_vars->tlsinqueue, &value32) != 0) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't determine new queue size!\n");
	} else {
...
int tls_funcs_decrypt_ready(struct tls_vars *mytls_vars)
{
	...
	if (queue_get_size(&mytls_vars->tlsinqueue, &value32) != 0) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't determine the queue depth!\n");
		return -1;
	}
		      ...
		      uint16_t insize)
{
	...
	if (encrdata == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory to store encrypted"
			     " data!\n");
		ipc_events_malloc_failed(NULL);
		return XEMALLOC;
	}
	...
	if (rc <= 0) {
		debug_printf(DEBUG_NORMAL,
			     "Error sending data to OpenSSL to be "
			     "encrypted.\n");
	...
	if (mytls_vars->tlsoutqueue == NULL) {
		...
		debug_printf(DEBUG_TLS_CORE,
			     "First packet in a possible chain.  Building queue.\n");
		if (queue_create(&mytls_vars->tlsoutqueue) != 0) {
			debug_printf(DEBUG_NORMAL,
				     "Couldn't create queue for outgoing data!!\n");
			return -1;
		}
	}
	...
	if (rc <= 0) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't read encrypted data from OpenSSL!" "\n");
		tls_funcs_process_error();
		FREE(encrdata);
		return -1;
	}
...
int tls_funcs_get_keyblock_len(struct tls_vars *mytls_vars)
{
	...
	if (mytls_vars->ssl->enc_read_ctx == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "No enc_read_ctx available in %s()!\n",
			     __FUNCTION__);
		return -1;
	}
				  ...
				  void *data, int len)
{
	...
	{
		debug_printf(DEBUG_AUTHTYPES,
			     "No hello extension data to set.  Ignoring.\n");
		return 0;
	}
...
uint8_t *tls_funcs_get_client_random(struct tls_vars * myvars)
{
	...
	if (temp == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory to store a "
				...
				uint16_t length)
{
	...
	if (myvars->derived_shared_secret == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory to store derived "
			     "shared secret!\n");
		ipc_events_malloc_failed(NULL);
		return -1;
	}
	...
	if (SSL_set_session_secret_cb
	    (myvars->ssl, tls_funcs_set_secret_cb, myvars) != 1) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't set the derived secret callback "
			     "function!\n");
		return -1;
	}
