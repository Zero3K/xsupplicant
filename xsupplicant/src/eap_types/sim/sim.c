...
int sim_build_fullauth(char *username, uint8_t *dataoffs, uint16_t *packet_offset, 
...
int sim_at_version_list(char *username, struct eaptypedata *eapdata, 
			uint8_t *dataoffs, uint16_t *packet_offset, uint8_t *out, 
			uint16_t *outptr)
{
...
int sim_v0_final_hash(struct eaptypedata *eapdata, char *sha1resp, uint8_t *out,
		      uint16_t *outptr, char *K_sres)
{
  struct typelengthres *typelenres = NULL;
  char *hash = NULL;
  int i = 0;
		   ...
		   uint8_t *out, uint16_t *outptr, char *K_int)
{
  ...
  char *hash = NULL, sha1resp[20], *at_mac_sres = NULL,  K_sres[16], K_encr[16], K_recv[32];
  char K_send[32];
  char *tusername = NULL;
  ...
  *packet_offset+=16;
  ...
  if (eapdata->workingversion == 0)
      ...
      if (hash == NULL)
	{
      ...
      tlen = strlen(username)+(8*3)+16+eapdata->verlistlen+2;
      ...
      if (hash == NULL)
	{
	  ...
	  else
	  {
		  ...
		  if (tusername == NULL)
		  {
			  debug_printf(DEBUG_NORMAL, "Unable to allocate memory to create a valid IMSI!\n");
			  return XEMALLOC;
		  }
      ...
      memcpy(&hash[strlen(tusername)+8], eapdata->triplet[1].ckey, 8);
      memcpy(&hash[strlen(tusername)+16],
	     eapdata->triplet[2].ckey, 8);
      memcpy(&hash[strlen(tusername)+24],
	     eapdata->nonce_mt, 16);
      memcpy(&hash[strlen(tusername)+24+16],
	     eapdata->verlist, eapdata->verlistlen);
      
      value16 = htons(eapdata->workingversion);
      memcpy(&hash[strlen(tusername)+24+16+eapdata->verlistlen], &value16, 2);
      
	  debug_printf(DEBUG_AUTHTYPES, "Building K_aut from :\n");
	  debug_hex_dump(DEBUG_AUTHTYPES, hash, (strlen(tusername)+24+16+eapdata->verlistlen+2));

      SHA1(hash, (strlen(tusername)+24+16+eapdata->verlistlen+2), sha1resp);
  ...
  if (eapdata->workingversion == 0)
    {
      retval = sim_v0_final_hash(eapdata, sha1resp, out, outptr, &K_sres[0]);
