...
int sim_reader_plugin_hook_available() 
{
		...
		if (cur->plugin_type & PLUGIN_TYPE_SIM_INTERFACE)
			break;
...
void sim_reader_plugin_update_reader_list(char **readername) 
{
		     ...
		     && ((cur->plugin_type & PLUGIN_TYPE_SIM_INTERFACE) ==
			 PLUGIN_TYPE_SIM_INTERFACE))
			 {
			hook =
			    (void *)platform_plugin_entrypoint(cur,
							       "sim_hook_update_reader_list");
 ...
 **/ 
int sim_reader_plugin_hook_get_2g_imsi(SCARDHANDLE * card_hdl, char reader_mode,
				       char *pin, char **imsi) 
{
			 ...
			 PLUGIN_TYPE_SIM_INTERFACE))
			    ...
			    (void *)platform_plugin_entrypoint(cur,
							       "sim_hook_get_2g_imsi");
			if (hook != NULL)
				 {
				result =
				    (*hook) (card_hdl, reader_mode, pin, imsi);
 ...
 * \retval 0 on success, or an SM_ERROR_* (from sm_handler.h) on error.
 **/ 
int sim_reader_plugin_hook_get_3g_imsi(SCARDHANDLE * card_hdl, char reader_mode,
				       char *pin, char **imsi) 
{
					 ...
					 char *cardreader) 
{
			 ...
			 PLUGIN_TYPE_SIM_INTERFACE))
			    ...
			    (void *)platform_plugin_entrypoint(cur,
							       "sim_hook_card_connect");
			if (hook != NULL)
				 {
				result =
				    (*hook) (card_ctx, card_hdl, cardreader);
				      ...
				      unsigned char *c_kc) 
{
	...
	int (*hook) (void **card_hdl, char reader_mode, unsigned char *Rand,
		      unsigned char *autn, unsigned char *c_auts,
		      char *res_len, unsigned char *c_sres, unsigned char *c_ck,
			 ...
			 PLUGIN_TYPE_SIM_INTERFACE))
			    ...
			    (void *)platform_plugin_entrypoint(cur,
				    ...
				    (*hook) ((void **)card_hdl, reader_mode,
					     Rand, autn, c_auts, res_len,
					     c_sres, c_ck, c_ik, c_kc);
					 ...
					 unsigned char *ckey) 
{
	...
	int (*hook) (void *card_hdl, char reader_mode,
			 ...
			 PLUGIN_TYPE_SIM_INTERFACE))
			    ...
			    (void *)platform_plugin_entrypoint(cur,
				    ...
				    (*hook) (card_hdl, reader_mode, challenge,
					     response, ckey);
