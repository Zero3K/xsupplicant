...
int eapaka_get_imsi(context * ctx)
{
	...
	if (!xsup_assert
	    ((ctx->prof->method->method_data != NULL),
	...
	if (ctx->prof->temp_username == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory for SIM identity!\n");
		return XEMALLOC;
	}
	...
	if (Strncpy(&username[1], 49, imsi, 18) != 0) {
		debug_printf(DEBUG_NORMAL,
			     "Attempted to overflow buffer in %s() at %d!\n",
...
int eapaka_setup(eap_type_data * eapdata)
{
	...
	if (eapdata->eap_data == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory for EAP-AKA "
...
void eapaka_check(eap_type_data * eapdata)
{
	...
	if (!xsup_assert
	    ((eapdata->eap_conf_data != NULL), "eapdata->eap_data != NULL",
	     FALSE)) {
			...
			uint16_t size)
{
	...
	while (packet_offset < size) {
		...
		case AT_PERMANENT_ID_REQ:
			...
			retval =
			    aka_do_at_identity(aka, eappayload, &packet_offset);
			if (retval != XENONE)
				return;
			break;
		...
		default:
			debug_printf(DEBUG_NORMAL,
				     "Unknown message in AKA identity request!\n");
			eap_type_common_fail(eapdata);
			return;
		}
	}
			    ...
			    uint16_t size)
{
	...
	case GENERAL_FAILURE_POST_AUTH:
		debug_printf(DEBUG_NORMAL,
			     "[EAP-AKA] General failure after authentication.\n");
		ipc_events_error(ctx, IPC_EVENT_ERROR_SIM_NOTIFICATION,
				 error_str);
		eap_type_common_fail(eapdata);
		break;
	...
	case USER_AUTHENTICATED:
		debug_printf(DEBUG_NORMAL,
			     "[EAP-AKA] Success.  User has been successfully authenticated.\n");
		return;
		break;

	case USER_DENIED:
		debug_printf(DEBUG_NORMAL,
			     "[EAP-AKA] User has been temporarily denied access to the requested service.\n");
		ipc_events_error(ctx, IPC_EVENT_ERROR_SIM_NOTIFICATION,
				 error_str);
		eap_type_common_fail(eapdata);
		break;
	...
	case USER_NO_SUBSCRIPTION:
		debug_printf(DEBUG_NORMAL,
			     "[EAP-AKA] User has not subscribed to the requested service.\n");
		ipc_events_error(ctx, IPC_EVENT_ERROR_SIM_NOTIFICATION,
				 error_str);
		eap_type_common_fail(eapdata);
		break;
	...
	default:
		debug_printf(DEBUG_NORMAL,
			     "Unknown AKA notification value (%d).  Assuming failure.\n",
			     ntohs(tlr->reserved));
		ipc_events_error(ctx, IPC_EVENT_ERROR_SIM_NOTIFICATION,
				 error_str);
		eap_type_common_fail(eapdata);
			 ...
			 uint16_t size)
{
	...
	while (packet_offset < size) {
		...
		case AT_RAND:
			retval =
			    aka_do_at_rand(aka, eappayload, &packet_offset);
			if (retval != XENONE)
				return;
			break;
		...
		case AT_AUTN:
			retval =
			    aka_do_at_autn(aka, eappayload, &packet_offset);
			if (retval != XENONE)
				return;
			break;
		...
		case AT_CHECKCODE:
			debug_printf(DEBUG_AUTHTYPES,
				     "Got an AT_CHECKCODE (Not supported)\n");
			aka_skip_not_implemented(eappayload, &packet_offset);
			break;
		...
		case AT_MAC:
			...
			if (retval == XEAKASYNCFAIL) {
				...
				debug_printf(DEBUG_NORMAL,
					     "MAC check failure in EAP-AKA.\n");
				eap_type_common_fail(eapdata);

				ctx = event_core_get_active_ctx();
				context_disconnect(ctx);
				return;
			} else if (retval != XENONE)
				return;
			break;

		default:
			debug_printf(DEBUG_NORMAL,
				     "Unknown AKA inner type %d!  Skipping.\n",
				     eappayload[packet_offset]);
			aka_skip_not_implemented(eappayload, &packet_offset);
			break;
		}
...
void eapaka_process(eap_type_data * eapdata)
{
	...
	case AKA_AUTHENTICATION_REJECT:
		debug_printf(DEBUG_AUTHTYPES,
			     "Got an AKA_AUTHENTICATION_REJECT!\n");
	...
	case AKA_SYNC_FAILURE:
		...
		debug_printf(DEBUG_AUTHTYPES,
			     "Not implemented!  (And, we should *NEVER*"
			     " get this!\n");
...
uint8_t *eapaka_buildResp(eap_type_data * eapdata)
{
	...
	if (akadata->chal_type == AKA_IDENTITY) {
		framecpy =
		    aka_resp_identity(akadata,
				      eap_type_common_get_eap_reqid(eapdata->
								    eapReqData),
				      eapdata->ident);
		eapdata->ignore = FALSE;
		...
		if (akadata->sync_fail == TRUE) {
			...
			return aka_do_sync_fail(akadata,
						eap_type_common_get_eap_reqid
						(eapdata->eapReqData));
		}
		...
		// Zero out the mac.
		memset((uint8_t *) &
		       framecpy[offset + sizeof(struct eap_header)], 0x00, 16);
...
uint8_t eapaka_isKeyAvailable(eap_type_data * eapdata)
{
	...
	if (akadata->keyingMaterial != NULL)
		return TRUE;
...
char *eapaka_get_username(void *config)
{
	...
	if (ctx == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Unable to get the IMSI for EAP-AKA!\n");
		return NULL;
	}
