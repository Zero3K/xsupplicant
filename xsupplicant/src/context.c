...
int context_init(context *work, char *intname)
{
  ...
  if (Strncpy(work->intName, (strlen(intname)+1), intname, strlen(intname) + 1) != 0)
  {
	  debug_printf(DEBUG_NORMAL, "Couldn't make a copy of the interface name in %s() at "
			"%d!\n", __FUNCTION__, __LINE__);
	  return XEGENERROR;
  }
...
void context_destroy(context *ctx)
{
  ...
  {
	  context_destroy_wireless_ctx((wireless_ctx **)&ctx->intTypeData);
  }
...
char config_build(context *ctx, char *network_name)
{
  ...
  {
	  debug_printf(DEBUG_CONFIG_PARSE, "Network name passed in is NULL!\n");
  ...
  debug_printf(DEBUG_CONFIG_PARSE, "Working from config file %s.\n",
      ...
      if (result != NULL) 
	{
      ...
      debug_printf(DEBUG_NORMAL, "No configuration information is in memory!  "
...
context *context_allocate(context **ctx)
{
	...
	if ((*ctx) == NULL)
	{
		debug_printf(DEBUG_NORMAL, "Couldn't allocate memory to store context information!\n");
		return NULL;
	}
 ...
 * \retval XENONE on success
 **/
int context_init_interface_hdwr(context **ctx, char *desc, char *device, char driveridx, FDEPTH flags)
{
	context *cur = NULL;
  ...
  if (wctx->enc_capa == 0)
    {
      debug_printf(DEBUG_CONTEXT, "Interface has no encryption capabilities, or "
		   "unknown abilitites.\n");
    } else {
      debug_printf(DEBUG_INT, "Card reported capabilitites :");
      if (wctx->enc_capa & DOES_WEP40) debug_printf_nl(DEBUG_INT, 
							   " WEP40");
      if (wctx->enc_capa & DOES_WEP104) debug_printf_nl(DEBUG_INT,
							    " WEP104");
      if (wctx->enc_capa & DOES_WPA) debug_printf_nl(DEBUG_INT, " WPA");
      if (wctx->enc_capa & DOES_WPA2) debug_printf_nl(DEBUG_INT, 
							  " WPA2");
      if (wctx->enc_capa & DOES_TKIP) debug_printf_nl(DEBUG_INT, 
							  " TKIP");
      if (wctx->enc_capa & DOES_CCMP) debug_printf_nl(DEBUG_INT, 
							  " CCMP");
 ...
 * \retval XEMALLOC on memory allocation errors
 **/
int context_init_interface(context **ctx, char *desc, char *device, char *drivernam, FDEPTH flags)
{
  context *intcur;
  ...
  else
  {
	  ...
	  {
		  debug_printf(DEBUG_INT, "No default connection defined for interface '%s'.\n", desc);
	  }
  }
...
void context_init_ints_from_conf(context **ctx)
{
    ...
    else
    {
      ...
      debug_printf(DEBUG_INT, "Couldn't init interface '%s'!\n", ints->description);
    }
...
int context_create_wireless_ctx(wireless_ctx **new_wctx, uint8_t drivernum)
{
	...
	if ((*new_wctx) == NULL)
	{
		debug_printf(DEBUG_NORMAL, "Couldn't allocate memory to store wireless "
				"context data!\n");
		return XEMALLOC;
	}
