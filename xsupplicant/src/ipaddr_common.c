...
int ipaddr_common_ip_is_valid(char *ipaddr)
{
	...
	if (strcmp(ipaddr, "0.0.0.0") == 0) return FALSE;
	if (strcmp(ipaddr, "255.255.255.255") == 0) return FALSE;
...
int ipaddr_common_is_gw_in_subnet(char *addr, char *netmask, char *gateway)
{
	...
	if ((addr == NULL) || (netmask == NULL) || (gateway == NULL)) return FALSE;
...
int ipaddr_common_is_broadcast(char *addr, char *netmask)
{
	...
	if ((addr == NULL) || (netmask == NULL)) return FALSE;
