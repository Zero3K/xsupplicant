...
struct found_ssids *config_ssid_find_by_wctx(wireless_ctx * wctx)
{
	...
	if (wctx->cur_essid == NULL)
		return NULL;	// If we don't have an SSID, then we won't find the SSID. :)
...
void config_ssid_add_wpa_ie(wireless_ctx * wctx, uint8_t * wpa_ie, uint8_t len)
{
	...
	if (!working->wpa_ie) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory to store the WPA "
...
void config_ssid_add_rsn_ie(wireless_ctx * wctx, uint8_t * rsn_ie, uint8_t len)
{
	...
	if (!working->rsn_ie) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory to store the RSN "
			     "IE!  Xsupplicant will not be able to use this ESSID! "
			     "(%s:%d)\n", __FUNCTION__, __LINE__);
 ...
 **/
void config_ssid_add_qual(wireless_ctx * wctx, unsigned char qual, char signal,
			  char noise, uint8_t percentage)
{
	struct found_ssids *working = NULL;
...
void config_ssid_dump(wireless_ctx * wctx)
{
	...
	while (cur != NULL) {
		if (cur == NULL) {
			debug_printf(DEBUG_PHYSICAL_STATE,
				     "NO VALID SSID INFORMATION AVAILABLE "
				     "TO DUMP!!\n");
			return;
		}
		...
		if (cur->ssid_name != NULL) {
			...
			if ((cur->wpa_ie != NULL)
				...
				&& (cur->wpa_ie_len < 255))) {
				debug_printf(DEBUG_PHYSICAL_STATE,
					     "WPA IE (%d) : ", cur->wpa_ie_len);
				debug_hex_printf(DEBUG_PHYSICAL_STATE,
						 cur->wpa_ie, cur->wpa_ie_len);
				debug_printf(DEBUG_PHYSICAL_STATE, "\n");
			}
			...
			if ((cur->rsn_ie != NULL)
				...
				&& (cur->rsn_ie_len < 255))) {
				debug_printf(DEBUG_PHYSICAL_STATE,
					     "RSN IE (%d) : ", cur->rsn_ie_len);
				debug_hex_printf(DEBUG_PHYSICAL_STATE,
						 cur->rsn_ie, cur->rsn_ie_len);
				debug_printf(DEBUG_PHYSICAL_STATE, "\n");
			}
		}
		cur = cur->next;
...
void config_ssid_clear(wireless_ctx * wctx)
{
	...
	liblist_delete_list((genlist **) & wctx->ssid_cache,
			    config_ssid_free_node);
 ...
 *         \c ssid_name.
 **/
struct found_ssids *config_ssid_find_by_name(wireless_ctx * wctx,
					     char *ssid_name)
{
	struct found_ssids *cur = NULL;
...
struct found_ssids *config_ssid_find_best_ssid(context * ctx)
{
	...
	if (ctx->intType != ETH_802_11_INT) {
		debug_printf(DEBUG_NORMAL,
			     "Attempted to find the 'best' SSID using an interface that isn't wireless!\n");
		return NULL;
	}
	...
	if (best) {
		...
		if (conf != NULL) {
			...
			if (retval > 0) {
				...
				ipc_events_ui(ctx, IPC_EVENT_UI_NEED_UPW,
					      conf->name);
...
void config_ssid_get_by_mac(context * ctx, uint8_t * forced_mac)
{
	...
	if (wctx == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Invalid wireless context in %s()!  Cannot use forced MAC address!\n",
			     __FUNCTION__);
		return;
	}
	...
	if (cur == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't find an SSID that matched the "
...
char *config_ssid_get_desired_ssid(context * ctx)
{
	...
	if (TEST_FLAG(ctx->flags, FORCED_CONN)) {
		if (ctx->conn == NULL) {
			debug_printf(DEBUG_NORMAL,
				     "The flag to set a forced connection is set, "
	...
	if (TEST_FLAG(globals->flags, CONFIG_GLOBALS_ASSOC_AUTO)) {
		...
		if ((cardif_GetSSID(ctx, cur_ssid, 33) == XENONE)
			...
			if (ssid != NULL) {
				...
				debug_printf(DEBUG_NORMAL,
					     "You are in manual association mode"
...
int config_ssid_using_wep(wireless_ctx * wctx)
{
	...
	if ((abil & ABIL_WPA_IE) || (abil & ABIL_RSN_IE))
		return FALSE;
 ...
 **/
void config_ssid_get_wpa_ie(wireless_ctx * wctx, uint8_t ** wpa_ie,
			    uint8_t * wpa_ie_len)
{
	struct found_ssids *ssid = NULL;
 ...
 **/
void config_ssid_get_rsn_ie(wireless_ctx * wctx, uint8_t ** rsn_ie,
			    uint8_t * rsn_ie_len)
{
	struct found_ssids *ssid = NULL;
...
uint8_t *config_ssid_get_mac_with_freq(wireless_ctx * wctx, double freq)
{
	...
	if (cur == NULL) {
		debug_printf(DEBUG_PHYSICAL_STATE,
			     "Unable to find the BSSID with SSID %s and Freq %lf!\n",
			     wctx->cur_essid, freq);
		return NULL;
	}
...
double config_ssid_get_best_freq(wireless_ctx * wctx)
{
	...
	if (best == NULL) {
		debug_printf(DEBUG_PHYSICAL_STATE,
			     "Unable to find the best frequency with SSID %s!\n",
			     wctx->cur_essid);
		return 0;
	}

	debug_printf(DEBUG_PHYSICAL_STATE,
		     "Found best frequency with SSID %s as %lf!\n",
