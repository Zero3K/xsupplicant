  ...
  else
    {
		if (ctx->intType == ETH_802_11_INT)
		{
	      if (wctx->cur_essid != NULL)
			{
			  debug_printf(DEBUG_NORMAL, "There is no configuration defined for "
			       "network '%s'.\n", wctx->cur_essid);
			}
		    else
			{
			  debug_printf(DEBUG_NORMAL, "This network does not have a configuration"
			       " defined for it!\n");
			}
...
void txSuppRsp(context *ctx)
{
  ...
  if (ctx->sendframe == NULL)
    {
      debug_printf(DEBUG_NORMAL, "Our 'to send' frame buffer is NULL!\n");
      return;
    }
  ...
  if (eapdata == NULL)
  {
	  debug_printf(DEBUG_NORMAL, "The upper layers didn't generate anything to send on '%s'.\n", ctx->desc);
	  return;
  }
  ...
  if (eapsize > 1520)
    {
      ...
      debug_printf(DEBUG_NORMAL, "The EAP message size appears to be invalid. "
		   "Ignoring!\n");
      return;
    }
  ...
  if (ctx->eap_state->eapRespData == NULL)
    {
      debug_printf(DEBUG_NORMAL, "Got a NULL response from the EAP layer! "
		   "Ignoring!\n");
      return;
    }
  ...
  if (frame_ptr == NULL)
    {
      debug_printf(DEBUG_NORMAL, "Our destination frame pointer is NULL!\n");
      return;
    }
...
int backend_sm_timeout_display(context *ctx)
{
  ...
  switch (ctx->statemachine->lastEapType)
      ...
      if (!TEST_FLAG(globals->flags, CONFIG_GLOBALS_NO_FRIENDLY_WARNINGS))
	{
	  ...
	  ipc_events_error(ctx, IPC_EVENT_ERROR_TIMEOUT_WAITING_FOR_ID, NULL);
...
void backend_sm_check_globals(context *ctx)
{
  ...
  if (ctx->statemachine->eapRestart == TRUE)
    {
      debug_printf(DEBUG_1X_BE_STATE, "Supplicant PAE has issued a restart.\n");
...
void backend_sm_change_to_initialize(context *ctx)
{
  ...
  if (ipc_events_statemachine_transition(ctx, IPC_STATEMACHINE_8021X_BACKEND, ctx->statemachine->beCurState, INITIALIZE) != IPC_SUCCESS)
  {
	  // Display, or log an error message, and move on.
	  debug_printf(DEBUG_NORMAL, "Unable to send IPC 802.1X backend state change message!\n");
  }
#endif
...
#if DOT1X_BACKEND_SM_EVENTS == 1
  // Let all connected listeners know that we transitioned state.
  if (ipc_events_statemachine_transition(ctx, IPC_STATEMACHINE_8021X_BACKEND, ctx->statemachine->beCurState, IDLE) != IPC_SUCCESS)
  {
	  // Display, or log an error message, and move on.
	  debug_printf(DEBUG_NORMAL, "Unable to send IPC 802.1X backend state change message!\n");
  }
...
#if DOT1X_BACKEND_SM_EVENTS == 1
  // Let all connected listeners know that we transitioned state.
  if (ipc_events_statemachine_transition(ctx, IPC_STATEMACHINE_8021X_BACKEND, ctx->statemachine->beCurState, REQUEST) != IPC_SUCCESS)
  {
	  // Display, or log an error message, and move on.
	  debug_printf(DEBUG_NORMAL, "Unable to send IPC 802.1X backend state change message!\n");
  }
...
#if DOT1X_BACKEND_SM_EVENTS == 1
  // Let all connected listeners know that we transitioned state.
  if (ipc_events_statemachine_transition(ctx, IPC_STATEMACHINE_8021X_BACKEND, ctx->statemachine->beCurState, RESPONSE) != IPC_SUCCESS)
  {
	  // Display, or log an error message, and move on.
	  debug_printf(DEBUG_NORMAL, "Unable to send IPC 802.1X backend state change message!\n");
  }
...
#if DOT1X_BACKEND_SM_EVENTS == 1
  // Let all connected listeners know that we transitioned state.
  if (ipc_events_statemachine_transition(ctx, IPC_STATEMACHINE_8021X_BACKEND, ctx->statemachine->beCurState, RECEIVE) != IPC_SUCCESS)
  {
	  // Display, or log an error message, and move on.
	  debug_printf(DEBUG_NORMAL, "Unable to send IPC 802.1X backend state change message!\n");
  }
...
#if DOT1X_BACKEND_SM_EVENTS == 1
  // Let all connected listeners know that we transitioned state.
  if (ipc_events_statemachine_transition(ctx, IPC_STATEMACHINE_8021X_BACKEND, ctx->statemachine->beCurState, FAIL) != IPC_SUCCESS)
  {
	  // Display, or log an error message, and move on.
	  debug_printf(DEBUG_NORMAL, "Unable to send IPC 802.1X backend state change message!\n");
  }
...
#if DOT1X_BACKEND_SM_EVENTS == 1
  // Let all connected listeners know that we transitioned state.
  if (ipc_events_statemachine_transition(ctx, IPC_STATEMACHINE_8021X_BACKEND, ctx->statemachine->beCurState, TIMEOUT) != IPC_SUCCESS)
  {
	  // Display, or log an error message, and move on.
	  debug_printf(DEBUG_NORMAL, "Unable to send IPC 802.1X backend state change message!\n");
  }
...
#if DOT1X_BACKEND_SM_EVENTS == 1
  // Let all connected listeners know that we transitioned state.
  if (ipc_events_statemachine_transition(ctx, IPC_STATEMACHINE_8021X_BACKEND, ctx->statemachine->beCurState, SUCCESS) != IPC_SUCCESS)
  {
	  // Display, or log an error message, and move on.
	  debug_printf(DEBUG_NORMAL, "Unable to send IPC 802.1X backend state change message!\n");
  }
...
int backend_sm_run(context *ctx)
{
  ...
  */
  ...
  switch (ctx->statemachine->beCurState)
    ...
    default:
      debug_printf(DEBUG_NORMAL, "Backend State Machine is in an UNKNOWN "
		   "state!\n");
      break;
    }
