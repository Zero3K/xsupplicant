...
struct rfc4137_eap_handler eaphandlers[] = {
	{EAP_TYPE_MD5, "EAP-MD5", eapmd5_check, eapmd5_process,
	 eapmd5_buildResp,
	 eapmd5_isKeyAvailable, eapmd5_getKey, eap_type_common_get_zero_len,
	 eap_type_common_upw_required, eap_type_common_get_username,
	 eapmd5_deinit},

	{EAP_TYPE_MSCHAPV2, "EAP-MSCHAPv2", eapmschapv2_check,
	 eapmschapv2_process,
	...
	{EAP_TYPE_TLS, "EAP-TLS", eaptls_check, eaptls_process,
	 eaptls_buildResp,
	 eaptls_isKeyAvailable, eaptls_getKey,
	 eap_type_common_get_common_key_len,
	...
	{EAP_TYPE_OTP, "EAP-OTP", eapotp_check, eapotp_process,
	 eapotp_buildResp,
	 eapotp_isKeyAvailable, eapotp_getKey, eap_type_common_get_zero_len,
	 eap_type_common_upw_required, eap_type_common_get_username,
	 eapotp_deinit},

	{EAP_TYPE_GTC, "EAP-GTC", eapotp_check, eapotp_process,
	 eapotp_buildResp,
	 eapotp_isKeyAvailable, eapotp_getKey, eap_type_common_get_zero_len,
...
#ifdef HAVE_TNC
	{EAP_TYPE_TNC, "EAP-TNC", eaptnc_check, eaptnc_process,
	 eaptnc_buildResp,
...
#ifdef EAP_SIM_ENABLE
	{EAP_TYPE_AKA, "EAP-AKA", eapaka_check, eapaka_process,
	 eapaka_buildResp,
	 eapaka_isKeyAvailable, eapaka_getKey,
	 eap_type_common_get_common_key_len,
	 eapaka_creds_required, eapaka_get_username, eapaka_deinit},

	{EAP_TYPE_SIM, "EAP-SIM", eapsim_check, eapsim_process,
	 eapsim_buildResp,
	 eapsim_isKeyAvailable, eapsim_getKey,
	 eap_type_common_get_common_key_len,
	...
	{EAP_TYPE_LEAP, "EAP-LEAP", eapleap_check, eapleap_process,
	 ...
	 eapleap_getKey_len, eap_type_common_upw_required, NULL,
	 eapleap_deinit},
#endif
	...
	{EAP_TYPE_FAST, "EAP-FAST", eapfast_check, eapfast_process,
	 ...
	 eap_type_common_get_common_key_len, eapfast_creds_required,
	 eapfast_get_username,
	 eapfast_deinit},
#endif
...
int eap_sm_init(eap_sm ** sm)
{
	...
	if ((*sm) == NULL) {
		...
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory for EAP state "
			     "machine data.\n");
		return XEMALLOC;
	}
	...
	if ((*sm)->active == NULL) {
		ipc_events_malloc_failed(NULL);
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory for active EAP "
			     "method data!\n");
		return XEMALLOC;
	}
...
void eap_sm_disp_state_change(int debug_level, eap_sm * sm, int newstate)
{
	...
	debug_printf(debug_level, "(EAP State Machine -- Phase %d) ",
		     sm->phase);
...
void eap_sm_change_to_disabled(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_DISABLED) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
#endif
...
void eap_sm_change_to_idle(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_IDLE) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
...
void eap_sm_change_to_initialize(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_INITIALIZE) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
	...
	if (sm->eapReqData == NULL)
		sm->eapReq = FALSE;	// Should fix the weird occasional state machine jam.
...
void parseEapReq(eap_sm * sm)
{
	...
	if (eapdata == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Attempt to parse EAP data that was NULL!\n");
		return;
	}
...
void eap_sm_dump_state(eap_sm * sm)
{
	...
	debug_printf(DEBUG_EAP_STATE,
		     "reqMethod = %d (%s) selectedMethod = %d (%s)\n",
		     sm->reqMethod, reqMethod, sm->selectedMethod,
		     selectedMethod);

	debug_printf(DEBUG_EAP_STATE,
		     "methodState = %d (%s) allowNotifications = %s  decision = %s\n",
		     sm->methodState, methodState, allowNotifications,
		     decision);
...
void eap_sm_change_to_received(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_RECEIVED) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
	...
	if ((sm->reqId == sm->lastId) && (sm->phase == 2)
	    && (sm->reqMethod == 26)) {
		debug_printf(DEBUG_EAP_STATE,
			     "Work around for non-incrementing EAP ID with EAP-MS-CHAPv2 in phase 2!\n");
		sm->lastId--;
	}
		...
		if (sm->reqId != sm->lastId) {
			if ((!((sm->lastId == 0xff) && (sm->reqId == 0x00))) ||
				...
				if (TEST_FLAG
					...
					debug_printf(DEBUG_NORMAL,
						     "We received ID %d, when we should "
						     "have received ID %d.\n",
						     sm->reqId, sm->lastId);
					debug_printf(DEBUG_NORMAL,
						     "Please ask your vendor to fix "
						     "this!\n");
				}
			}
		}
	...
	if (((sm->selectedMethod != EAP_TYPE_TNC)
	     && (sm->selectedMethod != NONE))
...
uint8_t allowMethod(eap_sm * sm)
{
	...
	if (eaphandlers[i].eap_type_handler == NO_EAP_AUTH) {
		debug_printf(DEBUG_NORMAL,
			     "Unsupported EAP type request.  Type is "
			     "%d.  (%02X Hex)\n", reqMethod, reqMethod);
		return FALSE;
	}

	debug_printf(DEBUG_EAP_STATE,
		     "EAP type %d is supported, checking if it is "
		     "allowed.\n", reqMethod);
	...
	if (sm->curMethods == NULL) {
		debug_printf(DEBUG_EAP_STATE,
			     "There doesn't appear to be any valid "
...
uint8_t *buildNak(eap_sm * sm)
{
	...
	if (sm->curMethods == NULL) {
		debug_printf(DEBUG_EAP_STATE,
			     "No EAP methods are configured for this "
	...
	if (outpacket == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory to create NAK "
			     "packet!\n");
		return NULL;
	}
...
void eap_sm_change_to_get_method(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_GET_METHOD) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
	...
	if (allowMethod(sm) == TRUE) {
		...
		if (sm->eapRespData == NULL) {
			...
			if (sm->curMethods != NULL) {
				debug_printf(DEBUG_NORMAL,
					     "Packet discarded because a valid "
...
void eap_sm_change_to_method(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_METHOD) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
	...
	if (eapstruct < 0) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't locate the EAP method that the server requested! (Phase %d)\n",
			     sm->phase);
	...
	if (eaphandlers[eapstruct].eap_check == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "No EAP check method available!  eapstruct index is %d!\n",
			     eapstruct);
		return;
	} else {
	...
	if (sm->ignore == FALSE) {
		keyavail =
		    eaphandlers[eapstruct].eap_isKeyAvailable(sm->active);
		...
		if (!sm->ignore) {
			...
			sm->eapRespData =
			    eaphandlers[eapstruct].eap_buildResp(sm->active);
	...
	if (sm->ignore) {
		if ((sm->selectedMethod != EAP_TYPE_LEAP)
		    && (sm->decision != UNCOND_SUCC)) {
			debug_printf(DEBUG_NORMAL,
				     "Packet discarded on request of higher "
				     "layer.\n");
		}
		eap_sm_change_state(sm, EAP_DISCARD);
		return;
	...
	if ((sm->methodState == DONE) && (sm->decision == EAP_FAIL)) {
		eap_sm_change_state(sm, EAP_FAILURE);
		return;
	}
...
void eap_sm_change_to_send_response(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_SEND_RESPONSE) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
	...
	if (sm->eapRespData == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "EAP type didn't return a valid packet to "
			     "send.  Ignoring.\n");
		eap_sm_change_state(sm, EAP_DISCARD);
		return;
	}
	...
	if (sm->lastRespData == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory to store a copy "
			     "of the EAP packet!\n");
	} else {
		debug_printf(DEBUG_EAP_STATE,
			     "Allocated %d byte(s) for retransmit if needed"
			     ".\n", eapsize);
...
void eap_sm_change_to_discard(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_DISCARD) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
...
void eap_sm_process_hints(eap_sm * sm)
{
	...
	if (eapsize > (OFFSET_TO_EAP + sizeof(struct eap_header) + 1)) {
		...
		if (!globals) {
			debug_printf(DEBUG_NORMAL,
				     "Couldn't get global configuration "
...
uint8_t *buildIdentity(uint8_t reqId, char *ident)
{
	...
	if (newframe == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't allocate memory to create EAP "
			     "packet!\n");
		return NULL;
	}
	...
	if (Strncpy((char *)username_ofs,
		    ((sizeof(struct eap_header) + strlen(ident) + 1) -
		     sizeof(struct eap_header)), ident,
		    strlen(ident) + 1) != 0) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't copy the user's identity in %s() at %d!\n",
...
void eap_sm_change_to_identity(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_IDENTITY) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
	...
	if (ctx == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Somehow we got to IDENTITY state in the EAP state machine, but we don't have a context!?\n");
		return;
	}

	debug_printf(DEBUG_NORMAL,
		     "Interface : %s --- Starting Authentication (Phase %d) ---\n",
		     ctx->desc, sm->phase);
...
void processNotify(uint8_t * eapReqData)
{
	...
	if (Strncpy
	    (notify_str, (length + 1),
	     (const char *)&eapReqData[sizeof(struct eap_header)],
	     length) != 0) {
		debug_printf(DEBUG_NORMAL,
			     "Attempt to overflow the buffer in %s() at %d!\n",
			     __FUNCTION__, __LINE__);
		return;
	}
...
void eap_sm_change_to_notification(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_NOTIFICATION) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
...
void eap_sm_change_to_retransmit(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_RETRANSMIT) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
...
void eap_sm_change_to_success(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_SUCCESS) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
	...
	debug_printf(DEBUG_NORMAL,
		     "Interface : %s --- Authenticated (Phase %d) ---\n",
		     ctx->desc, sm->phase);
#if 0
	if (ipc_events_authenticated(ctx, sm->phase) != IPC_SUCCESS) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't send 'starting authentication' log to UI!\n");
	}
#endif
}
...
void eap_sm_change_to_failure(eap_sm * sm)
{
	...
	if (ipc_events_statemachine_transition
	    (event_core_get_active_ctx(), IPC_STATEMACHINE_EAP,
	     sm->eap_sm_state, EAP_FAILURE) != IPC_SUCCESS) {
		// Display, or log an error message, and move on.
		debug_printf(DEBUG_NORMAL,
			     "Unable to send IPC EAP state change message!\n");
	}
	...
	debug_printf(DEBUG_NORMAL,
		     "Interface : %s --- Authentication Failed (Phase %d) ---\n",
		     ctx->desc, sm->phase);

#if 0
	if (ipc_events_authentication_failed(ctx, sm->phase) != IPC_SUCCESS) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't send 'authentication failed' log to UI!\n");
	}
#endif
	...
	if (sm->credsSent == TRUE) {
		...
		if (ctx->prof != NULL) {
			debug_printf(DEBUG_NORMAL,
				     "Authentication failed on '%s' after credentials were sent.  Flushing in memory credentials and requesting new ones.\n",
				     ctx->desc);
...
void eap_sm_change_state(eap_sm * sm, int newstate)
{
	...
	default:
		debug_printf(DEBUG_NORMAL,
			     "Request to change to state %d, which is "
			     "invalid!\n", newstate);
	}
}
