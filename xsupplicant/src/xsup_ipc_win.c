...
int xsup_ipc_win_create_pipe(int i)
{
	...
	if (pipes[i].hevent == NULL) {
		...
		debug_printf(DEBUG_NORMAL,
			     "Couldn't create event handler for IPC! (Error %d : %s)\n",
			     err, lpMsgBuf);
		LocalFree(lpMsgBuf);
		return -1;
	}
	...
	if (pSA != NULL) {
		if (ConvertStringSecurityDescriptorToSecurityDescriptor(szSD,
		    ...
		    0) {
			debug_printf(DEBUG_NORMAL,
				     "Unable to set rights on the named pipe!  Only administrative users will be able to control the supplicant.\n");
			FREE(pSA);
		}
	}
	...
	if (pipes[i].hdl == INVALID_HANDLE_VALUE) {
		...
		lpMsgBuf = GetLastErrorStr(err);
		debug_printf(DEBUG_NORMAL,
			     "Couldn't create named pipes for IPC control!  (Error %d : %s)\n",
			     err, lpMsgBuf);
		LocalFree(lpMsgBuf);
		return -1;
	}
	...
	event_core_register(pipes[i].hdl, NULL, xsup_ipc_win_event, 0,
			    HIGH_PRIORITY, handle_str);
	...
	if (ConnectNamedPipe(pipes[i].hdl, ovr) != 0) {
		...
		lpMsgBuf = GetLastErrorStr(err);
		debug_printf(DEBUG_NORMAL,
			     "Couldn't establish listener for named pipe! (Error %d : %s)\n",
			     err, lpMsgBuf);
		LocalFree(lpMsgBuf);
		return -1;
	}
...
int xsup_ipc_win_event(context * ctx, HANDLE hdl)
{
	...
	if (i == INSTANCES) {
		debug_printf(DEBUG_NORMAL,
			     "Event was triggered on an handle we know nothing about?  (hdl = %d)\n",
			     hdl);
	...
	if (!TEST_FLAG(pipes[i].flags, IPC_CONNECTED)) {
		...
		if (lovr == NULL) {
			debug_printf(DEBUG_NORMAL,
				     "Couldn't locate bound OVERLAPPED structure!\n");
			return -1;
		}
		...
		if (HasOverlappedIoCompleted(lovr) == TRUE) {
			...
			if (ReadFile
				...
				if (resulterr != ERROR_IO_PENDING) {
					debug_printf(DEBUG_NORMAL,
						     "Error setting PIPE read handler!\n");
					debug_printf(DEBUG_NORMAL,
						     "Error was : %d\n",
						     resulterr);
				}
		...
		if (GetOverlappedResult(pipes[i].hdl, lovr, &bxfer, FALSE) == 0) {
			...
			debug_printf(DEBUG_IPC,
				     "Got data on pipe %d.  (hevent = %d   hdl = %d)\n",
				     i, pipes[i].hevent, pipes[i].hdl);

#ifdef UNSAFE_DUMPS
			debug_hex_dump(DEBUG_IPC, pipes[i].buffer,
				       pipes[i].inbuf);
#endif
			...
			case IPC_CHANGE_TO_SYNC_ONLY:
				...
				debug_printf(DEBUG_IPC,
					     "Changed IPC pipe %d to be request/response only.\n",
					     pipes[i].hdl);
				break;
			...
			if ((result != NULL) && (ressize > 0)) {
				xsup_ipc_send_message(pipes[i].hdl, result,
						      ressize);
...
void xsup_ipc_send_message(HANDLE pipehdl, char *tosend, int tolen)
{
	...
	if ((tolen + sizeof(ipc_header)) < BUFSIZE) {
		...
		if (frag == NULL) {
			debug_printf(DEBUG_NORMAL,
				     "Couldn't allocate memory to store an IPC fragment "
				     "to be sent!\n");
			return;
		}
		...
		if (frag == NULL) {
			debug_printf(DEBUG_NORMAL,
				     "Couldn't allocate the buffer needed to send IPC fragments!\n");
			return;
		}
		...
		while (offset < tolen) {
			...
			if (WriteFile
			    (pipehdl, frag, (frag_size + sizeof(ipc_header)),
			     &totalbytes, NULL) == 0) {
				debug_printf(DEBUG_NORMAL,
					     "Couldn't send response document to IPC client!\n");
				FREE(frag);
			...
			if (totalbytes != (frag_size + sizeof(ipc_header))) {
				debug_printf(DEBUG_NORMAL,
					     "Runt packet sent!  IPC data will be corrupt.\n");
				debug_printf(DEBUG_NORMAL,
					     "Total Bytes Sent = %d    Fragment Size = %d\n",
					     totalbytes,
					     (frag_size + sizeof(ipc_header)));
				FREE(frag);
				return;
...
void xsup_ipc_send_eap_notify(char *notify)
{
	...
	if (buffer == NULL) {
		debug_printf(DEBUG_NORMAL,
			     "Couldn't send IPC EAP notify message!\n");
		return;
	}
