...
int lsa_calls_decrypt_secret(uint8_t **outData, uint16_t *outLen)
{
	...
	if (LsaOpenPolicy(NULL, (PLSA_OBJECT_ATTRIBUTES)&objAttr, POLICY_GET_PRIVATE_INFORMATION, &lsarHandle) != STATUS_SUCCESS)
	{
		debug_printf(DEBUG_NORMAL, "Unable to obtain a handle to the policy object needed to read the machine secret!\n");
		return -1;
	}
	...
	if (LsaRetrievePrivateData(lsarHandle, &secretString, &resultData) != STATUS_SUCCESS)
	{
		debug_printf(DEBUG_NORMAL, "Unable to read machine secret!  (Is this machine a domain member?)\n");
		return -1;
	}
