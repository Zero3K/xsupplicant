...
int win_ip_manip_init_iphlpapi() 
{
	    ...
	    (IpHlpSetStatic) GetProcAddress(hIPHlpApiMod,
					    "SetAdapterIpAddress");
...
int NotifyDNSChange(LPCTSTR lpszAdapterName) 
{
	      ...
	      NULL)
		if ((pDhcpNotifyProc) (NULL, wcAdapterName, FALSE, 0, 0, 0, 0)
		     == ERROR_SUCCESS)
...
int FlushDNS() 
{
	       ...
	       (DNSFLUSHPROC) GetProcAddress(hDnsDll,
...
int win_ip_manip_check_dhcp_thread_allowed(struct win_sock_data *sockData) 
{
	    ...
	    if (win_ip_manip_lock_mutex(&sockData->mutexDhcpOutstanding, 1) !=
		0)
		...
		if (win_ip_manip_unlock_mutex
		     (&sockData->mutexDhcpOutstanding, 1) != 0)
	    ...
	    sockData->dhcpOutstanding++;
	if (win_ip_manip_unlock_mutex(&sockData->mutexDhcpOutstanding, 1) !=
	      0)
...
void win_ip_manip_renew_ip_thread(void *ctxptr) 
{
			      ...
			      "Unable to obtain DHCP lock.  DHCP will fail.\n");
			...
			win_ip_manip_unlock_mutex(&sockData->mutexDhcpRunning,
						   2);
					...
					if (win_ip_manip_do_release_renew_ip
					     (ctx) != TRUE)
...
int win_ip_manip_do_release_renew_ip(context * ctx) 
{
	    ...
	    if ((dwRetVal = IpRenewAddress(&addrMap)) != NO_ERROR)
		  ...
		  || (ctx->conn->ip.dns3 != NULL)))
		    ...
		    win_ip_manip_set_dns_servers(ctx, ctx->conn->ip.dns1,
						 ctx->conn->ip.dns2,
...
void win_ip_manip_set_static_ip_thread(void *dataPtr) 
{
	...
	if (ipaddr_common_is_gw_in_subnet
	      (addrData->addr, addrData->netmask, addrData->gateway) == FALSE)
	...
	if (ipaddr_common_is_broadcast(addrData->addr, addrData->netmask) ==
	      TRUE)
...
int cardif_windows_events_enable_dhcp(context * ctx) 
{
		...
		return -1;
	      ...
	      (guid, 1, inet_addr("1.1.1.1"), inet_addr("255.255.255.0"),
