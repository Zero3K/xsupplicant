...
void tthandler_troubleticket_thread(void *data)
{
	...
	failed_plugins = plugin_hook_trouble_ticket_dump_file(tempdata->tempdir);
	if (failed_plugins < 0)
	{
		// ACK!  NASTY error!
...
void tthandler_create_troubleticket(char *tempdir, char *tt_path)
{
	...
	if (!xsup_assert((tempdir != NULL), "tempdir != NULL", FALSE)) return;
	if (!xsup_assert((tt_path != NULL), "tt_path != NULL", FALSE)) return;

	ttdata = Malloc(sizeof(ttdata_type));
	if (ttdata == NULL)
	{
		debug_printf(DEBUG_NORMAL, "Unable to allocate memory to store the trouble ticket request structure.\n");
		return;
	}
