...
int win_impersonate_desktop_user() 
{
		...
		return IMPERSONATE_HANDLE_IN_USE;
	      ...
	      (WTSGetActiveConsoleSessionId(), &desktopUserHandle) == FALSE)
	      ...
	      (desktopUserHandle, TOKEN_ALL_ACCESS, NULL, SecurityImpersonation,
...
void win_impersonate_back_to_self() 
{
			      ...
			      "Unable to revert to self.  (Error : %d)\n",
			      GetLastError());
...
char *win_impersonate_get_machine_name() 
{
		...
		return NULL;
	      ...
	      (ComputerNamePhysicalDnsFullyQualified, machineName, &mlen))
					 ...
					 uint16_t * length) 
{
			      ...
			      "Unable to decrypt the domain key!\n");
