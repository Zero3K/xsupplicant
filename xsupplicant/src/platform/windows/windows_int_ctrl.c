...
void windows_int_ctrl_take_ctrl(context * ctx) 
{
		...
		return;
			...
			0)
				      ...
				      "Disabled 802.1X on interface %s.\n",
				      ctx->desc);
...
void windows_int_ctrl_give_to_windows(context * ctx) 
{
		...
		return;
		    ...
		    if ((retval =
			 wlanapi_interface_enable_wzc(ctx->desc)) ==
			WLANAPI_NOT_AVAILABLE)
			 {
			if (wzc_ctrl_enable_wzc(ctx->intName) != 0)
				 {
				debug_printf(DEBUG_NORMAL,
					      "Unable to enable WZC for interface %s.\n",
					      ctx->desc);
...
void windows_int_ctrl_change(config_globals * newsettings) 
{
	...
	if (!xsup_assert((newsettings != NULL), "newsettings != NULL", FALSE))
		return;
	globals = config_get_globals();
	if (!xsup_assert((globals != NULL), "globals != NULL", FALSE))
		return;
	if (TEST_FLAG(globals->flags, CONFIG_GLOBALS_INT_CTRL) !=
	      TEST_FLAG(newsettings->flags, CONFIG_GLOBALS_INT_CTRL))
