...
void cardif_windows_ip_update() 
{
	...
	if (event_core_register
	      (ipupdate_handle, NULL, &ipupdatecallback, 0, HIGH_PRIORITY,
...
void cardif_windows_int_event_connect(context * ctx) 
{
	    ...
	    ipc_events_ui(NULL, IPC_EVENT_UI_LINK_UP, ctx->desc);
		    ...
		    UNSET_FLAG(wctx->flags, WIRELESS_SM_DISCONNECT_REQ);
			     ...
			     (ctx, &ssid, ssidsize) != 0)
					      ...
					      bssid[5]);
				cardif_windows_check_unexpected_change(ctx,
									NULL);
		...
		_beginthread(cardif_windows_events_delay_link_up_thread, 0,
			      ctx);
...
void cardif_windows_int_event_disconnect(context * ctx) 
{
		...
		return;
	      ...
	      && (cardif_GetBSSID(ctx, (char *)&bssid_dest) == XENONE))
...
char *cardif_windows_int_get_802_11_reason(ULONG reason) 
{
		    ...
		    _strdup
		    ("Deauthenticated because sending STA is leaving (or has left) the network");
		    ...
		    _strdup("Class 2 frame received from nonauthenticated STA");
		break;
	case 7:
		return
		    _strdup("Class 3 frame received from nonassociated STA");
		break;
	case 8:
		return
		    _strdup
		    ("Disassociated because sending STA is leaving (or has left) the network");
		break;
	case 9:
		return
		    _strdup
		    ("STA requresting (re)association is not authenticated with responding STA");
		    ...
		    _strdup
		    ("Disassociated because information in the Supported Channels element is unacceptible");
	...
	case 13:
		    ...
		    _strdup
		    ("Information element in 4-way handshake is different from (re)association request/probe response/beacon frame");
		    ...
		    _strdup
		    ("Invalid authenticated key management protocol (AKMP)");
		    ...
		    _strdup("Cipher suite rejected because of security policy");
	...
	case 32:
		    ...
		    _strdup
		    ("Disassocaited because QoS AP lacks sufficient bandwidth for this QoS STA");
		    ...
		    _strdup
		    ("Requested from peer STA as the STA is leaving the network (or resetting)");
		    ...
		    _strdup
		    ("Requested from peer STA as the STA received frames using the mechanism from which a setup is required");
					   ...
					   DWORD evtSize) 
{
	    ...
	    (PDOT11_DISASSOCIATION_PARAMETERS) & eventdata[pStat->
							   StatusBufferOffset];
		    ...
		    if (pDis->uReason & DOT11_ASSOC_STATUS_PEER_DEAUTHENTICATED)
			    ...
			    cardif_windows_int_get_802_11_reason(reason);
					   ...
					   DWORD evtSize) 
{
	    ...
	    (PDOT11_LINK_QUALITY_PARAMETERS) & eventdata[pStat->
							 StatusBufferOffset];
	pEntry =
	    (PDOT11_LINK_QUALITY_ENTRY) & eventdata[pParams->
		    ...
		    sprintf((char *)&temp, "%d", pEntry[0].ucLinkQuality);
		      ...
		      i <
		      (pParams->uLinkQualityListSize /
		       sizeof(DOT11_LINK_QUALITY_ENTRY)); i++)
			 {
			if (memcmp(pEntry[i].PeerMacAddr, ctx->dest_mac, 6) ==
			     0)
...
int cardif_windows_int_event_callback(context * ctx, HANDLE evtHandle) 
{
	    ...
	    if (!DeviceIoControl
		(sockData->devHandle, IOCTL_NDISPROT_INDICATE_STATUS, NULL, 0,
...
void cardif_windows_setup_int_events(context * ctx) 
{
	...
	if (sockData->eventdata == NULL)
	      ...
	      (sockData->devHandle, ctx, &cardif_windows_int_event_callback,
	...
	if (event_core_set_ovr(sockData->devHandle, EVENT_SECONDARY, ovr) !=
	      TRUE)
...
char *cardif_windows_event_get_guid(context * ctx) 
{
		...
		return NULL;
	      ...
	      ((ctx->intName != NULL), "ctx->intName != NULL", FALSE))
		return NULL;
    ...
    wchar_t * cardif_windows_events_get_ip_guid_str(context * ctx) 
{
		...
		return NULL;
	      ...
	      (CP_ACP, 0, fullpath, strlen(fullpath), longpath,
...
char *cardif_windows_events_get_interface_desc(wchar_t * dev, char live_only) 
{
	    ...
	    memset(&deviceNumber, 0x00, sizeof(deviceNumber));
		  ...
		  &dev[strlen("\\\\?\\Root#MS_PSCHEDMP#")], 4);
		     ...
		     (mySDIClass, &myData, (PSTR) & myString, 1024, NULL))
				...
				if (SetupDiGetDeviceRegistryProperty
				    (mySDIClass, &myData, SPDRP_FRIENDLYNAME,
...
void cardif_windows_events_interface_removed(void *devPtr) 
{
	    ...
	    cardif_windows_events_get_interface_desc((wchar_t *) devPtr, FALSE);
...
void cardif_windows_events_interface_inserted(void *devPtr) 
{
	    ...
	    cardif_windows_events_get_interface_desc((wchar_t *) devPtr, TRUE);
	...
	if (longName == NULL)
	      ...
	      (CP_ACP, 0, interfaceName, strlen(interfaceName), longName,
	    ...
	    if (event_core_locate_by_desc(interfaceName) != NULL)
			...
			if (cardif_windows_wmi_post_insert_bind(longName) ==
			      0)
