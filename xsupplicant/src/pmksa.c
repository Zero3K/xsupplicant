...
void pmksa_dump_cache(context *ctx)
{
	...
	debug_printf(DEBUG_INT, "---------------- PMKSA Cache Dump ---------------\n");
	while (cur != NULL)
	{
...
uint8_t *pmksa_create_pmkid(uint8_t *pmk, uint8_t *spa, uint8_t *aa)
{
	...
	if (tohash == NULL)
	{
		debug_printf(DEBUG_NORMAL, "Unable to allocate memory to create the PMKID hash string.\n");
		return NULL;
	}
	...
	if (pmkid == NULL)
	{
		debug_printf(DEBUG_NORMAL, "Unable to allocate memory to create PMKID!\n");
		FREE(tohash);
		return NULL;
	}
	...
	HMAC(EVP_sha1(), pmk, 32, tohash, (strlen(strhash)+12), pmkid, &ressize);
 ...
 **/
void pmksa_check_cache_depth(context *ctx)
{
	...
	while (cur != NULL)
	{
		if (cur->lifetime < oldest->lifetime) oldest = cur;
...
pmksa_cache_element *pmksa_pmkid_exists(context *ctx, uint8_t *pmkid)
{
	...
	if (!xsup_assert((pmkid != NULL), "pmkid != NULL", FALSE)) return NULL;

	if (!xsup_assert((ctx->intTypeData != NULL), "ctx->intTypeData != NULL", FALSE)) return NULL;
...
pmksa_cache_element *pmksa_sa_exists(context *ctx, uint8_t *aMac)
{
	...
	if (!xsup_assert((aMac != NULL), "aMac != NULL", FALSE)) return NULL;
...
int pmksa_add(context *ctx, uint8_t *aMac)
{
	...
	if (!xsup_assert((aMac != NULL), "aMac != NULL", FALSE)) return -1;
	...
	if (newElem == NULL)
	{
		debug_printf(DEBUG_NORMAL, "Unable to allocate memory to create new PMKSA cache entry.  This key will not be cached.\n");
		FREE(pmkid);
		return -1;
	}
	...
	if (newElem->pmk == NULL)
	{
		debug_printf(DEBUG_NORMAL, "Unable to allocate memory to cache the PMK.\n");
		FREE(pmkid);
		return -1;
	}
...
void pmksa_delete(context *ctx, pmksa_cache_element *toDelete)
{
	...
	if (!xsup_assert((toDelete != NULL), "toDelete != NULL", FALSE)) return;

	debug_printf(DEBUG_KEY_STATE, "Deleting PMKID %x from the PMKSA cache on %s.\n", toDelete->pmkid, ctx->desc);
	...
	debug_printf(DEBUG_KEY_STATE, "Unable to locate PMKID %x on interface %s!\n", toDelete->pmkid, ctx->desc);
}
 ...
 **/
void pmksa_age_out(context *ctx, uint8_t secs_elapsed)
{
	pmksa_cache_element *cur = NULL, *next = NULL;
	wireless_ctx *wctx = NULL;
	if (!xsup_assert((ctx != NULL), "ctx != NULL", FALSE)) return;

	if (!xsup_assert((secs_elapsed > 0), "secs_elapsed > 0", FALSE)) return;

	debug_printf(DEBUG_KEY_STATE, "PMKSA cache update for interface '%s'.  (%d secs elapsed)\n", ctx->desc, secs_elapsed);
	...
	while (cur != NULL)
	{
		...
		{
			debug_printf(DEBUG_NORMAL, "Aging out PMKSA entry for %02x:%02x:%02x:%02x:%02x:%02x on interface '%s'.\n",
				cur->authenticator_mac[0], cur->authenticator_mac[1], cur->authenticator_mac[2], cur->authenticator_mac[3],
				cur->authenticator_mac[4], cur->authenticator_mac[5], ctx->desc);
			next = cur->next;
 ...
 **/
void pmksa_seen(context *ctx, uint8_t *aMac, char *ssid)
{
	wireless_ctx *wctx = NULL;
	pmksa_cache_element *cur = NULL;
	config_globals *globals = NULL;
	...
	if (!xsup_assert((aMac != NULL), "aMac != NULL", FALSE)) return;

	if (!xsup_assert((ssid != NULL), "ssid != NULL", FALSE)) return;
	...
	while (cur != NULL)
	{
		if ((cur != NULL) && (memcmp(cur->authenticator_mac, aMac, 6) == 0) && (strcmp(cur->ssid, ssid) == 0))
		{
			// This entry has been seen.  Reset it's lifetime.

			debug_printf(DEBUG_INT, "Resetting ageout counter for %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	if (cur == NULL)
	{
		...
		debug_printf(DEBUG_NORMAL, "Creating OKC entry for new AP %02x:%02x:%02x:%02x:%02x:%02x on interface '%s'.\n",
			aMac[0], aMac[1], aMac[2], aMac[3], aMac[4], aMac[5], ctx->desc);
...
void pmksa_cache_deinit(context *ctx)
{
	...
	debug_printf(DEBUG_INT | DEBUG_DEINIT, "Clearing PMKSA cache for interface '%s'.\n", ctx->desc);
...
void pmksa_init_cache_update(context *ctx)
{
	...
	if (confdata == NULL) 
	{
		debug_printf(DEBUG_NORMAL, "Unable to get configuration information needed to enable the PMKSA cache maintenance.  Will use the default.\n");
		secs_elapsed = PMKSA_CACHE_REFRESH;
	}
	else
	{
	...
	timer_add_timer(ctx, PMKSA_CACHE_MGMT_TIMER, secs_elapsed, NULL, pmksa_cache_update);
}
...
int pmksa_populate_keydata(context *ctx, uint8_t *pmkid)
{
	...
	while (cur != NULL)
	{
		...
		if (memcmp(pmkid, cur->pmkid, 16) == 0)
		{
			if (ctx->statemachine->PMK != NULL) FREE(ctx->statemachine->PMK);

			ctx->statemachine->PMK = Malloc(32);
			if (ctx->statemachine->PMK == NULL)
			{
				debug_printf(DEBUG_NORMAL, "Unable to allocate memory to store the PMK for interface '%s'!\n", ctx->desc);
				return -1;
			}
 ...
 **/
struct found_ssids *pmksa_add_to_int_cache_list(context *ctx, struct found_ssids *ssid, 
												pmksa_list *apply_list, int entries)
{
	int i = 0;
	int x = 0;
	...
	if (!xsup_assert((ssid != NULL), "ssid != NULL", FALSE)) return NULL;

	if (!xsup_assert((apply_list != NULL), "apply_list != NULL", FALSE)) return NULL;

	if (!xsup_assert((entries > 0), "entries > 0", FALSE)) return NULL;
	...
	for (i = (entries-1); i >= 0; i--)
	{
		...
		if ((apply_list[i].ssid_element == NULL) || (config_ssid_best_signal(ssid, apply_list[i].ssid_element) == ssid))
		{
			// Move each entry down one in the list.  Except for the lowest entry, which
			// should be overwritten.
			for (x = i; x >= 1; x--)
			{
				apply_list[x-1].cache_element = apply_list[x].cache_element;
				apply_list[x-1].ssid_element = apply_list[x].ssid_element;
			}	

			apply_list[i].ssid_element = ssid;
			apply_list[i].cache_element = pmksa_sa_exists(ctx, ssid->mac);
			break;   // Stop the loop.
		}
	}
	...
	for (i = 0; i < entries; i++)
	{
		if (apply_list[i].ssid_element == NULL) return NULL;
	}
...
void pmksa_apply_cache(context *ctx)
{
	...
	{
		debug_printf(DEBUG_INT, "No PMKIDs are supported on this interface!  Won't attempt to set any.\n");
		return;
	}
	...
	if (apply_list == NULL)
	{
		debug_printf(DEBUG_NORMAL, "Unable to allocate memory to store our PMK cache list.\n");
		return;
	}

	if (wctx->cur_essid == NULL)
	{
		debug_printf(DEBUG_NORMAL, "Our current SSID is unknown on interface '%s'!  Can't generate PMKID list!\n", ctx->desc);
		return;
	}
	...
	while (ssids != NULL)
	{
		...
		if (strcmp(wctx->cur_essid, ssids->ssid_name) == 0)
		{
			...
			if (pmksa_sa_exists(ctx, ssids->mac) != NULL)
			{
				...
				{
					lowest = pmksa_add_to_int_cache_list(ctx, ssids, apply_list, wctx->pmkids_supported);
				}
			}
		}
		ssids = ssids->next;
