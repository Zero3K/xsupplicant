...
 SSIDListDlg::SSIDListDlg(QWidget * parent, QWidget * parentWindow,
			   Emitter * e, TrayApp * supplicant) :QWidget(parent),
...
} SSIDListDlg::~SSIDListDlg() 
{
		...
		Util::myDisconnect(m_pRefreshButton, SIGNAL(clicked()), this,
				    SLOT(rescanNetworks()));
	if (m_pConnectButton != NULL)
		Util::myDisconnect(m_pConnectButton, SIGNAL(clicked()), this,
				    SLOT(connectToSelectedNetwork()));
	if (m_pSSIDList != NULL)
		 {
		Util::myDisconnect(m_pSSIDList,
				    SIGNAL(ssidSelectionChange
					   (const WirelessNetworkInfo &)), this,
				    SLOT(handleSSIDListSelectionChange
					 (const WirelessNetworkInfo &)));
		Util::myDisconnect(m_pSSIDList,
				    SIGNAL(ssidDoubleClick
					   (const WirelessNetworkInfo &)), this,
				    SLOT(handleSSIDListDoubleClick
					 (const WirelessNetworkInfo &)));
		delete m_pSSIDList;
		}
	Util::myDisconnect(&m_refreshTimer, SIGNAL(timeout()), this,
			     SLOT(refreshScanTimeout()));
...
bool SSIDListDlg::initUI() 
{
	    ...
	    // load form
	    m_pRealForm =
	    FormLoader::buildform("SSIDListWindow.ui", m_pParentWindow);
	    ...
	    qFindChild < QPushButton * >(m_pRealForm, "buttonRefresh");
	m_pConnectButton =
	    qFindChild < QPushButton * >(m_pRealForm, "buttonConnect");
	m_pHeaderLabel =
	    qFindChild < QLabel * >(m_pRealForm, "headerAvailableNetworks");
		...
		Util::myConnect(m_pRefreshButton, SIGNAL(clicked()), this,
				 SLOT(rescanNetworks()));
	if (m_pConnectButton != NULL)
		Util::myConnect(m_pConnectButton, SIGNAL(clicked()), this,
				 SLOT(connectToSelectedNetwork()));
	
	    // assume existing rows in table is min we want displayed
	    m_pSSIDList =
	    new SSIDList(m_pRealForm, m_pSSIDTable, m_pSSIDTable->rowCount());
	    ...
	    Util::myConnect(m_pSSIDList,
			 ...
			 SIGNAL(ssidDoubleClick(const WirelessNetworkInfo &)),
			 this,
			 SLOT(handleSSIDListDoubleClick
			      (const WirelessNetworkInfo &)));
	Util::myConnect(&m_refreshTimer, SIGNAL(timeout()), this,
			 SLOT(refreshScanTimeout()));
...
} void SSIDListDlg::rescanNetworks(void) 
{
	    ...
	    retVal =
	    xsupgui_request_get_devname(this->m_curAdapter.toAscii().data(),
					&adapterName);
		    ...
		    xsupgui_request_get_physical_state(adapterName, &pState);
		if (retVal != REQUEST_SUCCESS || pState == WIRELESS_ASSOCIATED)
			 {
			if (QMessageBox::
			     warning(m_pRealForm, tr("Warning"),
				     tr
				     ("Refreshing the list of networks may cause your current connection to be dropped.  Do you wish to proceed?"),
			...
			Util::myConnect(m_pEmitter,
					    SIGNAL(signalScanCompleteMessage
						       (const QString &)),
					    this,
					    SLOT(wirelessScanComplete
						 (const QString &)));
			
			    // request scan for available networks
			    retVal =
			    xsupgui_request_wireless_scan(adapterName, FALSE);
				...
				if (m_pRescanDialog == NULL)
					    ...
					    new WirelessScanDlg(this,
								m_pRealForm);
					if (m_pRescanDialog != NULL)
						 {
						Util::
						    myConnect(m_pRescanDialog,
							      SIGNAL
							      (scanCancelled()),
							      this,
							      SLOT(cancelScan
					...
					Util::myConnect(m_pRescanDialog,
...
void SSIDListDlg::wirelessScanComplete(const QString & deviceName) 
{
		...
		Util::myDisconnect(m_pEmitter,
				       ...
				       SLOT(wirelessScanComplete
					    (const QString &)));
		Util::myDisconnect(m_pRescanDialog, SIGNAL(scanCancelled()),
...
void SSIDListDlg::cancelScan(void) 
{
			    ...
			    SIGNAL(signalScanCompleteMessage(const QString &)),
			    this, SLOT(wirelessScanComplete(const QString &)));
	Util::myDisconnect(m_pRescanDialog, SIGNAL(scanCancelled()), this,
			    SLOT(cancelScan()));
} void SSIDListDlg::
handleSSIDListSelectionChange(const WirelessNetworkInfo & network) 
{
...
handleSSIDListDoubleClick(const WirelessNetworkInfo & network) 
{
	...
	int retVal;
	    ...
	    xsupgui_request_enum_connections((CONFIG_LOAD_GLOBAL |
			...
			char *adapterName = NULL;
			    ...
			    xsupgui_request_get_devname(this->m_curAdapter.
							toAscii().data(),
							&adapterName);
			if (retVal == REQUEST_SUCCESS && adapterName != NULL)
				retVal =
				    xsupgui_request_set_connection(adapterName,
								   connList.
								   at(0).
								   toAscii().
								   data());
				    ...
				    tr
				    ("An error occurred while connecting to the network '%1'.").
				    arg(netInfo.m_name);
	    ...
	    if (found == false)
		     ...
		     && pNewConn != NULL)
			    ...
			    _strdup(netInfo.m_name.toAscii().data());
			    ...
			    switch (netInfo.m_assoc_modes) {
				    ...
				    arg(netInfo.m_name);
				     ...
				     information(m_pRealForm, tr(""), msg,
					...
					(CONFIG_LOAD_USER,
					 pNewConn) == REQUEST_SUCCESS)
					    ...
					    xsupgui_request_get_devname(this->
									m_curAdapter.
									toAscii
									().
									data(),
...
void SSIDListDlg::finishConnectionWizard(bool success,
					   ...
					   const QString &) 
{
		    ...
		    xsupgui_request_get_devname(this->m_curAdapter.toAscii().
						data(), &adapterName);
				...
				success =
			      ...
			      && pConn->ssid != NULL
			      && QString(pConn->ssid).isEmpty() == false)
				message =
				    tr
				    ("An error occurred while connecting to the wireless network '%1'.").
				    arg(QString(pConn->ssid));
						 ...
						 QString adapterDesc) 
{
	    ...
	    new ConnectionSelectDlg(this, m_pRealForm, connList, adapterDesc);
...
} void SSIDListDlg::refreshScanTimeout(void) 
{
			    ...
			    SIGNAL(signalScanCompleteMessage(const QString &)),
			    this, SLOT(wirelessScanComplete(const QString &)));
	QMessageBox::critical(m_pRealForm, tr("Network Scan Timeout"),
				tr
				("The scan for wireless networks timed out. Please try again."));
