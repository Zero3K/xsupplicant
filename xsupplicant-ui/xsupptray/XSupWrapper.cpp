...
QString XSupWrapper::getUniqueConnectionName(const QString & suggestedName)
{
	...
	if (suggestedName.contains(rx) == true)
	{
		...
		newNamePrefix.remove((newNamePrefix.length() - rx.cap(0).length()),
			rx.cap(0).length());
	}
	else
	{
								   ...
								   config_profiles ** pProfile) 
{
	...
	*pProfile = NULL;
	...
	int retVal = xsupgui_request_get_profile_config(config_type,
								  ...
								  config_trusted_server ** pServer) 
{
	...
	*pServer = NULL;
	...
	int retVal =xsupgui_request_get_trusted_server_config(config_type,
		serverName.toAscii().data(), pServer);
...
bool XSupWrapper::isProfileInUse(const QString & profileName)
{
	...
	{
		conn_enum * pConfig = NULL;
		int retVal;

		retVal = xsupgui_request_enum_connections((CONFIG_LOAD_GLOBAL | CONFIG_LOAD_USER),
			&pConfig);
		...
		if (pMethod->method_num == EAP_TYPE_PEAP)
		{
			...
			if (mypeap != NULL)
			{
				...
				if (!serverName.isEmpty())
				{
					...
					XSupWrapper::getConfigServer((*inconfig),
						serverName,
						pServer);
...
bool XSupWrapper::isTrustedServerInUse(const QString & serverName)
{
	...
	if (retVal == REQUEST_SUCCESS && pProfile != NULL)
	{
		...
		while (pProfile[i].name != NULL && inUse == false)
		{
			...
			XSupWrapper::getTrustedServerForProfile(pProfile[i].config_type,
				QString(pProfile[i].name),
				&pServer,
				&config_type);
...
void XSupWrapper::getAndDisplayErrors(void) 
{
	...
	if (retval == REQUEST_SUCCESS)
	{
		if (msgs && msgs[0].errmsgs)
		{
			...
			QMessageBox::critical(NULL,
				QWidget::tr("XSupplicant Error Summary"),
		...
		if ((retval == REQUEST_SUCCESS) && (pName != NULL))
		{
			...
			if (connectionName.compare(pName) == 0)
			{
				...
				else
				{
					...
					if (xsupgui_request_get_1x_state(interfaceName.toAscii().data(),
						&state) == REQUEST_SUCCESS)
