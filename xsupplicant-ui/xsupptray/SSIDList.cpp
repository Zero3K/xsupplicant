...
SSIDList::SSIDList(QWidget * parent, QTableWidget * tableWidget,
		     int minRowCount /* =0 */ )
...
} SSIDList::~SSIDList() 
{
		...
		Util::myDisconnect(m_pTableWidget,
				    SIGNAL(itemSelectionChanged()), this,
				    SLOT(handleSSIDTableSelectionChange()));
		Util::myDisconnect(m_pTableWidget,
				    SIGNAL(cellDoubleClicked(int, int)), this,
				    SLOT(handleSSIDTableDoubleClick(int, int)));
...
} void SSIDList::initUI(void) 
{
		    ...
		    // network name
		    m_pTableWidget->horizontalHeaderItem(SSIDList::COL_NAME)->
		    setText(tr("Network Name"));
		m_pTableWidget->horizontalHeader()->
		    setResizeMode(SSIDList::COL_NAME, QHeaderView::Stretch);
		
		    // signal
		    m_pTableWidget->horizontalHeaderItem(SSIDList::COL_SIGNAL)->
		    setText(tr("Signal"));
		m_pTableWidget->horizontalHeader()->
		    resizeSection(SSIDList::COL_SIGNAL, 72);
		
		    // security
		    m_pTableWidget->
		    horizontalHeaderItem(SSIDList::COL_SECURITY)->
		    setText(tr("Security"));
		m_pTableWidget->horizontalHeader()->
		    resizeSection(SSIDList::COL_SECURITY, 100);
		
		    // 802.11
		    m_pTableWidget->horizontalHeaderItem(SSIDList::COL_802_11)->
		    setText(tr("802.11"));
		m_pTableWidget->horizontalHeader()->
		    resizeSection(SSIDList::COL_802_11, 80);
		    ...
		    m_pTableWidget->
		    setItemDelegateForColumn(SSIDList::COL_802_11,
					     new TableImageDelegate);
		
		    // don't show header on left side of table
		    m_pTableWidget->verticalHeader()->setVisible(false);
		m_pTableWidget->clearContents();
		Util::myConnect(m_pTableWidget,
				  SIGNAL(itemSelectionChanged()), this,
				  SLOT(handleSSIDTableSelectionChange()));
...
void SSIDList::refreshList(const QString & adapterName) 
{
	...
	int nNetworks = m_curNetworks.size();
	m_pTableWidget->setRowCount(std::max <
				     int >(this->m_minRowCount, nNetworks));
			    ...
			    nameItem =
			    new QTableWidgetItem(m_curNetworks.at(i).m_name,
						 1000 + i);
			if (nameItem != NULL)
				m_pTableWidget->setItem(i, SSIDList::COL_NAME,
							 nameItem);
			    ...
			    new QTableWidgetItem(securityText, 1000 + i);
			if (securityItem != NULL)
				m_pTableWidget->setItem(i,
							 SSIDList::COL_SECURITY,
							 securityItem);
				...
				unsigned char modes =
				    m_curNetworks.at(i).m_modes;
				GraphSortItem * standards =
				    new GraphSortItem(modes);
...
} void SSIDList::refreshCompleteList() 
{
	...
	int nNetworks = m_curNetworks.size();
			    ...
			    new QTableWidgetItem(signalText, 1000 + i);
				...
				unsigned char modes =
				    ...
				    iter = m_pixmapMap.constFind(labelFileName);
					    ...
					    loadicon(labelFileName);
					if (p != NULL)
						 {
						m_pixmapMap.
						    insert(labelFileName, *p);
						iter =
						    m_pixmapMap.
						    constFind(labelFileName);
...
QList < WirelessNetworkInfo > SSIDList::getNetworkInfo(QString adapterName) 
{
				    ...
				    xsupgui_request_enum_ssids(pInterfaceList
							       [i].name,
							       &pSSIDList);
...
/*
					...
					xsupgui_request_free_ssid_enum
					    (&pSSIDList);
...
QList < WirelessNetworkInfo > SSIDList::getCompleteNetworkInfo() 
{
			    ...
			    xsupgui_request_enum_ssids(pInterfaceList[i].name,
						       &pSSIDList);
					    ...
					    // if not empty ssid (this represents a non-broadcast SSID)
					    if (QString(pSSIDList[j].ssidname).
						isEmpty() == false)
						    ...
						    pSSIDList[j].ssidname;
						networkInfo.m_signalStrength =
						    int (pSSIDList[j].
							 percentage);
						networkInfo.m_assoc_modes = 0;
						unsigned int abilities =
						    pSSIDList[j].abil;
						if ((abilities & ABILITY_ENC)
						     != 0)
...
void SSIDList::handleSSIDTableSelectionChange(void) 
{
				...
				if (selectedItems.at(i)->column() ==
				     SSIDList::COL_NAME)
					 {
					emit ssidSelectionChange(m_curNetworks.
								  at
								  (selectedItems.
								   at(i)->
								   type() -
...
void SSIDList::handleSSIDTableDoubleClick(int row, int) 
{
		    ...
		    m_pTableWidget->item(row, SSIDList::COL_NAME);
		
		    // check if they clicked in a row w/ a network listed
		    if (item != NULL)
			emit ssidDoubleClick(m_curNetworks.
					      at(item->type() - 1000));
...
void SSIDList::tempAssocModeHack(void) 
{
		      ...
		      m_assoc_modes & WirelessNetworkInfo::
		      SECURITY_WPA2_ENTERPRISE) != 0)
			iter->m_assoc_modes =
			    WirelessNetworkInfo::SECURITY_WPA2_ENTERPRISE;
		
		else if ((iter->
			  m_assoc_modes & WirelessNetworkInfo::
			  SECURITY_WPA_ENTERPRISE) != 0)
			iter->m_assoc_modes =
			    WirelessNetworkInfo::SECURITY_WPA_ENTERPRISE;
		
		else if ((iter->
			  m_assoc_modes & WirelessNetworkInfo::
			  SECURITY_WPA2_PSK) != 0)
			iter->m_assoc_modes =
			    WirelessNetworkInfo::SECURITY_WPA2_PSK;
		
		else if ((iter->
			  m_assoc_modes & WirelessNetworkInfo::
			  SECURITY_WPA_PSK) != 0)
			iter->m_assoc_modes =
			    WirelessNetworkInfo::SECURITY_WPA_PSK;
