...
bool MachineAuthWizard::initUI(void)
{
	    ...
	    setWindowTitle(tr("Machine Authentication Wizard - XSupplicant"));
	    ...
	    if (m_pCancelButton != NULL)
		    ...
		    setText(tr("Configure Machine Authentication"));
...
bool MachineAuthWizard::loadPages(void) 
{
		...
		int cnt = m_pStackedWidget->count();
		    ...
		    for (int i = 0; i < MachineAuthWizard::pageLastPage; i++)
				    ...
				    new WizardPageNetworkTypes(this,
							       m_pStackedWidget);
				    ...
				    new WizardPageMachineAuthFinished(this,
								      m_pStackedWidget);
...
{
	if (newPageIdx != MachineAuthWizard::pageNoPage
    ...
    bool MachineAuthWizard::saveGlobalSettings() 
{
		...
		myGlobals->wiredMachineAuthConnection =
		    _strdup(m_connData.m_connectionName.toAscii().data());
		...
		myGlobals->wirelessMachineAuthConnection =
		    _strdup(m_connData.m_connectionName.toAscii().data());
...
MachineAuthWizard::wizardPages MachineAuthWizard::getNextPage(void)
{
			    ...
			    MachineAuthWizard::pageMachineAuthFinishPage;
