...
bool ConnectMgrDlg::initUI(void) 
{
	...
	// load form
	m_pRealForm = FormLoader::buildform("ConnectionManagerWindow.ui",
		m_pParentWindow);
	if (m_pRealForm == NULL)
		return false;
	...
	if (m_pCheckSupplicants != NULL)
		m_pCheckSupplicants->setText(tr("Run other supplicant check on startup"));
	...
	if (m_pAllowMachineAuthContinue != NULL)
		m_pAllowMachineAuthContinue->setText(tr("Allow logged on users to keep the machines authentication"));

	m_pForceMulticast = qFindChild < QCheckBox * >(m_pRealForm, "forceMulticast");
	if (m_pForceMulticast != NULL)
		m_pForceMulticast->setText(tr("Force wired connections to use the multicast MAC address"));
...
void ConnectMgrDlg::clearWiredAutoConnection(const QString & connectionName) 
{
	...
	if (xsupgui_request_get_connection_config(CONFIG_LOAD_GLOBAL, connectionName.toAscii().data(),
		&myConn) == REQUEST_SUCCESS)
	{
		...
		if (xsupgui_request_get_are_administrator(&are_admin) != REQUEST_SUCCESS)
		{
			QMessageBox::critical(this, tr("Error"),
				tr("Unable to determine if you have permissions to edit this setting."));
		...
		{
			QMessageBox::critical(this, tr("Error"),
				tr("The default wired connection is defined in the system configuration.  You must be an administrator to change it."));
			xsupgui_request_free_connection_config(&myConn);
			return;
		}
	}
	...
	for (x = CONFIG_LOAD_GLOBAL; x <= CONFIG_LOAD_USER; x++)
	{
		...
		if (xsupgui_request_enum_connections(x, &connEnum) == REQUEST_SUCCESS)
		{
			for (i = 0; connEnum[i].name != NULL; i++)
			{
				if (((connEnum[i].ssid == NULL)	|| (strlen(connEnum[i].ssid) == 0))
					&& (connEnum[i].priority !=	DEFAULT_PRIORITY))
				{
					if (xsupgui_request_get_connection_config(x, connEnum[i].name, &myConn) != REQUEST_SUCCESS)
					{
						QMessageBox::critical(this,
							tr("Error"),
							tr("Unable to reset the priority settings on connection %1.  The wired default behavior may not be what is expected.").arg(connEnum[i].name));
					}
					else
					{
	...
	for (x = CONFIG_LOAD_GLOBAL; x <= CONFIG_LOAD_USER; x++)
	{
		...
		if (xsupgui_request_enum_connections(x, &connEnum) == REQUEST_SUCCESS)
		{
			for (i = 0; connEnum[i].name != NULL; i++)
			{
				...
				if ((!alreadySet) && ((connEnum[i].ssid == NULL)
					|| (strlen(connEnum[i].ssid) == 0))
					&& (connectionName == QString(connEnum[i].name)))
				{
					if (xsupgui_request_get_connection_config(x, connEnum[i].name, &myConn) != REQUEST_SUCCESS)
					{
						QMessageBox::critical(this,
							tr("Error"),
							tr("Unable to set the priority settings on connection %1.  The wired default behavior may not be what is expected.").arg(connEnum[i].name));
					}
					else
					{
						myConn->priority = 1;
...
void ConnectMgrDlg::populateConnectionsList(void) 
{
	if (m_pConnections != NULL && m_pConnectionsTable != NULL)
	{
		...
		m_pConnectionsTable->setRowCount(std::max < int >(this->m_minConnListRowCount,
			m_nConnections));
		...
		for (int i = 0; i < m_nConnections; i++)
		{
			...
			int retVal = xsupgui_request_get_connection_config(m_pConnections[i].config_type,
				m_pConnections[i].name, &pConfig);
			if (retVal == REQUEST_SUCCESS && pConfig != NULL)
			{
			...
			if (bVolatile == false)
			{
				...
				nameItem = new QTableWidgetItem(m_pConnections[i].name,
					QTableWidgetItem::UserType + m_pConnections[i].config_type);
				if (nameItem != NULL)
...
void ConnectMgrDlg::deleteSelectedConnection(void) 
{
	...
	if (selectedItems.isEmpty() == false)
	{
		...
		if ((selItem->row() >= 0) && (selItem->row() < m_nConnections))
		{
			...
			if (xsupgui_request_get_is_connection_in_use(connName.toAscii().data(),
				&inuse) == REQUEST_SUCCESS)
			{
				...
				else
				{
					QMessageBox::warning(m_pRealForm,
						...
						tr("The connection '%1' cannot be deleted because it is currently in use.  Please disconnect from the network before deleting the connection.").arg(connName));
					canDelete = false;
				}
			}
			...
			if (canDelete == true)
			{
				...
				if (QMessageBox::question(m_pRealForm,
					...
					QMessageBox::No) ==	QMessageBox::Yes)
				{
					...
					if (success == false)
					{
						QMessageBox::critical(m_pRealForm,
							...
							tr("An error occurred when attempting to delete the connection '%1'.  If the connection is in use, please disconnect and try again.").arg(connName));
					}
					else
					{
						...
						if (pProfile != NULL && XSupWrapper::isProfileInUse(QString(pProfile->name)) == false)
						{
							...
							{
								QMessageBox::critical(m_pRealForm,
									tr("Error Deleting Profile"),
									tr("An error occurred while attempting to delete the profile '%1'").arg(pProfile->name));
							}
							else
							{
								...
								if (pServer != NULL	&& XSupWrapper::isTrustedServerInUse(QString(pServer->name)) == false)
									...
									QMessageBox::critical(m_pRealForm,
									tr("Error Deleting Trusted Server"),
									tr("The trused server '%1' cannot be deleted because it is being used by multiple profiles").arg(pServer->name));
							}
						...
						else if (pProfile != NULL)
						{
							QMessageBox::critical(m_pRealForm,
								tr("Error Deleting Profile"),
								tr("The profile '%1' cannot be deleted because it is being used by multiple connection profiles").arg(pProfile->name));
						}
	...
	if (m_pConnWizard == NULL)
	{
		...
		if (m_pConnWizard != NULL)
		{
			...
			if (m_pConnWizard->create() == true)
			{
				...
				Util::myConnect(m_pConnWizard,
					SIGNAL(finished(bool, const QString &, const QString &)),
					this,
					SLOT(finishConnectionWizard(bool, const QString &, const QString &)));
				m_pConnWizard->init();
				m_pRealForm->setCursor(Qt::ArrowCursor);
				m_pConnWizard->show();
			}
			else
			{
				...
				QMessageBox::critical(m_pRealForm, tr("Error"),
					tr("An error occurred when attempting to launch the Connection Wizard"));
										   ...
										   const QString &) 
{
	if (success == false)
		...
		tr("An error occurred while saving the configuration data you provided."));
	this->cleanupConnectionWizard();
}
...
void ConnectMgrDlg::cleanupConnectionWizard(void) 
{
	if (m_pConnWizard != NULL)
	{
		...
		Util::myDisconnect(m_pConnWizard,
			SIGNAL(finished(bool, const QString &, const QString &)), this,
			SLOT(finishConnectionWizard(bool, const QString &, const QString &)));
		delete m_pConnWizard;
		m_pConnWizard = NULL;
	}
											...
											const QString &) 
{
	if (success == false)
		QMessageBox::critical(m_pRealForm,
			tr("Error saving machine authentication data"),
			tr("An error occurred while saving the machine authentication data you provided."));
...
void ConnectMgrDlg::cleanupMachineAuthWizard(void) 
{
	if (m_pMachineAuth != NULL)
	{
		...
		Util::myDisconnect(m_pMachineAuth,
			SIGNAL(finished(bool, const QString &, const QString &)), this,
			SLOT(finishMachineAuthWizard(bool, const QString &,	const QString &)));
	...
	if (success == true && pConfig != NULL)
	{
		...
		if (xsupgui_request_get_is_connection_in_use(connName.toAscii().data(), &inuse) == REQUEST_SUCCESS)
		{
			...
			else
			{
				QMessageBox::warning(m_pRealForm,
					tr("Connection In Use"),
					tr("The connection '%1' cannot be edited because it is currently in use.  Please disconnect from the network before editing the connection.").arg(connName));
				editable = false;
			}
		}
		...
		if (editable == true)
		{
			...
			{
				success = XSupWrapper::getConfigProfile(profile_type,
					QString(pConfig->profile), &pProfile);
			...
			if (success == true && pProfile != NULL)
				success = XSupWrapper::getTrustedServerForProfile(profile_type,
					QString(pProfile->name), &pServer, &ts_type);
			...
			{
				// create the wizard if it doesn't already exist
				m_pConnWizard =	new ConnectionWizard(QString(""), this,
					m_pRealForm,
					m_pEmitter);
				if (m_pConnWizard != NULL)
				{
		...
		if (m_pMachineAuth->create() == true)
		{
			...
			if (xsupgui_request_get_connection_config(CONFIG_LOAD_GLOBAL, "Machine Authentication Connection", &pConn) == REQUEST_SUCCESS)
			{
				...
				else
				{
					...
					QMessageBox::critical(this,
						tr("Machine Authentication Configuration Error"),
						tr("There was an error gathering existing machine authentication data to edit."));
					return;
				}
			}
...
void ConnectMgrDlg::configUpdate() 
{
	...
	{
		QMessageBox::critical(this, tr("Error"),
			tr("Unable to allocate memory needed to store configuration globals."));
		return;
	}
	...
	else
	{
		...
		default:
			QMessageBox::critical(this, tr("Form design error"),
				tr("You have selected a log level setting that is not understood.  Your form design may be incorrect.  Defaulting to NORMAL logging."));
			globals->loglevel = DEBUG_NORMAL;
			break;
		}
	}
	...
	if (xsupgui_request_set_globals_config(globals) != REQUEST_SUCCESS)
	{
		QMessageBox::critical(this, tr("Cannot Save"),
			tr("Unable to save your configuration data."));
...
void ConnectMgrDlg::viewLog() 
{
		...
		|| (m_pViewLogDialog->attach() == false))
	{
		QMessageBox::critical(this, tr("Form Error"),
			tr("Unable to load the form 'ViewLogWindow.ui'."));
...
void ConnectMgrDlg::populateSettingsTabs() 
{
	...
	if (xsupgui_request_get_globals_config(&myglobals) != REQUEST_SUCCESS)
	{
		QMessageBox::critical(this, tr("Request Failed"),
			tr("Unable to get global configuration settings."));
		return;
	}
