...
#endif	/*  */
ConnectionSelectDlg::ConnectionSelectDlg(QWidget * parent,
...
bool ConnectionSelectDlg::initUI(void)
{
	    ...
	    // load form
	    m_pRealForm =
	    FormLoader::buildform("ConnectionPromptWindow.ui", m_pParentWindow);
	    ...
	    m_pConnectionCombo =
	    qFindChild < QComboBox * >(m_pRealForm, "comboBoxConnection");
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelDialogMsg");
		    ...
		    setText(tr
			    ("More than one connection profile is configured for this network.  Please indicate which should be used to connect"));
...
void ConnectionSelectDlg::okay(void) 
{
	    ...
	    // if only one connection for this network and adapter, connect to it
			...
			success =
			    ...
			    retVal =
			    xsupgui_request_get_devname(m_adapterToUse.
							toAscii().data(),
							&adapterName);
			    ...
			    arg(QString(pConn->ssid).arg(retVal));
