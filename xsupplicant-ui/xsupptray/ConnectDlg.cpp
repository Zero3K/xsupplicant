...
} ConnectDlg::~ConnectDlg() 
{
		...
		Util::myDisconnect(m_pWirelessBrowseButton, SIGNAL(clicked()),
				    this, SLOT(showSSIDList()));
	if (m_pWirelessAdapterList != NULL)
		Util::myConnect(m_pWirelessAdapterList,
				 SIGNAL(currentIndexChanged(int)), this,
				 SLOT(selectWirelessAdapter(int)));
	if (m_pWirelessConnectionList != NULL)
		Util::myDisconnect(m_pWirelessConnectionList,
				    SIGNAL(currentIndexChanged(int)), this,
				    SLOT(selectWirelessConnection(int)));
	if (m_pWiredAdapterList != NULL)
		Util::myConnect(m_pWiredAdapterList,
				 SIGNAL(currentIndexChanged(int)), this,
				 SLOT(selectWiredAdapter(int)));
	if (m_pWiredConnectionList != NULL)
		Util::myDisconnect(m_pWiredConnectionList,
				    SIGNAL(currentIndexChanged(int)), this,
				    SLOT(selectWiredConnection(int)));
	if (m_pConnWizardButton != NULL)
		Util::myDisconnect(m_pConnWizardButton, SIGNAL(clicked()),
				    this, SLOT(launchConnectionWizard()));
	if (m_pWiredConnectButton != NULL)
		Util::myDisconnect(m_pWiredConnectButton, SIGNAL(clicked()),
				    this,
				    SLOT(connectDisconnectWiredConnection()));
		...
		Util::myConnect(m_pWirelessConnectionInfo, SIGNAL(clicked()),
				 this, SLOT(showWirelessConnectionInfo()));
	if (m_pWiredConnectionInfo != NULL)
		Util::myConnect(m_pWiredConnectionInfo, SIGNAL(clicked()),
				 this, SLOT(showWiredConnectionInfo()));
	Util::myDisconnect(&m_timer, SIGNAL(timeout()), this,
			     SLOT(timerUpdate()));
			    ...
			    this, SLOT(slotSignalUpdate(const QString &, int)));
			     ...
			     SLOT(interfaceInserted(char *)));
	Util::myDisconnect(m_pEmitter, SIGNAL(signalInterfaceRemoved(char *)),
			    this, SLOT(interfaceRemoved(char *)));
			    ...
			    SLOT(stateChange
			     ...
			     SIGNAL(signalConnectionUnbound(const QString &)),
			     this, SLOT(updateWirelessState(void)));
...
bool ConnectDlg::initUI(void)
{
		...
		Util::myConnect(m_pWirelessBrowseButton, SIGNAL(clicked()),
				 this, SLOT(showSSIDList()));
		...
		Util::myConnect(m_pWirelessConnectionList,
				 SIGNAL(currentIndexChanged(int)), this,
				 SLOT(selectWirelessConnection(int)));
		...
		Util::myConnect(m_pWiredConnectionList,
				 SIGNAL(currentIndexChanged(int)), this,
				 SLOT(selectWiredConnection(int)));
	if (m_pConnWizardButton != NULL)
		Util::myConnect(m_pConnWizardButton, SIGNAL(clicked()), this,
				 SLOT(launchConnectionWizard()));
	if (m_pWiredConnectButton != NULL)
		Util::myConnect(m_pWiredConnectButton, SIGNAL(clicked()), this,
				 SLOT(connectDisconnectWiredConnection()));
	if (m_pWirelessConnectButton != NULL)
		Util::myConnect(m_pWirelessConnectButton, SIGNAL(clicked()),
				 this,
				 SLOT(connectDisconnectWirelessConnection()));
	...
	Util::myConnect(m_pEmitter, SIGNAL(signalInterfaceRemoved(char *)),
			 this, SLOT(interfaceRemoved(char *)));
...
void ConnectDlg::show(void) 
{
	    ...
	    if (m_pAdapterTabControl != NULL)
		    ...
		    if (m_pAdapterTabControl->isTabEnabled(0) == true
			|| m_pAdapterTabControl->isTabEnabled(1) == false)
	    ...
	    if ((m_pWiredAdapterList == NULL
		 ...
		 0)  &&(m_pWirelessAdapterList == NULL
			 || m_pWirelessAdapterList->count() == 0))
				       ...
				       ("XSupplicant was unable to locate any network adapters in the system.  You will not be able to connect to any networks."));
...
void ConnectDlg::populateWirelessAdapterList(void) 
{
		...
		Util::myDisconnect(m_pWirelessAdapterList,
...
} void ConnectDlg::populateWiredAdapterList(void) 
{
		    ...
		    // QT generates events while populating a combobox, so stop listening during this time
		Util::myDisconnect(m_pWiredAdapterList,
				       SIGNAL(currentIndexChanged(int)),
				       this, SLOT(selectWiredAdapter(int)));
		QString oldSelection =
		    m_pWiredAdapterList->itemText(m_pWirelessAdapterList->
						  currentIndex());
...
void ConnectDlg::showSSIDList() 
{
		    ...
		    new SSIDListDlg(this, m_pRealForm, m_pEmitter, m_pTrayApp);
		     ...
		     || m_pSSIDListDlg->create() == false)
					       ...
					       tr
					       ("The SSID List Dialog form was unable to be created.  It is likely that the UI design file was not available.  Please correct this and try again."));
...
} void ConnectDlg::cleanupConnectionWizard(void) 
{
				    ...
				    SLOT(cleanupConnectionWizard()));
		Util::myDisconnect(m_pConnWizard, SIGNAL(finished(bool)), this,
		...
		isConnectionActive(m_currentWirelessAdapterName,
		       ...
		       connectToConnection(m_currentWirelessAdapterName,
					   ...
					   currentText())) != REQUEST_SUCCESS)
						       ...
						       tr("Connection Error"),
						       tr
						       ("Unable to establish a wireless connection.  Error : %1").
						       arg(result));
			    ...
			    while (msgs[i].errmsgs != NULL)
				    ...
				    QString("- %1\n").arg(msgs[i].errmsgs);
						...
						("The following errors were returned from XSupplicant while starting up or attempting to connect.\n%1")
					       ...
					       tr("XSupplicant Error"),
					       tr
					       ("The supplicant service indicated there were queued errors, but didn't return any when asked!"));
...
void ConnectDlg::disconnectWiredConnection(void) 
{
	      ...
	      REQUEST_SUCCESS)
				       ...
				       ("An error occurred while disconnecting device '%1'.  (Error : %2)\n").
...
void ConnectDlg::updateWirelessState(void) 
{
		...
		char *connName = NULL;
		    ...
		    if (m_pAdapterTabControl != NULL
			&& m_pAdapterTabControl->currentIndex() == 0)
...
void ConnectDlg::updateWiredState(void) 
{
		      ...
		      (m_currentWiredAdapterName.toAscii().data(),
		       &state) == REQUEST_SUCCESS)
			    ...
			    // don't mess with timer if this page isn't visible
			    if (m_pAdapterTabControl != NULL
				&& m_pAdapterTabControl->currentIndex() == 1)
				 {
				if (status == Util::status_connected)
					this->
					    startConnectedTimer
					    (m_currentWiredAdapterName.
					     toAscii().data());
...
//      this->updateElapsedTime();
		...
		m_pConnInfo->showTime();	// Update the info dialog at the same interval.
}
...
} void ConnectDlg::stateChange(const QString & intName, int, int, int,
				  unsigned int) 
{
...
void ConnectDlg::showWirelessConnectionInfo(void) 
{
		    ...
		    new ConnectionInfoDlg(this, m_pRealForm, m_pEmitter,
					  &m_time);
					       ...
					       tr
					       ("An error occurred when trying to launch the Connection Info dialog"));
...
void ConnectDlg::updateWirelessSignalStrength(void) 
{
	    ...
	    (m_currentWirelessAdapterName.toAscii().data(), &signal);
...
{
} void ConnectDlg::slotSignalUpdate(const QString & intName,
				       int sigStrength) 
{
