...
void ConnectDlg::show(void) 
{
			 ...
			 || m_pWirelessAdapterList->count() == 0))
	{
		QMessageBox::critical(m_pRealForm,
				...
				tr("Unable to locate any network adapters in the system.  You will not be able to connect to any networks."));
	}
}
	...
	if (m_pSSIDListDlg == NULL)
	{
		     ...
		     || m_pSSIDListDlg->create() == false)
		{
			QMessageBox::critical(m_pRealForm,
					       ...
					       tr("The SSID List Dialog form was unable to be created.  It is likely that the UI design file was not available.  Please correct this and try again."));
			if (m_pSSIDListDlg != NULL)
			{
				delete m_pSSIDListDlg;
				m_pSSIDListDlg = NULL;
					   ...
					   m_pWirelessConnectionList->currentText())) != REQUEST_SUCCESS)
		{
			...
			case IPC_ERROR_INTERFACE_NOT_FOUND:
				QMessageBox::critical(this,
						       tr("Connection Error"),
						       tr("The requested interface is no longer available."));
				break;
			
			case IPC_ERROR_INVALID_CONN_NAME:
				QMessageBox::critical(this,
						       tr("Connection Error"),
						       tr("The connection name requested is invalid."));
				break;
			
			case IPC_ERROR_SSID_NOT_FOUND:
				QMessageBox::critical(this,
						       tr("Connection Error"),
						       tr("The requested wireless network was not found."));
				break;
			
			case IPC_ERROR_INVALID_PROF_NAME:
				QMessageBox::critical(this,
						       tr("Connection Error"),
						       tr("The connection you are attempting to connect to is missing a profile."));
				break;
...
void ConnectDlg::showWirelessConnectionInfo(void) 
{
	if (m_pConnInfo == NULL)
	{
		...
		if (m_pConnInfo != NULL && m_pConnInfo->create() != false)
		{
			...
			QMessageBox::critical(m_pRealForm, tr("Error"),
					       tr("An error occurred when trying to launch the Connection Info dialog"));
	}
	else
	{
		m_pConnInfo->setAdapter(m_currentWirelessAdapterDesc);
