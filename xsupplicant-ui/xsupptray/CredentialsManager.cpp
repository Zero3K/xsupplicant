...
} CredentialsManager::~CredentialsManager() 
{
			    ...
			    SIGNAL(signalConnectionDisconnected
				   (const QString &)), this,
					       ...
					       const QString & password) 
{
		...
		for (iter = m_credVector.begin(); iter != m_credVector.end();
		      iter++)
	    ...
	    push_back(CredentialsManager::
		      CredData(connectionName, adaptDesc, userName, password));
					     ...
					     int, int newstate, unsigned int) 
{
	      ...
	      && (newstate == AUTHENTICATED || newstate == S_FORCE_AUTH))
		 {
		char *connName;
		int retval =
		    xsupgui_request_get_conn_name_from_int(intName.toAscii().
							   data(), &connName);
			    ...
			    XSupWrapper::getConfigConnection(config_type,
							     QString(connName),
							     &pConn);
				    ...
				    XSupWrapper::
				    getConfigConnection(config_type,
							QString(connName),
							&pConn);
				    ...
				    if (pConn->association.auth_type !=
					AUTH_PSK)
					 {
					QVector <
					    CredentialsManager::CredData >::
					    iterator iter;
					for (iter = m_credVector.begin();
					      iter != m_credVector.end();
					      iter++)
						 {
						if (iter->m_connectionName.
						     compare(connName) == 0)
							 {
							QString errMsg;
							pConn->flags &=
							    ~CONFIG_VOLATILE_CONN;
							if (pConn->association.
							     auth_type ==
							     AUTH_EAP)
				      ...
				      iter != m_credVector.end(); iter++)
					     ...
					     compare(connName) == 0)
							...
							psk != NULL)
							      ...
							      association.psk);
						pConn->association.psk =
						    _strdup(iter->m_password.
							    toAscii().data());
						     ...
						     (config_type,
						      pConn) == REQUEST_SUCCESS)
							    ...
							    critical(NULL,
								     ...
								     ("error"),
								     tr
								     ("Unable to save your PSK password"));
...
void CredentialsManager::connectionDisconnected(const QString & intName) 
{
	    ...
	    // this should be a safe course of action
		    ...
		    xsupgui_request_get_devdesc(intName.toAscii().data(),
						&intDesc);
		if (retval == REQUEST_SUCCESS && intDesc != NULL)
			 {
			QVector <
			    CredentialsManager::CredData >::iterator iter;
