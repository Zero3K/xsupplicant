    ...
    ConnectionWizardData::ConnectionWizardData() 
{
	    ...
	    // get distinct name
	    m_connectionName =
	    XSupWrapper::getUniqueConnectionName(QWidget::tr("New Connection"));
...
} bool ConnectionWizardData::toProfileOuterIdentity(config_profiles *
						      const pProfile) 
{
	    ...
	    if (m_machineAuth == false)
				    ...
				    _strdup(m_username.toAscii().data());
			}
		
		else
			pProfile->identity =
			    _strdup(m_outerIdentity.toAscii().data());
		}
	return true;
}

bool ConnectionWizardData::toProfileEAP_AKAProtocol(config_profiles *
						      const pProfile)
{
		    ...
		    (config_eap_method *) malloc(sizeof(config_eap_method));
		if (pProfile->method == NULL)
			success = false;
		
		else
			 {
			memset(pProfile->method, 0x00,
				sizeof(config_eap_method));
			pProfile->method->method_num = EAP_TYPE_AKA;
			pProfile->method->method_data =
			    (config_eap_aka *) malloc(sizeof(config_eap_aka));
			if (pProfile->method->method_data == NULL)
				success = false;
			
			else
				 {
				memset(pProfile->method->method_data, 0x00,
					sizeof(config_eap_aka));
				akaData =
				    (struct config_eap_aka *)pProfile->method->
				    method_data;
				akaData->reader =
				    _strdup(m_SCreader.toAscii().data());
...
bool ConnectionWizardData::toProfileEAP_SIMProtocol(config_profiles *
						      const pProfile)
{
		    ...
		    (config_eap_method *) malloc(sizeof(config_eap_method));
			    ...
			    (config_eap_sim *) malloc(sizeof(config_eap_sim));
					...
					sizeof(config_eap_sim));
				simData =
				    (struct config_eap_sim *)pProfile->method->
				    method_data;
...
// assumes profile is being built from scratch and thus none of this data is populated
    bool ConnectionWizardData::toProfileEAP_MD5Protocol(config_profiles *
							const pProfile)
{
		    ...
		    (config_eap_method *) malloc(sizeof(config_eap_method));
			    ...
			    (config_pwd_only *) malloc(sizeof(config_pwd_only));
					...
					sizeof(config_pwd_only));
				md5 =
				    (config_pwd_only *) pProfile->method->
				    method_data;
				if (!m_password.isEmpty())
					 {
					md5->password =
					    _strdup(m_password.toAscii().
						    data());
...
bool ConnectionWizardData::toProfileEAP_PEAPProtocol(config_profiles *
						       const pProfile,
								    ...
								    * mscv2;
								    ...
								    (m_machineAuth
								     == true)
									    ...
									    (mscv2->
									     flags,
									     FLAGS_EAP_MSCHAPV2_MACHINE_AUTH);
...
bool ConnectionWizardData::toProfileEAP_TLSProtocol(config_profiles *
						      const pProfile,
						      config_trusted_server
						      const *const pServer)
{
		    ...
		    // We don't want to set the outer username here because it is the only username.  (So we want to prompt.)
			    ...
			    malloc(sizeof(struct config_eap_method));
				    ...
				    malloc(sizeof(config_eap_tls));
					    ...
					    (config_eap_tls *) pProfile->
					    method->method_data;
					memset(mytls, 0x00,
						sizeof(config_eap_tls));
					
					    // server cert
					    if (pServer != NULL)
						 {
						mytls->trusted_server =
						    _strdup(pServer->name);
						...
						SET_FLAG(mytls->flags,
							  EAP_TLS_FLAGS_SESSION_RESUME);
					
					else
						UNSET_FLAG(mytls->flags,
							    EAP_TLS_FLAGS_SESSION_RESUME);
...
bool ConnectionWizardData::toProfileEAP_FASTProtocol(config_profiles *
						       const pProfile,
						       config_trusted_server
						       const *const pServer)
{
			    ...
			    malloc(sizeof(struct config_eap_method));
				    ...
				    malloc(sizeof(config_eap_fast));
					    ...
					    (config_eap_fast *) pProfile->
					    method->method_data;
					memset(myfast, 0x00,
						sizeof(config_eap_fast));
						...
						UNSET_FLAG(myfast->flags,
							    EAP_FAST_PROVISION_ANONYMOUS);
					if (m_authenticatedProvisioning ==
					      true)
...
bool ConnectionWizardData::toProfileEAP_TTLSProtocol(config_profiles *
						       const pProfile,
						       config_trusted_server
						       const *const pServer)
{
	    ...
	    if (m_eapProtocol != ConnectionWizardData::eap_ttls)
		    ...
		    (config_eap_method *) malloc(sizeof(config_eap_method));
			    ...
			    malloc(sizeof(config_eap_ttls));
				    ...
				    (config_eap_ttls *) pProfile->method->
				    method_data;
						...
						SET_FLAG(myttls->flags,
							  TTLS_FLAGS_VALIDATE_SERVER_CERT);
						myttls->trusted_server =
						    _strdup(pServer->name);
				    ...
				    if (m_innerTTLSProtocol ==
					ConnectionWizardData::inner_pap)
					 {
					myttls->phase2_type =
					    (ttls_phase2_type) TTLS_PHASE2_PAP;
					myttls->phase2_data =
					    (config_pwd_only *)
					    malloc(sizeof(config_pwd_only));
					if (myttls->phase2_data == NULL)
						success = false;
					
					else
						 {
						memset(myttls->phase2_data,
							0x00,
							sizeof
							(config_pwd_only));
				...
				else if (m_innerTTLSProtocol ==
					 ConnectionWizardData::inner_chap)
					 {
					myttls->phase2_type =
					    (ttls_phase2_type) TTLS_PHASE2_CHAP;
					 ...
					 ConnectionWizardData::inner_eap_md5)
					    ...
					    (ttls_phase2_type) TTLS_PHASE2_EAP;
					myttls->phase2_data =
					    (config_eap_method *)
					    malloc(sizeof(config_eap_method));
					   ...
					   pServer)
{
		...
		||(m_wireless == true 
		   &&(m_wirelessAssocMode ==
		    ...
		    // create profile
		    success =
		    XSupWrapper::createNewProfile(m_profileName, &pProfile,
				    ...
				    this->toProfileEAP_PEAPProtocol(pProfile,
								    pServer);
				break;
			case ConnectionWizardData::eap_ttls:
				success =
				    this->toProfileEAP_TTLSProtocol(pProfile,
								    pServer);
				break;
			case ConnectionWizardData::eap_fast:
				success =
				    this->toProfileEAP_FASTProtocol(pProfile,
								    pServer);
				break;
			case ConnectionWizardData::eap_aka:
				success =
				    this->toProfileEAP_AKAProtocol(pProfile);
				break;
			case ConnectionWizardData::eap_sim:
				success =
				    this->toProfileEAP_SIMProtocol(pProfile);
				break;
			case ConnectionWizardData::eap_md5:
				success =
				    this->toProfileEAP_MD5Protocol(pProfile);
				break;
			case ConnectionWizardData::eap_tls:
				success =
				    this->toProfileEAP_TLSProtocol(pProfile,
								   pServer);
					      ...
					      pProfile)
{
	    ...
	    XSupWrapper::createNewConnection(m_connectionName, &pConn,
					     m_newConnection == false);
	    ...
	    // save out. That = bad
				...
				pConn->association.association_type =
				    ASSOC_OPEN;
				...
				if (m_wirelessEncryptMeth ==
				     ConnectionWizardData::encrypt_CCMP)
					pConn->association.pairwise_keys |=
					    CRYPT_FLAGS_CCMP;
			    ...
			    _strdup(m_IPAddress.toAscii().data());
			if (m_gateway != "")
				pConn->ip.gateway =
				    _strdup(m_gateway.toAscii().data());
			pConn->ip.netmask =
			    _strdup(m_netmask.toAscii().data());
			pConn->ip.dns1 =
			    _strdup(m_primaryDNS.toAscii().data());
			if (m_secondaryDNS != "")
				pConn->ip.dns2 =
				    _strdup(m_secondaryDNS.toAscii().data());
		   ...
		   &&(m_wirelessAssocMode ==
			...
			((m_eapProtocol == ConnectionWizardData::eap_peap
			 ...
			 && m_validateCert == true))
			    ...
			    XSupWrapper::createNewTrustedServer(m_serverName,
								&pServer,
								(m_newConnection
								 == false
								 && m_hasServer
					    ...
					    _strdup((m_commonNames.join(",")).
						    toAscii().data());
					UNSET_FLAG(pServer->flags,
					    ...
					    (char **)malloc(numCerts *
							    sizeof(char *));
					if (pServer->location != NULL)
						 {
						memset(pServer->location, 0x00,
							numCerts *
							sizeof(char *));
...
bool ConnectionWizardData::toSupplicantProfiles(config_connection **
						  retConnection,
						  config_profiles ** retProfile,
						  config_trusted_server **
						  retServer) 
{
	...
	success = this->toProfileData(&pProfile, pServer) == true
	    && success == true;
	success = this->toConnectionData(&pConn, pProfile) == true
	    && success == true;
			   ...
			   config_trusted_server const *const pServer)
{
	    ...
	    m_wireless = pConfig->ssid != NULL
	    && QString(pConfig->ssid).isEmpty() == false;
		...
		if ((pConfig->association.pairwise_keys & CRYPT_FLAGS_CCMP) ==
		      CRYPT_FLAGS_CCMP)
			m_wirelessEncryptMeth =
			    ConnectionWizardData::encrypt_CCMP;
		
		else if ((pConfig->association.
			  pairwise_keys & CRYPT_FLAGS_CCMP) == CRYPT_FLAGS_TKIP)
		    ...
		    (pConfig->flags & CONFIG_NET_IS_HIDDEN) ==
		    CONFIG_NET_IS_HIDDEN;
		    ...
		    m_wiredSecurity = (pConfig->profile != NULL
				       && QString(pConfig->profile).isEmpty() ==
				       false);
			...
			if (QString(pProfile->identity) !=
			     QString("anonymous"))
				m_outerIdentity = QString(pProfile->identity);
			}
		if (pProfile->method != NULL)
			 {
			config_eap_method * pEAPMethod =
			    (config_eap_method *) pProfile->method;
		...
		if (pServer->common_name != NULL
		     && QString(pServer->common_name).isEmpty() == false)
			 {
			m_commonNames =
			    QString(pServer->common_name).split(",");
...
void ConnectionWizardData::initFromEAP_AKAProtocol(config_eap_method *
						     method) 
{
...
void ConnectionWizardData::initFromEAP_SIMProtocol(config_eap_method *
						     method) 
{
...
void ConnectionWizardData::initFromEAP_PEAPProtocol(config_eap_method *
						      method) 
{
	m_eapProtocol = ConnectionWizardData::eap_peap;
	if (method->method_data != NULL)
		 {
		config_eap_peap * pPEAPData =
		    (config_eap_peap *) method->method_data;
		if (pPEAPData->phase2 != NULL)
			 {
			m_username = pPEAPData->identity;
			if (TEST_FLAG
			      (pPEAPData->flags, EAP_TLS_FLAGS_SESSION_RESUME))
		      ...
		      (pPEAPData->flags, FLAGS_PEAP_VALIDATE_SERVER_CERT))
...
void ConnectionWizardData::initFromEAP_TTLSProtocol(config_eap_method *
						      method) 
{
	m_eapProtocol = ConnectionWizardData::eap_ttls;
	if (method->method_data != NULL)
		 {
		config_eap_ttls * pTTLSData =
		    (config_eap_ttls *) method->method_data;
				...
				config_pwd_only * pConfig =
				    (config_pwd_only *) pTTLSData->phase2_data;
				...
				config_eap_method * inner_method =
				    (config_eap_method *) pTTLSData->
				    phase2_data;
		      ...
		      (pTTLSData->flags, TTLS_FLAGS_VALIDATE_SERVER_CERT))
...
void ConnectionWizardData::initFromEAP_FASTProtocol(config_eap_method *
						      method) 
{
	m_eapProtocol = ConnectionWizardData::eap_fast;
	if (method->method_data != NULL)
		 {
		config_eap_fast * pFASTData =
		    (config_eap_fast *) method->method_data;
		      ...
		      (pFASTData->flags, EAP_FAST_VALIDATE_SERVER_CERT))
		      ...
		      (pFASTData->flags, EAP_FAST_PROVISION_AUTHENTICATED))
...
void ConnectionWizardData::initFromEAP_MD5Protocol(config_eap_method *
						     method) 
{
	m_eapProtocol = ConnectionWizardData::eap_md5;
	if (method->method_data != NULL)
		 {
		config_pwd_only * pMD5Data =
		    (config_pwd_only *) method->method_data;
		m_password = pMD5Data->password;
		if (TEST_FLAG
		      (pMD5Data->flags, CONFIG_PWD_ONLY_USE_LOGON_CREDS))
...
void ConnectionWizardData::initFromEAP_TLSProtocol(config_eap_method *
						     method) 
{
	m_eapProtocol = ConnectionWizardData::eap_tls;
	if (method->method_data != NULL)
		 {
		config_eap_tls * pTLSData =
		    (config_eap_tls *) method->method_data;
