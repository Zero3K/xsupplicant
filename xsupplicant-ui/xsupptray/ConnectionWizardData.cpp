				...
				else
				{
					...
					else if (m_innerPEAPProtocol == ConnectionWizardData::inner_eap_gtc)
					{
						...
						else
						{
							...
							{
								memset(myeap->method_data,
									0x00, sizeof(config_pwd_only));
										 ...
										 pServer)
{
	...
	{
		// create profile
		success = XSupWrapper::createNewProfile(m_profileName, &pProfile,
...
bool ConnectionWizardData::toServerData(config_trusted_server ** retServer) 
{
		...
		||m_wirelessAssocMode == ConnectionWizardData::assoc_WPA2_ENT)))
	{
		...
		if ((m_eapProtocol == ConnectionWizardData::eap_tls) || 
			...
			&& m_validateCert == true))
		{
			success = XSupWrapper::createNewTrustedServer(m_serverName,
				&pServer, (m_newConnection == false && m_hasServer == true));
						   ...
						   config_trusted_server const *const pServer)
{
	...
	else
	{
		...
		m_wiredSecurity = (pConfig->profile != NULL
			&& QString(pConfig->profile).isEmpty() == false);
	}
