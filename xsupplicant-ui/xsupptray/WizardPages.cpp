...
} bool WizardPageNetworkType::create(void) 
{
	    ...
	    int_enum * pInterfaceList = NULL;
		    ...
		    if (m_numWiredAdapters != 0 || m_numWirelessAdapters != 0)
				    ...
				    setDisabled(m_numWirelessAdapters == 0);
			if (m_pRadioButtonWired != NULL)
				m_pRadioButtonWired->
				    setDisabled(m_numWiredAdapters == 0);
...
const ConnectionWizardData & WizardPageNetworkType::wizardData(void) 
{
	      ...
	      && m_pRadioButtonWireless->isChecked() == true)
...
bool WizardPageWiredSecurity::create(void)
{
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelMessage");
		    ...
		    setText(tr
			    ("What type of network security (if any) is employed on the network to which you wish to connect?"));
	m_pRadioButtonDot1X =
	    qFindChild < QRadioButton * >(m_pRealForm, "radioDot1X");
	if (m_pRadioButtonDot1X != NULL)
		m_pRadioButtonDot1X->setText(tr("802.1X (default)"));
	m_pRadioButtonNone =
	    qFindChild < QRadioButton * >(m_pRealForm, "radioNone");
...
const ConnectionWizardData & WizardPageWiredSecurity::wizardData(void) 
{
	     ...
	     && m_pRadioButtonDot1X->isChecked() == true)
		m_curData.m_wiredSecurity = true;
	
	else if (m_pRadioButtonNone != NULL
		 && m_pRadioButtonNone->isChecked() == true)
...
bool WizardPageIPOptions::create(void)
{
	    ...
	    FormLoader::buildform("wizardPageIPOptions.ui", m_pParentWidget);
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelMessage");
		    ...
		    setText(tr
			    ("How would you like to obtain an IP address? <i>(If you are unsure, use the default setting)<i>"));
	m_pRadioButtonAuto =
	    qFindChild < QRadioButton * >(m_pRealForm, "radioAuto");
	if (m_pRadioButtonAuto != NULL)
		m_pRadioButtonAuto->
		    setText(tr("Obtain an IP Address Automatically (default)"));
	m_pRadioButtonStatic =
	    qFindChild < QRadioButton * >(m_pRealForm, "radioStatic");
	if (m_pRadioButtonStatic != NULL)
		m_pRadioButtonStatic->
		    setText(tr("Use Static IP Address Settings"));
...
const ConnectionWizardData & WizardPageIPOptions::wizardData(void) 
{
	...
	else if (m_pRadioButtonStatic != NULL
		 && m_pRadioButtonStatic->isChecked())
...
bool WizardPageStaticIP::create(void)
{
	    ...
	    FormLoader::buildform("wizardPageStaticIP.ui", m_pParentWidget);
	if (m_pRealForm == NULL)
		return false;
	
	    // cache off pointers to UI objects
	    m_pIPAddress =
	    qFindChild < QLineEdit * >(m_pRealForm, "dataFieldIPAddress");
	m_pNetmask =
	    qFindChild < QLineEdit * >(m_pRealForm, "dataFieldNetmask");
	m_pGateway =
	    qFindChild < QLineEdit * >(m_pRealForm, "dataFieldGateway");
	m_pPrimaryDNS =
	    qFindChild < QLineEdit * >(m_pRealForm, "dataFieldPrimaryDNS");
	m_pSecondaryDNS =
	    qFindChild < QLineEdit * >(m_pRealForm, "dataFieldSecondaryDNS");
	
	    // dynamically populate text
	    QLabel * pLabel =
	    qFindChild < QLabel * >(m_pRealForm, "headerStaticIP");
...
bool WizardPageStaticIP::validate(void)
{
	    ...
	    if (m_pIPAddress != NULL)
		     ...
		     ||(Util::isNetmaskValid(m_pNetmask->text()) == false))
					      ...
					      tr("Invalid Netmask"),
					      tr
					      ("Please provide a valid Netmask"));
		     ...
		     || (Util::isIPAddrValid(m_pPrimaryDNS->text()) == false))
			 {
			QMessageBox::warning(m_pRealForm,
					      tr("Invalid Primary DNS Server"),
					      tr
					      ("Please provide a valid Primary DNS Server address"));
		     ...
		     && ((m_pSecondaryDNS->hasAcceptableInput() == false) 
			 ||(Util::isIPAddrValid(m_pSecondaryDNS->text()) ==
			    false)))
					      ...
					      tr
					      ("Please provide a valid Secondary DNS Server address"));
...
bool WizardPageFinished::create(void)
{
	    ...
	    FormLoader::buildform("wizardPageFinished.ui", m_pParentWidget);
	if (m_pRealForm == NULL)
		return false;
	
	    // cache off pointers to objects        
	    m_pConnectionName =
	    qFindChild < QLineEdit * >(m_pRealForm, "dataFieldConnectionName");
	m_pConnectButton =
	    qFindChild < QPushButton * >(m_pRealForm, "buttonConnect");
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelMessage");
		    ...
		    setText(tr
			    ("Your network connection is now configured.\n\nPlease enter a name for the connection profile and press \"Finish\" to close the wizard."));
	QLabel * pConnNameLabel =
	    qFindChild < QLabel * >(m_pRealForm, "labelConnectionName");
...
bool WizardPageFinished::validate(void) 
{
					      ...
					      tr
					      ("Please input a valid name for the connection."));
			      ...
			      getConfigConnection(CONFIG_LOAD_GLOBAL, connName,
						  &pConfig) == true)
			     || (XSupWrapper::
				  getConfigConnection(CONFIG_LOAD_USER,
						      connName,
						      &pConfig) == true))
				    ...
				    QMessageBox::warning(m_pRealForm,
							 ...
							 tr
							 ("A connection already exists with the name you have provided.  Please choose another name."));
...
WizardPageWirelessNetwork::~WizardPageWirelessNetwork() 
{
		...
		Util::myDisconnect(m_pRadioButtonVisible,
				    SIGNAL(clicked(bool)), this,
				    SLOT(handleVisibleClicked(bool)));
	if (m_pSSIDList != NULL)
		 {
		Util::myDisconnect(m_pSSIDList,
				    SIGNAL(ssidSelectionChange
					   (const WirelessNetworkInfo &)), this,
				    SLOT(handleSSIDSelection
					 (const WirelessNetworkInfo &)));
		delete m_pSSIDList;
		}
} bool WizardPageWirelessNetwork::create(void) 
{
	m_pRealForm =
	    FormLoader::buildform("wizardPageWirelessNetwork.ui",
				  m_pParentWidget);
	if (m_pRealForm == NULL)
		return false;
	
	    // cache pointers to UI objets
	    m_pRadioButtonVisible =
	    qFindChild < QRadioButton * >(m_pRealForm, "radioVisible");
	m_pRadioButtonOther =
	    qFindChild < QRadioButton * >(m_pRealForm, "radioOther");
	m_pTableWidget =
	    qFindChild < QTableWidget * >(m_pRealForm,
					  "dataTableAvailableWirelessNetworks");
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelMessage");
		    ...
		    setText(tr
			    ("Choose a wireless network from the list to connect to, or choose \"Other\" to enter network information manually."));
		    ...
		    new SSIDList(this, m_pTableWidget,
				 m_pTableWidget->rowCount());
	    ...
	    if (m_pRadioButtonVisible != NULL)
		Util::myConnect(m_pRadioButtonVisible, SIGNAL(clicked(bool)),
				 this, SLOT(handleVisibleClicked(bool)));
	if (m_pSSIDList != NULL)
		Util::myConnect(m_pSSIDList,
				 SIGNAL(ssidSelectionChange
					(const WirelessNetworkInfo &)), this,
				 SLOT(handleSSIDSelection
				      (const WirelessNetworkInfo &)));
...
void WizardPageWirelessNetwork::
handleSSIDSelection(const WirelessNetworkInfo & networkData) 
{
...
void WizardPageWirelessNetwork::init(const ConnectionWizardData & data) 
{
		    ...
		    if (m_pSSIDList->selectNetwork(m_curData.m_networkName) ==
			true)
...
const ConnectionWizardData & WizardPageWirelessNetwork::wizardData(void) 
{
	    ...
	    if (m_pRadioButtonVisible != NULL
		&& m_pRadioButtonVisible->isChecked() == true)
				...
				m_curData.m_wirelessAssocMode =
				    ConnectionWizardData::assoc_none;
				break;
			case WirelessNetworkInfo::SECURITY_STATIC_WEP:
				m_curData.m_wirelessAssocMode =
				    ConnectionWizardData::assoc_WEP;
				break;
			case WirelessNetworkInfo::SECURITY_WPA2_PSK:
				m_curData.m_wirelessAssocMode =
				    ConnectionWizardData::assoc_WPA2_PSK;
				break;
			case WirelessNetworkInfo::SECURITY_WPA_PSK:
				m_curData.m_wirelessAssocMode =
				    ConnectionWizardData::assoc_WPA_PSK;
	...
	else if (m_pRadioButtonOther != NULL
		 && m_pRadioButtonOther->isChecked() == true)
...
WizardPageWirelessInfo::~WizardPageWirelessInfo() 
{
		...
		Util::myDisconnect(m_pHiddenNetwork, SIGNAL(stateChanged(int)),
				    this, SLOT(hiddenStateChanged(int)));
	if (m_pAssocMode != NULL)
		Util::myDisconnect(m_pAssocMode,
				    SIGNAL(currentIndexChanged(int)), this,
				    SLOT(assocModeChanged(int)));
} bool WizardPageWirelessInfo::create(void) 
{
	m_pRealForm =
	    FormLoader::buildform("wizardPageWirelessInfo.ui", m_pParentWidget);
	if (m_pRealForm == NULL)
		return false;
	
	    // cache pointers to UI objets
	    m_pNetworkName =
	    qFindChild < QLineEdit * >(m_pRealForm, "dataFieldNetworkName");
	m_pAssocMode =
	    qFindChild < QComboBox * >(m_pRealForm, "comboBoxSecurity");
	m_pEncryption =
	    qFindChild < QComboBox * >(m_pRealForm, "comboBoxEncryption");
	m_pHiddenNetwork =
	    qFindChild < QCheckBox * >(m_pRealForm, "checkBoxHidden");
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelMessage");
		    ...
		    setText(tr
			    ("Please provide details about the wireless network you would like to connect to.  If you are unsure of this information, click \"Back\" and choose from the list of visible wireless networks."));
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelEncryption");
	    ...
	    // set up event handling
	    if (m_pHiddenNetwork != NULL)
		Util::myConnect(m_pHiddenNetwork, SIGNAL(stateChanged(int)),
				 this, SLOT(hiddenStateChanged(int)));
	if (m_pAssocMode != NULL)
		Util::myConnect(m_pAssocMode, SIGNAL(currentIndexChanged(int)),
				 this, SLOT(assocModeChanged(int)));
	    ...
	    if (m_pHiddenNetwork != NULL) {
		    ...
		    setValidator(new
				 QRegExpValidator(QRegExp("^[\\w|\\W]{1,32}$"),
						  m_pNetworkName));
...
void WizardPageWirelessInfo::init(const ConnectionWizardData & data) 
{
		...
		this->hiddenStateChanged(m_curData.
					  m_hiddenNetwork ? Qt::Checked : Qt::
					  Unchecked);
...
const ConnectionWizardData & WizardPageWirelessInfo::wizardData(void) 
{
			...
			m_curData.m_wirelessEncryptMeth =
			    ConnectionWizardData::encrypt_TKIP;
			break;
		case 1:
			m_curData.m_wirelessEncryptMeth =
			    ConnectionWizardData::encrypt_CCMP;
			break;
		case 2:
			m_curData.m_wirelessEncryptMeth =
			    ConnectionWizardData::encrypt_WEP;
...
bool WizardPageWirelessInfo::validate(void)
{
					      ...
					      tr("Invalid Network Name"),
					      tr
					      ("The name you entered for the wireless network is invalid"));
...
void WizardPageWirelessInfo::hiddenStateChanged(int newState) 
{
			    ...
			    if (m_pAssocMode != NULL
				&& (m_pAssocMode->currentIndex() == 0
				    || m_pAssocMode->currentIndex() == 1))
...
bool WizardPageDot1XProtocol::create(void) 
{
	    ...
	    FormLoader::buildform("wizardPageDot1XProtocol.ui",
				  m_pParentWidget);
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelMessage");
		    ...
		    setText(tr
			    ("Indicate the EAP protocol to use for 802.1X authentication:"));
...
const ConnectionWizardData & WizardPageDot1XProtocol::wizardData(void) 
{
			    ...
			    // EAP-PEAP
			    m_curData.m_eapProtocol =
			    ConnectionWizardData::eap_peap;
...
WizardPageFASTInnerProtocol::~WizardPageFASTInnerProtocol() 
{
		...
		Util::myDisconnect(m_pAnonymousProvision,
				    SIGNAL(toggled(bool)), this,
				    SLOT(slotToggleRadioButton(bool)));
...
bool WizardPageFASTInnerProtocol::create(void) 
{
	    ...
	    FormLoader::buildform("wizardPageFASTInnerProtocol.ui",
				  m_pParentWidget);
	    ...
	    qFindChild < QCheckBox * >(m_pRealForm, "checkBoxValidateCert");
	m_pAnonymousProvision =
	    qFindChild < QRadioButton * >(m_pRealForm, "unauthProv");
	if (m_pAnonymousProvision != NULL)
		m_pAnonymousProvision->
		    setText(tr("Use anonymous provisioning"));
	m_pAuthenticatedProvision =
	    qFindChild < QRadioButton * >(m_pRealForm, "authProv");
	if (m_pAuthenticatedProvision != NULL)
		m_pAuthenticatedProvision->
		    setText(tr("Use authenticated provisioning"));
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelMessage");
		    ...
		    setText(tr
			    ("Enter your FAST settings for 802.1X authentication below.  The Outer Identity will be sent unencrypted."));
	...
	Util::myConnect(m_pAnonymousProvision, SIGNAL(toggled(bool)), this,
			  SLOT(slotToggleRadioButton(bool)));
...
void WizardPageFASTInnerProtocol::init(const ConnectionWizardData & data) 
{
		...
		if (m_curData.m_innerFASTProtocol ==
		      ConnectionWizardData::inner_eap_mschapv2)
			m_pProtocol->setCurrentIndex(0);
		
		else if (m_curData.m_innerFASTProtocol ==
			 ConnectionWizardData::inner_eap_gtc)
...
WizardPageDot1XCert::~WizardPageDot1XCert() 
{
		...
		Util::myDisconnect(m_pVerifyName, SIGNAL(stateChanged(int)),
				    this, SLOT(handleValidateChecked(int)));
	if (m_pCertTable != NULL) {
		Util::myDisconnect(m_pCertTable, SIGNAL(cellClicked(int, int)),
				    this, SLOT(handleCertTableClick(int, int)));
	}
} bool WizardPageDot1XCert::create(void) 
{
	m_pRealForm =
	    FormLoader::buildform("wizardPageDot1XCert.ui", m_pParentWidget);
	if (m_pRealForm == NULL)
		return false;
	
	    // cache off pointers to UI objects     
	    m_pCertTable =
	    qFindChild < QTableWidget * >(m_pRealForm, "tableCertList");
	m_pNameField =
	    qFindChild < QLineEdit * >(m_pRealForm, "dataFieldCommonName");
	m_pVerifyName =
	    qFindChild < QCheckBox * >(m_pRealForm, "checkBoxVerifyName");
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelMessage");
		    ...
		    setText(tr
			    ("Choose a Trusted CA Certificate to validate against:"));
	QLabel * pLabel =
	    qFindChild < QLabel * >(m_pRealForm, "labelNameInstructions");
	if (pLabel != NULL)
		pLabel->
		    setText(tr
			    ("Use \"*\" for prefix wildcarding and commas to separate multiple entries"));
	if (m_pVerifyName != NULL)
		m_pVerifyName->setText(tr("Verify Server Common Name"));
	
	    // set up event handling
	    if (m_pVerifyName != NULL)
		Util::myConnect(m_pVerifyName, SIGNAL(stateChanged(int)), this,
				 SLOT(handleValidateChecked(int)));
	if (m_pCertTable != NULL) {
		Util::myConnect(m_pCertTable, SIGNAL(cellClicked(int, int)),
				 this, SLOT(handleCertTableClick(int, int)));
	    ...
	    if (m_pNameField != NULL)
		    ...
		    setValidator(new
				 QRegExpValidator(QRegExp
						  ("^(?:(?:(?:\\*)|(?:\\w{1,253}))(?:\\.\\w{1,253})+)(?:\\,(?:(?:(?:\\*)|(?:\\w{1,253}))(?:\\.\\w{1,253})+))*$"),
						  m_pNameField));
		...
		int minRows = m_pCertTable->rowCount();
		    ...
		    // signal
		    m_pCertTable->horizontalHeaderItem(1)->
		    setText(tr("Trusted Certification Authority"));
		m_pCertTable->horizontalHeader()->setResizeMode(1,
								 QHeaderView::
								 Stretch);
...
void WizardPageDot1XCert::init(const ConnectionWizardData & data) 
{
		...
		handleValidateChecked(m_curData.
				       m_verifyCommonName ? Qt::Checked : Qt::
				       Unchecked);
	}
	if (m_pNameField != NULL)
		m_pNameField->setText(m_curData.m_commonNames.
				       join(QString(",")));
	    ...
	    if (m_pCertTable != NULL)
			...
			while (m_pCertArray[m_numCerts].certname != NULL)
				    ...
				    m_pCertTable->cellWidget(i, 0);
				if (widget != NULL)
					 {
					int index = item->type() - 1000;
					if (m_pCertArray != NULL && index > 0
					      && index < m_numCerts
					      && m_curData.m_serverCerts.
					      contains(m_pCertArray[index].
						       location))
...
const ConnectionWizardData & WizardPageDot1XCert::wizardData() 
{
		...
		m_curData.m_commonNames =
		    m_pNameField->text().split(QString(","));
...
bool WizardPageSCReader::create(void)
{
	    ...
	    FormLoader::buildform("wizardPageSIMReader.ui", m_pParentWidget);
	if (m_pRealForm == NULL)
		return false;
	
	    // cache off pointers to objects        
	    m_pReader =
	    qFindChild < QComboBox * >(m_pRealForm, "comboBoxReader");
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelReader");
	    ...
	    qFindChild < QCheckBox * >(m_pRealForm, "checkBoxAutoRealm");
...
void WizardPageSCReader::init(const ConnectionWizardData & data) 
{
				...
				m_pReader->setCurrentIndex(m_pReader->
							    findText(m_curData.
								     m_SCreader));
		...
		m_pAutoRealm->setChecked(m_curData.m_autoRealm);
...
} bool WizardPageDot1XUserCert::create(void) 
{
	    ...
	    FormLoader::buildform("wizardPageDot1XUserCert.ui",
				  m_pParentWidget);
		...
		m_pCertTable->horizontalHeader()->setResizeMode(0,
								 QHeaderView::
								 Stretch);
		
		    // issued to
		    m_pCertTable->horizontalHeaderItem(1)->
		    setText(tr("Issued To"));
		m_pCertTable->horizontalHeader()->setResizeMode(1,
								 QHeaderView::
								 Stretch);
		
		    // expires
		    m_pCertTable->horizontalHeaderItem(2)->
		    setText(tr("Expires"));
		m_pCertTable->horizontalHeader()->setResizeMode(2,
								 QHeaderView::
								 Stretch);
...
void WizardPageDot1XUserCert::init(const ConnectionWizardData & data) 
{
	    ...
	    if (m_pCertTable != NULL)
				      ...
				      && index < m_numCerts
				      && m_curData.m_userCert ==
				      QString(m_pCertArray[index].location))
...
const ConnectionWizardData & WizardPageDot1XUserCert::wizardData() 
{
			...
			if (item != NULL)
				m_curData.m_userCert =
				    QString(m_pCertArray[item->type() - 1000].
					    location);
...
bool WizardPageNetworkTypes::create(void)
{
	    ...
	    FormLoader::buildform("wizardPageSelectNetworkTypes.ui",
				  m_pParentWidget);
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelMessage");
		    ...
		    setText(tr
			    ("Please indicate the types of network you would like to use machine authentication on:"));
	m_pCheckBoxWired =
	    qFindChild < QCheckBox * >(m_pRealForm, "wiredCheckbox");
	if (m_pCheckBoxWired != NULL)
		m_pCheckBoxWired->setText(tr("Wired"));
	m_pCheckBoxWireless =
	    qFindChild < QCheckBox * >(m_pRealForm, "wirelessCheckbox");
...
const ConnectionWizardData & WizardPageNetworkTypes::wizardData(void) 
{
	      ...
	      && m_pCheckBoxWireless->isChecked() == true)
...
bool WizardPageMachineAuthFinished::create(void)
{
	    ...
	    FormLoader::buildform("wizardPageMachineAuthDone.ui",
				  m_pParentWidget);
		...
		pTemp->
		    setText(tr
			    ("Click Finish to save this configuration, or Back to go back and change options."));
...
void WizardPageMachineAuthFinished::init(const ConnectionWizardData & data) 
{
	    ...
	    tr
	    ("Machine authentication has been successfully configured<br>with the following options :<br><ul>");
	if (m_curData.m_wireless)
		resultData +=
		    QString("<li>") + tr("Machine Authentication on Wireless") +
		    QString("</li>");
	if (m_curData.m_wired)
		resultData +=
		    QString("<li>") + tr("Machine Authentication on Wired") +
		    QString("</li>");
	if ((!m_curData.m_wired) && (!m_curData.m_wireless))
		resultData +=
		    QString("<li>") + tr("Machine Authentication is Disabled") +
		    QString("</li>");
...
{
	m_curData.m_connectionName =
	    QString("Machine Authentication Connection");
	m_curData.m_profileName = QString("Machine Authentication Profile");
	m_curData.m_serverName =
	    QString("Machine Authentication Trusted Server");
...
bool WizardPageAuthOptions::create(void)
{
	    ...
	    FormLoader::buildform("wizardPageSelectAuthOptions.ui",
				  m_pParentWidget);
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelMessage");
		    ...
		    setText(tr
			    ("Please select any options you would like to enable for this configuration :"));
	m_pCheckBoxUseLogonCreds =
	    qFindChild < QCheckBox * >(m_pRealForm, "useLogonCreds");
	if (m_pCheckBoxUseLogonCreds != NULL)
		m_pCheckBoxUseLogonCreds->
		    setText(tr("Use logon credentials (if available)"));
...
void WizardPageAuthOptions::init(const ConnectionWizardData & data) 
{
		...
		m_pCheckBoxUseLogonCreds->setChecked(m_curData.
						      m_useLogonCreds);
...
const ConnectionWizardData & WizardPageAuthOptions::wizardData(void) 
{
	      ...
	      && m_pCheckBoxUseLogonCreds->isChecked() == true)
...
bool WizardPageCredentials::create(void)
{
	    ...
	    FormLoader::buildform("wizardPageCredentials.ui", m_pParentWidget);
	    ...
	    pMsgLabel = qFindChild < QLabel * >(m_pRealForm, "labelMessage");
		    ...
		    setText(tr
			    ("How would you like to obtain credentials? <i>(If you are unsure, use the default setting)<i>"));
	m_pRadioButtonPrompt =
	    qFindChild < QRadioButton * >(m_pRealForm, "radioPromptForCreds");
	if (m_pRadioButtonPrompt != NULL)
		m_pRadioButtonPrompt->
		    setText(tr("Prompt for credentials (default)"));
	m_pRadioButtonStore =
	    qFindChild < QRadioButton * >(m_pRealForm, "radioStoredCreds");
	    ...
	    qFindChild < QLabel * >(m_pRealForm, "labelUsername");
	if (m_pUsernameLabel != NULL)
		m_pUsernameLabel->setText(tr("Username : "));
	m_pPasswordLabel =
	    qFindChild < QLabel * >(m_pRealForm, "labelPassword");
	if (m_pPasswordLabel != NULL)
		m_pPasswordLabel->setText(tr("Password : "));
	m_pUsernameEdit =
	    qFindChild < QLineEdit * >(m_pRealForm, "usernameEdit");
	m_pPasswordEdit =
	    qFindChild < QLineEdit * >(m_pRealForm, "passwordEdit");
	Util::myConnect(m_pRadioButtonPrompt, SIGNAL(toggled(bool)), this,
...
const ConnectionWizardData & WizardPageCredentials::wizardData(void) 
{
	     ...
	     && (m_pRadioButtonPrompt->isChecked()))
	     ...
	     && (m_pRadioButtonStore->isChecked()))
			...
			&& (m_curData.m_innerPEAPProtocol ==
				...
				|| (m_pPasswordEdit->text() == ""))
						       ...
						       tr
						       ("You need to provide a username and password, or select the 'Prompt for credentials' option."));
						       ...
						       tr
						       ("You need to provide a username, or select the 'Prompt for credentials' option."));
...
{
	Util::myDisconnect(m_pRadioButtonPrompt, SIGNAL(toggled(bool)), this,
			    SLOT(slotToggled(bool)));
