...
{
	runInnerTest("1 createSystemProfileConfig()",
		      createSystemProfileConfig());
	runInnerTest("1 createUserProfileConfig()",
		       createUserProfileConfig());
	runInnerTest("1 checkSystemProfileConfig()",
		       checkSystemProfileConfig());
	runInnerTest("1 checkUserProfileConfig()", checkUserProfileConfig());
	runInnerTest("1 createVolatileSystemProfileConfig()",
		       createVolatileSystemProfileConfig());
	runInnerTest("1 createVolatileUserProfileConfig()",
		       createVolatileUserProfileConfig());
	runInnerTest("1 checkVolatileSystemProfileConfig()",
		       checkVolatileSystemProfileConfig(true));
	runInnerTest("1 checkVolatileUserProfileConfig()",
		       checkVolatileUserProfileConfig(true));
		...
		return false;
	runInnerTest("2 checkVolatileSystemProfileConfig()",
		       checkVolatileSystemProfileConfig(false));
	runInnerTest("2 checkVolatileUserProfileConfig()",
		       checkVolatileUserProfileConfig(false));
...
//      runInnerTest("1 checkUserSwitchToVolatile()", checkUserSwitchToVolatile());
	...
	runInnerTest("1 checkDeleteSystemProfileConfig()",
		       checkDeleteSystemProfileConfig());
	runInnerTest("1 checkDeleteUserProfileConfig()",
		       checkDeleteUserProfileConfig());
	if (runInnerTest("2 writeConfigs()", writeConfigs()) == false)
		return false;
	cout <<
	    "Finish implementation!!  (Need to test settings for all EAP methods)\n";
    ...
    bool ProfileConfigTests::createSystemProfileConfig() 
{
	    ...
	    (struct config_profiles *)malloc(sizeof(struct config_profiles));
	if (system_profile == NULL)
		 {
		innerError
		    ("Unable to allocate memory needed to create a test profile.\n");
	...
	memset(system_profile->method->method_data, 0x00,
		 sizeof(struct config_pwd_only));
	system_profile->next = NULL;
	if ((result =
	       xsupgui_request_set_profile_config(CONFIG_LOAD_GLOBAL,
						  system_profile)) !=
	      REQUEST_SUCCESS)
		 {
		innerError
		    ("Error creating new system level profile!  (Error : " +
		     Util::itos(result) + ")\n");
...
bool ProfileConfigTests::createUserProfileConfig() 
{
	...
	user_profile->method->method_data =
	    malloc(sizeof(struct config_pwd_only));
	...
	memset(user_profile->method->method_data, 0x00,
		 sizeof(struct config_pwd_only));
	user_profile->next = NULL;
	if ((result =
	       xsupgui_request_set_profile_config(CONFIG_LOAD_USER,
						  user_profile)) !=
	      REQUEST_SUCCESS)
		 {
		innerError("Error creating new profile!  (Error : " +
			    Util::itos(result) + ")\n");
...
bool ProfileConfigTests::checkSystemProfileConfig() 
{
	       ...
	       xsupgui_request_get_profile_config(CONFIG_LOAD_GLOBAL,
						  SYSTEM_CONF_NAME,
						  &prof)) != REQUEST_SUCCESS)
		 {
		innerError
		    ("Error reading system configuration test profile!  (Error : "
		     + Util::itos(result) + ")\n");
		return false;
		}
	if ((prof->name == NULL)
	      || (strcmp(prof->name, SYSTEM_CONF_NAME) != 0))
		    ...
		    ("Compliance isn't -256 when it should have been!\n");
		return false;
		}
	if (prof->temp_password != NULL)
		 {
		innerError
		    ("temp_password was non-NULL!  (Shouldn't be possible!)\n");
		return false;
		}
	if (prof->temp_username != NULL)
		 {
		innerError
		    ("temp_username was non-NULL!  (Shouldn't be possible!)\n");
...
bool ProfileConfigTests::checkUserProfileConfig() 
{
	       ...
	       xsupgui_request_get_profile_config(CONFIG_LOAD_USER,
		    ...
		    ("Error reading user configuration test profile!  (Error : "
		     + Util::itos(result) + ")\n");
...
bool ProfileConfigTests::checkVolatileSystemProfileConfig(bool expected) 
{
	      ...
	      || (strcmp(prof->name, SYSTEM_VOLATILE_CONF_NAME) != 0))
		    ...
		    ("Flags weren't set to volatile when they should have been!\n");
		return false;
		}
	if (prof->compliance != 0xffffff00)
		 {
		innerError
		    ("Compliance isn't -256 when it should have been! (Value = "
		     + Util::itos(prof->compliance) + ")\n");
...
bool ProfileConfigTests::checkVolatileUserProfileConfig(bool expected) 
{
	      ...
	      || (strcmp(prof->name, USER_VOLATILE_CONF_NAME) != 0))
...
{
	if (xsupgui_request_rename_profile
	     (CONFIG_LOAD_GLOBAL, SYSTEM_CONF_NAME,
	      SYSTEM_VOLATILE_CONF_NAME) == REQUEST_SUCCESS)
		 {
		innerError
		    ("Engine allowed a volatile config to be renamed to the name of a static config!\n");
		return false;
		}
	if (xsupgui_request_rename_profile
	      (CONFIG_LOAD_GLOBAL, SYSTEM_VOLATILE_CONF_NAME,
	       SYSTEM_CONF_NAME) == REQUEST_SUCCESS)
		 {
		innerError
		    ("Engine allowed a static config to be renamed to the name of a volatile config!\n");
		return false;
		}
	if (xsupgui_request_rename_profile
	      (CONFIG_LOAD_USER, USER_CONF_NAME,
	       USER_VOLATILE_CONF_NAME) == REQUEST_SUCCESS)
		 {
		innerError
		    ("Engine allowed a volatile user config to be renamed to the name of a static config!\n");
		return false;
		}
	if (xsupgui_request_rename_profile
	      (CONFIG_LOAD_USER, USER_VOLATILE_CONF_NAME,
	       USER_CONF_NAME) == REQUEST_SUCCESS)
		 {
		innerError
		    ("Engine allowed a static user config to be renamed to the name of a volatile config!\n");
...
bool ProfileConfigTests::checkSystemSwitchToVolatile() 
{
	       ...
	       xsupgui_request_get_profile_config(CONFIG_LOAD_GLOBAL,
		    ...
		    ("Unable to read the system configuration test profile. (Error : "
		     + Util::itos(result) + ")\n");
	       ...
	       xsupgui_request_set_profile_config(CONFIG_LOAD_GLOBAL,
		    ...
		    ("Engine allowed us to change a static configuration to volatile!\n");
...
bool ProfileConfigTests::checkBadDelete() 
{
	     ...
	     (CONFIG_LOAD_GLOBAL, USER_CONF_NAME, 1) == REQUEST_SUCCESS)
		 {
		innerError
		    ("Engine allowed us to delete a user level config with the system level flag!?\n");
		return false;
		}
	if (xsupgui_request_delete_profile_config
	      (CONFIG_LOAD_USER, SYSTEM_CONF_NAME, 1) == REQUEST_SUCCESS)
		 {
		innerError
		    ("Engine allowed us to delete a system level config with the user level flag!?\n");
...
bool ProfileConfigTests::checkDeleteSystemProfileConfig() 
{
	       ...
	       xsupgui_request_delete_profile_config(CONFIG_LOAD_GLOBAL,
						     SYSTEM_CONF_NAME,
						     1)) != REQUEST_SUCCESS)
		 {
		innerError
		    ("Unable to delete the profile we created for testing.  (Error : "
		     + Util::itos(result) + ")\n");
...
bool ProfileConfigTests::checkDeleteUserProfileConfig() 
{
	       ...
	       xsupgui_request_delete_profile_config(CONFIG_LOAD_USER,
						     USER_CONF_NAME,
						     1)) != REQUEST_SUCCESS)
    ...
    bool ProfileConfigTests::checkRename() 
{
	       ...
	       xsupgui_request_rename_profile(CONFIG_LOAD_GLOBAL,
					      SYSTEM_CONF_NAME,
					      SYSTEM_RENAME_CONF_NAME)) !=
	      REQUEST_SUCCESS)
		 {
		innerError("Unable to rename the profile!  (Error : " +
			    Util::itos(result) + ")\n");
	      ...
	      REQUEST_SUCCESS)
		    ...
		    ("Unable to rename the profile back to its original name!  (Error : "
		     + Util::itos(result) + ")\n");
		return false;
		}
	if ((result =
	       xsupgui_request_rename_profile(CONFIG_LOAD_USER, USER_CONF_NAME,
					      USER_RENAME_CONF_NAME)) !=
	      REQUEST_SUCCESS)
		 {
		innerError("Unable to rename the user profile!  (Error : " +
			    Util::itos(result) + ")\n");
		return false;
		}
	if ((result =
	       xsupgui_request_rename_profile(CONFIG_LOAD_USER,
					      USER_RENAME_CONF_NAME,
					      USER_CONF_NAME)) !=
	      REQUEST_SUCCESS)
		 {
		innerError
		    ("Unable to rename the user profile back to its original name!  (Error : "
		     + Util::itos(result) + ")\n");
...
bool ProfileConfigTests::checkProfileEnum() 
{
	       ...
	       xsupgui_request_enum_profiles(CONFIG_LOAD_USER,
					     &profenum)) != REQUEST_SUCCESS)
	      ...
	      || (foundInEnum(profenum, SYSTEM_CONF_NAME) == false))
...
bool ProfileConfigTests::foundInEnum(profile_enum * profenum, char *tofind)
{
		...
		if ((profenum[i].name != NULL)
		     && (strcmp(profenum[i].name, tofind) == 0))
			return true;
...
struct config_eap_method *ProfileConfigTests::createEAPMD5Test() 
{
	    ...
	    malloc(sizeof(struct config_eap_method));
		    ...
		    ("Couldn't allocate memory to store EAP configuration structure.\n");
		    ...
		    ("Couldn't allocate memory to store EAP-MD5 configuration structure.\n");
...
bool ProfileConfigTests::checkEAPMD5Test(struct config_eap_method * eapmd5)
{
		    ...
		    ("No EAP-MD5 configuration data was found in memory!\n");
...
bool ProfileConfigTests::checkEAPMD5Config() 
{
		...
		return false;	// createEAPMD5Test() should have already screamed.
	if ((result =
	      xsupgui_request_set_profile_config(CONFIG_LOAD_GLOBAL,
						 prof)) != REQUEST_SUCCESS)
		 {
		innerError
		    ("Unable to write EAP-MD5 config to engine.  (Error : " +
		     Util::itos(result) + ")\n");
		...
		innerError("Unable to read back the profile.  (Error : " +
			    Util::itos(result) + ")\n");
		return false;
		}
	if (checkEAPMD5Test(prof->method) == false)
		return false;	// The check should have screamed.
	if (freeEAPMD5Test(&prof->method) == false)
		return false;	// The check should have screamed.
	return true;
...
bool ProfileConfigTests::checkInvalidConfigDest() 
{
	      ...
	      REQUEST_SUCCESS)
		    ...
		    ("Unable to read the system profile that was written earlier. (Error : "
		     + Util::itos(result) + ")\n");
			...
			continue;
		       ...
		       xsupgui_request_get_profile_config(i, SYSTEM_CONF_NAME,
							  &temp)) !=
		      IPC_ERROR_INVALID_CONFIG)
			 {
			innerError
			    ("Attempt to read an invalid config didn't fail properly! (Error : "
			    ...
			    ("Failure to read an invalid config did not return a NULL value!\n");
			return false;
			}
		if ((result =
		       xsupgui_request_set_profile_config(i,
							  read_back_config)) !=
		      IPC_ERROR_INVALID_CONFIG)
			 {
			innerError
			    ("Attempt to write an invalid config didn't fail properly!  (Error : "
			     + Util::itos(result) + "  Iteration : " +
			     Util::itos(i) + ")\n");
...
bool ProfileConfigTests::checkEAPMSCHAPv2Config() 
{
		...
		return false;	// createEAPMSCHAPv2Test() should have already screamed.
	if ((result =
	      xsupgui_request_set_profile_config(CONFIG_LOAD_GLOBAL,
						 prof)) != REQUEST_SUCCESS)
		 {
		innerError("Unable to write config to engine.  (Error : " +
			    Util::itos(result) + ")\n");
	...
	if (checkEAPMSCHAPv2Test(prof->method) == false)
		return false;	// The check should have screamed.
	if (freeEAPMSCHAPv2Test(&prof->method) == false)
		return false;	// The check should have screamed.
	return true;
...
struct config_eap_method *ProfileConfigTests::createEAPMSCHAPv2Test() 
{
	    ...
	    malloc(sizeof(struct config_eap_method));
		    ...
		    ("Couldn't allocate memory to store EAP-MSCHAPv2 configuration structure.\n");
...
#define ALL_MSCHAPV2_FLAGS    (FLAGS_EAP_MSCHAPV2_VOLATILE | FLAGS_EAP_MSCHAPV2_IAS_QUIRK | FLAGS_EAP_MSCHAPV2_MACHINE_AUTH)
    bool ProfileConfigTests::checkEAPMSCHAPv2Test(struct config_eap_method *
						   eapmscv2)
{
		    ...
		    ("No EAP-MSCHAPv2 configuration data was found in memory!\n");
	...
	if (strcmp(mscv2data->nthash, "000102030405060708090a0b0c0d0e0f") !=
	      0)
		...
		innerError("Flags were invalid!  (Expected : " +
			    Util::
			    itos((FLAGS_EAP_MSCHAPV2_VOLATILE |
				  FLAGS_EAP_MSCHAPV2_IAS_QUIRK)) + "  Got : " +
			    Util::itos(mscv2data->flags) + ")\n");
		return false;
		}
	return true;
}

bool ProfileConfigTests::freeEAPMSCHAPv2Test(struct config_eap_method **
					       eapmscv2)
{
...
bool ProfileConfigTests::checkEAPAKAConfig() 
{
		...
		return false;	// createEAPAKATest() should have already screamed.
	...
	if (checkEAPAKATest(prof->method) == false)
		return false;	// The check should have screamed.
	if (freeEAPAKATest(&prof->method) == false)
		return false;	// The check should have screamed.
	return true;
...
struct config_eap_method *ProfileConfigTests::createEAPAKATest() 
{
	    ...
	    malloc(sizeof(struct config_eap_method));
		    ...
		    ("Couldn't allocate memory to store EAP-AKA configuration structure.\n");
...
bool ProfileConfigTests::checkEAPAKATest(struct config_eap_method * eapaka)
{
		    ...
		    ("No EAP-AKA configuration data was found in memory!\n");
...
bool ProfileConfigTests::checkEAPSIMConfig() 
{
		...
		return false;	// should have already screamed.
	...
	if (checkEAPSIMTest(prof->method) == false)
		return false;	// The check should have screamed.
	if (freeEAPSIMTest(&prof->method) == false)
		return false;	// The check should have screamed.
	return true;
...
struct config_eap_method *ProfileConfigTests::createEAPSIMTest() 
{
	    ...
	    malloc(sizeof(struct config_eap_method));
		    ...
		    ("Couldn't allocate memory to store EAP-SIM configuration structure.\n");
...
bool ProfileConfigTests::checkEAPSIMTest(struct config_eap_method * eapsim)
{
		    ...
		    ("No EAP-SIM configuration data was found in memory!\n");
...
bool ProfileConfigTests::checkEAPGTCConfig() 
{
	...
	if (checkEAPGTCTest(prof->method) == false)
		return false;	// The check should have screamed.
	if (freeEAPGTCTest(&prof->method) == false)
		return false;	// The check should have screamed.
	return true;
...
struct config_eap_method *ProfileConfigTests::createEAPGTCTest() 
{
	    ...
	    malloc(sizeof(struct config_eap_method));
		    ...
		    ("Couldn't allocate memory to store EAP-GTC configuration structure.\n");
...
bool ProfileConfigTests::checkEAPGTCTest(struct config_eap_method * eapgtc)
{
		    ...
		    ("No EAP-GTC configuration data was found in memory!\n");
...
bool ProfileConfigTests::checkEAPOTPConfig() 
{
	...
	if (checkEAPOTPTest(prof->method) == false)
		return false;	// The check should have screamed.
	if (freeEAPOTPTest(&prof->method) == false)
		return false;	// The check should have screamed.
	return true;
...
struct config_eap_method *ProfileConfigTests::createEAPOTPTest() 
{
	    ...
	    malloc(sizeof(struct config_eap_method));
		    ...
		    ("Couldn't allocate memory to store EAP-OTP configuration structure.\n");
...
bool ProfileConfigTests::checkEAPOTPTest(struct config_eap_method * eapotp)
{
		    ...
		    ("No EAP-OTP configuration data was found in memory!\n");
		    ...
		    ("EAP-OTP password was found when it shouldn't have been!\n");
...
bool ProfileConfigTests::checkEAPTLSConfig() 
{
	...
	if (checkEAPTLSTest(prof->method) == false)
		return false;	// The check should have screamed.
	if (freeEAPTLSTest(&prof->method) == false)
		return false;	// The check should have screamed.
	return true;
...
struct config_eap_method *ProfileConfigTests::createEAPTLSTest() 
{
	    ...
	    malloc(sizeof(struct config_eap_method));
		    ...
		    ("Couldn't allocate memory to store EAP-TLS configuration structure.\n");
...
bool ProfileConfigTests::checkEAPTLSTest(struct config_eap_method * eaptls)
{
		    ...
		    ("No EAP-TLS configuration data was found in memory!\n");
	...
	if (TEST_FLAG(tlsdata->flags, EAP_TLS_FLAGS_SESSION_RESUME) !=
...
bool ProfileConfigTests::checkEAPFASTConfig() 
{
	...
	if (checkEAPFASTTest(prof->method) == false)
		return false;	// The check should have screamed.
	if (freeEAPFASTTest(&prof->method) == false)
		return false;	// The check should have screamed.
	return true;
...
struct config_eap_method *ProfileConfigTests::createEAPFASTTest() 
{
	    ...
	    malloc(sizeof(struct config_eap_method));
		    ...
		    ("Couldn't allocate memory to store EAP-FAST configuration structure.\n");
...
bool ProfileConfigTests::checkEAPFASTTest(struct config_eap_method * eapfast)
{
		    ...
		    ("No EAP-FAST configuration data was found in memory!\n");
...
bool ProfileConfigTests::checkEAPPEAPConfig() 
{
	...
	if (checkEAPPEAPTest(prof->method) == false)
		return false;	// The check should have screamed.
	if (freeEAPPEAPTest(&prof->method) == false)
		return false;	// The check should have screamed.
	return true;
...
struct config_eap_method *ProfileConfigTests::createEAPPEAPTest() 
{
	    ...
	    malloc(sizeof(struct config_eap_method));
		    ...
		    ("Couldn't allocate memory to store EAP-PEAP configuration structure.\n");
...
#define ALL_PEAP_FLAGS   (EAP_TLS_FLAGS_SESSION_RESUME | FLAGS_PEAP_MACHINE_AUTH | FLAGS_PEAP_USE_LOGON_CREDS | FLAGS_PEAP_PROPER_PEAPV1_KEYS)	// Add more as needed. ;)
    bool ProfileConfigTests::checkEAPPEAPTest(struct config_eap_method *
					       eappeap)
{
		    ...
		    ("No EAP-PEAP configuration data was found in memory!\n");
