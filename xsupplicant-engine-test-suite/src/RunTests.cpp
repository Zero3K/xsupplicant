...
{
	// Clean up our tests.
	for (vector<TestBase*>::iterator i = nonNetworkTests.begin(); i != nonNetworkTests.end(); ++i)
	{
		delete (*i);
	}

	for (vector<TestBase*>::iterator i = networkTests.begin(); i != networkTests.end(); ++i)
	{
		delete (*i);
	}
}
...
void RunTests::executeNonNetworkTests()
{
	for (vector<TestBase*>::iterator i = nonNetworkTests.begin(); i != nonNetworkTests.end(); ++i)
	{
		if ((*i)->setupTest() == true)
		{
			cout << "Test : " << (*i)->getTestName() << " : Started\n";
			(*i)->executeTest();
			(*i)->teardownTest();
		}
		...
		if ((*i)->checkResults() == false)
		{
			...
			cout << "******  Test : " << (*i)->getTestName() << " - FAILED!\n\n\n";
			nonNetworkTests_failed++;

			if ((*i)->terminal_error() == true)
			{
				cout << " -- Test indicated this is a terminal error!\n";
				return;
			}
		}
		else
...
void RunTests::showScoreBoard()
{
	...
	cout << "-------------------------\n";
	cout << "  Success : " << left << setw(10) << Util::itos(nonNetworkTests_success) << setw(10) << "Failed : " << Util::itos(nonNetworkTests_failed) << "\n";
	cout << "\n";
	cout << "Network Enabled Tests\n";
	cout << "---------------------\n";
	cout << "  Success : " << left << setw(10) << Util::itos(networkTests_success) << setw(10) << "Failed : " << Util::itos(networkTests_failed) << "\n";
}