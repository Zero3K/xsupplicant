...
{
	runInnerTest("checkSettingsSurviveRestart()",
		      checkSettingsSurviveRestart());
	...
	runInnerTest("checkStaleKeyTimeoutRange()",
		       checkStaleKeyTimeoutRange());
	runInnerTest("checkAssocTimeoutRange()", checkAssocTimeoutRange());
	runInnerTest("checkPassiveTimeoutRange()",
		       checkPassiveTimeoutRange());
	runInnerTest("checkActiveTimeoutRange()", checkActiveTimeoutRange());
	runInnerTest("checkIdleWhileTimeoutRange()",
		       checkIdleWhileTimeoutRange());
	runInnerTest("checkPMKSAAgeOutRange()", checkPMKSAAgeOutRange());
	runInnerTest("checkPMKSACacheCheckRange()",
		       checkPMKSACacheCheckRange());
	runInnerTest("checkDeadConnectionTimeoutRange()",
		       checkDeadConnectionTimeoutRange());
...
bool GlobalConfigTests::checkLogsToKeepRange() 
{
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
		 {
		innerError("Unable to read configuration globals!  (Error : " +
			    Util::itos(result) + ")\n");
		       ...
		       xsupgui_request_set_globals_config(globals)) !=
		      REQUEST_SUCCESS)
			 {
			innerError
			    ("Unable to set configuration globals!  (Error : " +
			     Util::itos(result) + ")\n");
		       ...
		       xsupgui_request_get_globals_config(&globals)) !=
		      REQUEST_SUCCESS)
			 {
			innerError
			    ("Unable to read configuration globals!  (Error : "
			     + Util::itos(result) + ")\n");
			if (globals != NULL)
				 {
				innerError
				    ("AND globals pointer wasn't NULL!\n");
			    ...
			    ("Value passed to the engine wasn't returned when reread!  (Got : "
			     + Util::itos(globals->logs_to_keep) +
			     "  Expected : " + Util::itos(value) + ")\n");
...
bool GlobalConfigTests::checkSizeToRollRange() 
{
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
		      ...
		      REQUEST_SUCCESS)
			    ...
			    ("Value passed to the engine wasn't returned when reread!  (Got : "
			     + Util::itos(globals->size_to_roll) +
			     "  Expected : " + Util::itos(value) + ")\n");
		      ...
		      REQUEST_SUCCESS)
			    ...
			    ("Unable to read configuration globals!  (Error : "
			     + Util::itos(result) + ")\n");
				    ...
				    ("Value returned wasn't what was expected!  (Got : "
				     + Util::itos(globals->logtype) +
				     "  Expected : 0  Value : " +
				     Util::itos(value) + ")\n");
...
bool GlobalConfigTests::checkStaleKeyTimeoutRange() 
{
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
		      ...
		      REQUEST_SUCCESS)
			    ...
			    ("Value passed to the engine wasn't returned when reread!  (Got : "
			     + Util::itos(globals->stale_key_timeout) +
			     "  Expected : " + Util::itos(value) + ")\n");
...
bool GlobalConfigTests::checkAssocTimeoutRange() 
{
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
		      ...
		      REQUEST_SUCCESS)
			    ...
			    ("Value passed to the engine wasn't returned when reread!  (Got : "
			     + Util::itos(globals->assoc_timeout) +
			     "  Expected : " + Util::itos(value) + ")\n");
...
bool GlobalConfigTests::checkPassiveTimeoutRange() 
{
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
		      ...
		      REQUEST_SUCCESS)
			    ...
			    ("Value passed to the engine wasn't returned when reread!  (Got : "
			     + Util::itos(globals->passive_timeout) +
			     "  Expected : " + Util::itos(value) + ")\n");
...
bool GlobalConfigTests::checkActiveTimeoutRange() 
{
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
		      ...
		      REQUEST_SUCCESS)
			    ...
			    ("Value passed to the engine wasn't returned when reread!  (Got : "
			     + Util::itos(globals->active_timeout) +
			     "  Expected : " + Util::itos(value) + ")\n");
...
bool GlobalConfigTests::checkPMKSAAgeOutRange() 
{
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
		      ...
		      REQUEST_SUCCESS)
			    ...
			    ("Value passed to the engine wasn't returned when reread!  (Got : "
			     + Util::itos(globals->pmksa_age_out) +
			     "  Expected : " + Util::itos(value) + ")\n");
...
bool GlobalConfigTests::checkPMKSACacheCheckRange() 
{
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
		      ...
		      REQUEST_SUCCESS)
			    ...
			    ("Value passed to the engine wasn't returned when reread!  (Got : "
			     + Util::itos(globals->pmksa_cache_check) +
			     "  Expected : " + Util::itos(value) + ")\n");
...
bool GlobalConfigTests::checkDeadConnectionTimeoutRange() 
{
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
		      ...
		      REQUEST_SUCCESS)
			    ...
			    ("Value passed to the engine wasn't returned when reread!  (Got : "
			     + Util::itos(globals->dead_connection_timeout) +
			     "  Expected : " + Util::itos(value) + ")\n");
...
bool GlobalConfigTests::checkIdleWhileTimeoutRange() 
{
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
		      ...
		      REQUEST_SUCCESS)
			    ...
			    ("Value passed to the engine wasn't returned when reread!  (Got : "
			     + Util::itos(globals->idleWhile_timeout) +
			     "  Expected : " + Util::itos(value) + ")\n");
    ...
    bool GlobalConfigTests::checkFlags() 
{
	       ...
	       xsupgui_request_set_globals_config(globals)) != REQUEST_SUCCESS)
		 {
		innerError("Unable to set configuration globals!  (Error : " +
			    Util::itos(result) + ")\n");
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
		    ...
		    ("Flags read back don't match flags passed in!  (Expected : "
		     + Util::itos(KNOWN_CONFIG_FLAGS) + "  Got : " +
		     Util::itos(globals->flags) + ")\n");
...
bool GlobalConfigTests::checkLogFileLocation() 
{
	       ...
	       xsupgui_request_get_globals_config(&globals)) != REQUEST_SUCCESS)
			    ...
			    ("Attempt to create a temporary directory failed because it already existed!?\n");
			return false;
			break;
		case EEXIST:
			innerError
			    ("Attempted to create a directory that already exists.\n");
			    ...
			    ("Unable to create a directory for an unknown reason.\n");
...
bool GlobalConfigTests::globalConfigsMatch(struct config_globals * g1,
					     struct config_globals * g2)
{
			    ...
			    ("g1->logpath was not NULL, but g2->logpath was.\n");
			    ...
			    ("g1->logpath was NULL, but g2->logpath was not!\n");
			return false;
			}
		}
	if (strcmp(g1->logpath, g2->logpath) != 0)
		 {
		innerError("Log paths didn't match.  (Got : " +
			    string(g1->logpath) + "  Expected : " +
			    string(g2->logpath) + ")\n");
			    ...
			    ("g1->log_facility was NULL, but g2->log_facility wasn't!\n");
			    ...
			    ("g2->log_facility was NULL, but g1->log_facility wasn't!\n");
			    ...
			    ("g1->ipc_group_name was NULL, but g2->ipc_group_name wasn't!\n");
			    ...
			    ("g2->ipc_group_name was NULL, but g1->ipc_group_name wasn't!\n");
...
bool GlobalConfigTests::checkSettingsSurviveRestart() 
{
	       ...
	       xsupgui_request_get_globals_config(&saved_globals)) !=
	      REQUEST_SUCCESS)
	    ...
	    globals->loglevel = 68;
	       ...
	       xsupgui_request_get_globals_config(&reread_globals)) !=
	      REQUEST_SUCCESS)
	       ...
	       xsupgui_request_set_globals_config(saved_globals)) !=
	      REQUEST_SUCCESS)
	...
	if (globalConfigsMatch(globals, reread_globals) == false)
		return false;
